...
- 検証: Manager build 緑 / **テスト 154 件合格**（PR3a 12 + round1 1 [GC破損行保護] + round2 2 [partial世代で削除抑止 / 破損行で削除抑止+failed計上]、いずれも fail-first）。既存 snapshot テストも緑（META 8 フィールド化・SoT リファクタ無回帰）。
