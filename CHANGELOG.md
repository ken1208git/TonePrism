...
- **`SYSLIB0014` 抑止**: サービスモードのネット速度テスト（`Internet()` / #287）が `HttpWebRequest` + `ServicePointManager` を使う。net10 では obsolete だが shim として動作（接続数上限は SocketsHttpHandler 既定 unlimited ＝並列 DL 維持）。機械的移行で速度テストのロジック書き換えは behavioral リスクのため `<NoWarn>SYSLIB0014</NoWarn>` で抑止、**HttpClient 化は将来別 issue で検討**。
- bump: net10 移行という土台変更のため minor（v0.2.1 → v0.3.0）。Bundle 反映は v0.9.0。

**Note**: LauncherAgent は Manager「アップデート」タブの版数表示に未掲載（#310 で追加予定）のため、SPEC §3.7.8 の exe 版数公開前ゲート（`Assert-<Name>Version`）は現時点で対象外（「版数表示 ∧ stamp パイプライン」の両条件のうち「表示」が未成立）。#310 完了時に必要化。
