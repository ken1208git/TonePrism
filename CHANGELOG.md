...
- **画面下部に操作ボタン（戻る / スキップ / 進む）** を追加。**キーボード/コントローラーは `←`/`→` でボタン間をフォーカス移動し、`Enter`/決定 (`ui_accept`) で押下する形式**（中断メニューと同じ操作系。Godot 標準のフォーカス系に委ね、`_input` では `ui_left`/`ui_right`/`ui_accept` を consume しない）。マウス/タッチは直接クリック。初期フォーカスは「進む」、先頭スライドでは「戻る」を無効化（フォーカスは「進む」へ自動退避）、最終スライドでは「進む」を「ストアへ」表記に。選択中が分かるよう明るい太枠のフォーカススタイルを付与。`Esc` (`ui_cancel`) は素早いスキップ用ショートカットとして残置。マウス移動でカーソル表示・キー/パッドで非表示（store_browse と同じ分離）。ボタンバーは下端中央にアンカー+offset で高さ72pxの帯として明示配置（`MarginContainer`+preset の min-size タイミング依存で高さ0に潰れる問題を回避）。
