...
**Round 8 review fix (senior M-1 / M-2 / M-3 / M-4 + L-2 / L-5 / L-6 / L-7、L 系 3 件 defer)** — version bump なし、本 entry に統合:
...
- **L 系 defer 3 件 (merge 後 follow-up issue 候補)**:
  - **L-1**: `MainForm.cs:184` の `Task.Run(() => checker.CheckAsync(...))` 冗長 (`CheckAsync` は HttpClient.GetAsync 経由の真の async、`Task.Run` で thread pool を 1 つ消費 + 直ちに async state machine に戻る overhead が無駄)。動作影響なし、別 issue で sweep。
