...
#### Fixed (#312 — 説明文入力中の Enter で誤保存される不具合)

- **ゲーム追加 / 編集 / バージョンアップ各フォームの「説明文」入力欄で Enter を押すと改行されず保存が走っていた不具合を修正**。`txtDescription` は `Multiline = true` だが `AcceptsReturn` 未設定（=false）だったため、Enter が `AcceptButton`（保存ボタン）を発火していた。3 フォームの Designer で `AcceptsReturn = true` を設定し、Enter を**改行**にした。
- 検証: build 緑。**実機で各フォームの説明欄に複数行入力（Enter で改行・保存されない）・保存後に改行が反映されることを目視**。
