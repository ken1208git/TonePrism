...
- **`UpdateDownloader.cs` legacy validation list 同期**: `ValidateStagingLegacy` (L369-381) の hardcoded filename list を `GCTonePrism_*.exe` → `TonePrism_*.exe` 全件 update。本 PR で NEW Manager が NEW zip を apply する forward path は機能するが、OLD Manager v0.11.0 が NEW zip を validate する path は本 validation 失敗で abort する (= ハード切替の意図された behavior)
