...
- 検証: Manager build 緑 / **テスト 172 件合格**（PR3b 新規 16 件＝ペアリング 9: T以下で最大 / replace-in-session(db>manifest) / host一致優先 / host無し→全体最新 / T以前なし→null / 空 / auto+manual 横断 / 同秒tie→名前降順 / 不正ヘッダ skip ＋ eligibility 7: auto/manual のみ true・safety/unknown/空/null は false・大文字小文字非依存、いずれも fail-first）。**※実機 UI（2モード確認ダイアログ・レポートのアセット節・DB+assets round-trip・本番 SMB 体感・safety 復元で games が消えないこと）は pre-release で目視（F1〜F6）＝本 PR は #250 の実機 UI 初確認対象。**
