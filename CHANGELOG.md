...
- 検証: Manager build 緑 / **テスト 135 件合格**（コアレス 2 + round1 2 [B-1/C-1] + round2 1 [復元が壊れ/不完全を置換前に弾く] + round3 3 [#6 中止で pending アセットを未バックアップ警告化 / `CheckIntegrity` の分類 / 「開けるが非 ok」を `allowIntegrityWarnings` で gate]、いずれも fail-first で確認）。**※実機（UIは実機で確認）: 非ブロッキングで操作継続・進捗 1 段表示・中止→未バックアップ警告＋今すぐバックアップで復旧・閉じる確認、を目視済（残像/見切れ/枠なしの実機フィードバックは反映済）。本番 SMB での体感・多 PC・並行編集 churn（C-1）・worker 稼働中の復元/手動同時実行（#2/#4）・連続アセット操作直後の UI 応答（#1）・整合性警告の確認ダイアログ（#3）は pre-release 全体テストで。**
