...
- **ストアセクション編集フォームのソースドロップダウンから「人気ランキング」「最近プレイ」を一時的に外した** (`StoreSectionForm.cs` `AllSources`)。両ソースは実ランキング（responses の in-memory 集計）が #297 PR2 / v0.9.0 で未実装のため現状プレースホルダ：「人気」は実態がタイトル順なのに「人気」と表示され**来場者に誤解**を与え、「最近プレイ」は0行で**セクション自動非表示**になりスタッフが「追加したのに出ない」と混乱しうる。誤って選べないよう選択肢から除外。
  - canonical mapping（`GetCanonicalFromSource` / source 文字列変換）は**残置**したので、既存セクション（もし `popular`/`recently_played` で保存済みでも）の読み込みは壊れない（選択肢に出ないだけ）。
  - **#297 PR2 で実ランキングを実装したら `AllSources` に `("人気ランキング", 1)` と `("最近プレイ", 3)` を戻すこと**（コード内コメント＋#297 に再追加メモを記載）。
- 検証: VS MSBuild で Release ビルド成功（compile 確認）。**※実機で Manager のストアセクション編集→ソースに「人気」「最近プレイ」が出ないこと・他ソースは従来どおり選べることは目視**。
