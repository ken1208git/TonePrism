...
- レビュー対応（堅牢化）:
  - **タイトル無し＋最小化の窓を `focus`(pid) 経路で取りこぼさない**よう、`FindTopLevelWindow` 用に `IsMeaningfulVisibleWindow(acceptMinimized:true)` を追加。最小化窓は `GetWindowRect` がオフスクリーン小サイズを返し size 判定を通らないため、タイトル無し排他FS窓が最小化した瞬間に発見できず復帰しなかった。probe 経路は従来どおり（最小化を「可視」と誤検知して PLAYING/anomaly を狂わせない）。WOLF 等タイトル有りは元から救えていたが本対応で取りこぼしクラスも復帰可。
