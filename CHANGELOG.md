...
- **UI 文言の平易化（ユーザー指摘）**: エンドユーザー向け画面の `games`/`guide` というフォルダ名併記を**中身で説明**する表現に置換＝「ゲームファイル (games/guide)」→「**ゲームファイルや初回説明の画像**」（確認ダイアログ `RestoreConfirmForm`・復元レポート `RestoreReportForm`・今すぐバックアップ通知）。docs `usage/manager.md` の呼称「案内画像」→「初回説明の画像」に統一。**ただしレポートの手動コピー手順**（「当時の games フォルダを games/ 配下にコピー」等）は、ユーザーが実際にエクスプローラーで `games` フォルダを操作するためフォルダ名を残置（運用に必要）。コード内コメント（開発者向け）も技術名 games/guide のまま。
