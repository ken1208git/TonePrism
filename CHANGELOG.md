...
- **GDScript 内 `prism.db` literal sweep**: `scripts/path_manager.gd` / `scripts/logger.gd` / `scripts/session_heartbeat.gd` の DB filename literal を `prism.db` → `toneprism.db`
...
- **GitHub repo URL rename**: `GitHubReleaseChecker.cs` `Owner` / `Repo` const + `Release.ps1` `$GitHubRepoSlug` + CHANGELOG 末尾 link def 全件 `ken1208git/GCTonePrism` → `ken1208git/TonePrism`。GitHub 自動 redirect で過去 release tag link も継続動作。**round 1 review fix (H-2 + L-4)**: CHANGELOG 末尾 link def block は「過去 history retain」原則の **例外として forward-only update 許容**、過去 release tag URL も新 repo URL 表記で SoT 化 (= GitHub redirect 経由で動作、新 URL が canonical な方が将来 reader にとって明瞭)。SPEC §10.x v1.10.30 row + 本 entry で link def の inline body (= retain) と URL block (= forward update) を切り分け明示
