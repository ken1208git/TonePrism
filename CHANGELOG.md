...
- **per-version 純粋ロード（games フォールバックなし）**: `LoadGameDataForVersion` は version の description/arguments をそのまま読む。当初は version 値が空のとき `games` 値にフォールバックする自己修復を入れたが、「意図的に空の版」と「desync」を区別できず、その版を選んだだけで active(games) 値が旧版へ silent 転写される footgun（Codex P2 / review #1）だったため撤回。新規ゲームは AddGameForm が初期版に `games` と同じ値を持たせるため desync は発生せず、純粋 per-version で安全。本番はまっさら新規で既存 desync データも無い。
