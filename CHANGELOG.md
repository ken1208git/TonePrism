...
- 検証: Manager build 緑 / **テスト 152 件合格**（#250 PR3a 新規 12 + レビュー対応 1 [GC 破損行でも blob 保護]、いずれも fail-first: RoundTrip / 余剰削除 / 不変skip無再コピー / 削除再生成 / blob欠落は IsPartial かつ live保持 / blob欠落で削除しない / パストラバーサル拒否 / 空manifestガード+allowEmpty / case差を誤削除しない / guide空で games保持 / キャンセル / manifest不在 / GC破損行保護）。既存 snapshot テストも緑（SoT リファクタ無回帰）。
