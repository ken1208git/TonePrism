...
- **画面下部にクリック可能な操作ボタン（戻る / スキップ / 進む）** を追加（マウス/タッチ用、キーボード ←/→/Esc と併用）。先頭スライドでは「戻る」を無効化、最終スライドでは「進む」を「ストアへ」表記に。ボタンは `focus_mode=NONE`（Enter の二重発火防止、キー操作は `_input` 側）。マウス移動でカーソル表示・キー/パッドで非表示（store_browse と同じ分離）。
