...
- 検証: Manager build 緑 / **テスト 188 件合格**（撤去回帰テスト `V22ToV23_DropsEventTables` ＋ `UpdateGameId` 回帰テストを追加）。**※新規 DB および v22 実 DB での v23 到達（3 テーブル不在・integrity ok）の sqlite3.exe 実機検証は本番DB作成と pre-release で実施**。
