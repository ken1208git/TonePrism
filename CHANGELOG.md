...
- **(#250 PR3b) アセット復元エンジンを復元 UI へ配線（DB＋ゲームファイルの一貫時点復元）**: PR3a で入れた `AssetRestoreService`（reconcile-in-place）はエンジンのみで UI 未配線だった（「取れるが戻せない」が半分残っていた）。本 PR で復元フローに配線し、**控えのある世代を復元すると DB だけでなく games/guide もその時点へ一緒に戻す**（控えの無い世代は DBのみ）。これで #250 の核心「DB とアセットを一貫した時点に復元」が実機操作で完結する。<br>※ round2 のユーザー判断で「DBのみ/DB+ゲーム」のチェックボックス選択は廃止し、**復元＝一貫時点復元に一本化**＋**復元前の自動退避で可逆化**する形に変更（下記「round2 ユーザー判断」参照）。
  ...
  - **確認ダイアログ（`RestoreConfirmForm`）の控え表示・警告**（round2 で確定形に変更）: 控えのある世代では「ゲームファイル (games/guide) もこの時点に戻す／この時点より後に追加・変更したファイルは削除される／削除前に自動退避するのでやり直せる」を**赤字で明示**＋対の控えの日時/取得PC/ファイル数を表示。控えの無い世代は「DBのみ復元（ゲームファイルは現在のまま）」。`public bool RestoreAssets`（= 控えあり）で呼出側へ伝える（チェックボックスは round2 で廃止）。
...
- **レビュー round2 ユーザー判断（同 PR・version 据え置き）**: review #2（既定 ON の破壊性）/ #3（アセット復元に safety_*.db 相当の退避が無い非対称）について、ユーザー判断で次の方針に確定。**(a) 「DBのみ / DB+ゲーム」チェックボックスを廃止し、復元＝一貫時点復元に一本化**＝控えのある世代は常に games/guide も戻す。代わりに確認ダイアログで「この時点より後に追加・変更したゲームファイルは削除される」旨を赤字で強く明示する（`RestoreConfirmForm` から `chkRestoreAssets` を撤去、`RestoreAssets => _pairedSnapshot != null`）。**(b) 破壊的 reconcile の前に現在の状態を自動退避して可逆化**＝`btnRestore_Click` ワーカーで reconcile に入る前に `BackupService.RunManualBackup` で現 DB+assets を退避（DB の safety_*.db と対称な安全網。手動配置の pool 未登録ファイルも含めて控える）。退避は破壊前なのでキャンセル可。**退避が不完全（SMB 失敗 / 部分取得 / アセット控え無効）なら破壊的 reconcile を行わず DBのみ復元へ degrade**（games 無変更＝安全）し、その旨をユーザーに明示。成功時は「やり直すなら直前の自動退避バックアップを復元」と案内。トレードオフ: DB+ゲーム復元時は退避（全 games 走査）＋reconcile（再走査）で SMB I/O が増える（ユーザー了承済、稀な操作）。
- **レビュー round3 対応（同 PR・version 据え置き）**: (#1 Medium) **退避フェーズのキャンセルが degrade（DBのみ復元）に化けて DB が置換される穴を是正**。`RunManualBackup` はアセット取得フェーズ（10-99%・最長の SMB 走査＝最も「中止」を押しやすい）のキャンセルを内部で握って `Success(AssetSnapshot=Skipped)` を返す（throw しない）。これを genuine な退避失敗と同一視して degrade すると、ユーザーが中止したのに `Restore()` へ進んで DB が置換され、「退避は破壊前なのでキャンセル＝不変」の設計意図に反していた（データ損失ではない＝safety_*.db で可逆・games 無変更だが、コア動線の非一貫）。退避直後に `token.ThrowIfCancellationRequested()` を入れ、**ユーザー意思のキャンセルは全体中止（DB 置換しない）**に倒した。genuine な退避失敗（SMB I/O 等・非キャンセル）だけが degrade に進む。(#4 Low) 退避失敗 degrade 時の成功メッセージが「整合性に問題なし」と「DBのみ復元（退避失敗）」を同居させる矛盾を、degrade では「データベースのみ復元しました」に切替えて解消。(#3 non-issue) host tie-break の `entry.PcName`（ファイル名 regex 抽出）と manifest `Host`（META フィールド）は、両者とも `SanitizeHostForFileName`（`_`→`-` 置換で host に `_` を残さない）由来のため食い違わないことをコードで確認（既存の不変条件を再確認、コード変更なし）。(#2 既知の制約) 退避バックアップは undo 可能にするため `manual` trigger で作る＝manual は retention 対象外なので、DB+ゲーム復元を繰り返すと退避 `.db` が履歴に蓄積する（CAS pool の blob 増分は内容アドレスで小さい）。専用 trigger + retention は別 issue で検討。
- 検証: Manager build 緑 / **テスト 173 件合格**（PR3b 新規 17 件＝ペアリング 10: T以下で最大 / replace-in-session(db>manifest) / host一致優先 / **host 一致でも時間的に近い別 host を優先（degraded）** / host無し→全体最新 / T以前なし→null / 空 / auto+manual 横断 / 同秒tie→名前降順 / 不正ヘッダ skip ＋ eligibility 7: auto/manual のみ true・safety/unknown/空/null は false・大文字小文字非依存、いずれも fail-first）。**※実機 UI（控えあり世代の警告ダイアログ・復元前自動退避→reconcile→レポート・DB+assets round-trip・退避失敗で DBのみ degrade・本番 SMB 体感・safety 復元で games が消えないこと）は pre-release で目視（F1〜F7）＝本 PR は #250 の実機 UI 初確認対象。**
