...
- 検証: Manager build 緑 / **テスト 100 件合格**（`AssetSnapshotServiceTests` 13: 初回プール格納 / **同一中身は別パスでも 1 個** / **名前同じ中身違いは別保存** / 不変は新規 0 / 変更で新 blob / retention+GC で未参照 pool 削除 / **grace で直近未参照 blob は残る** / **blob mtime は配置時刻** / **履歴ありで sources 消失→Skipped** / manual 温存 / 無効 / 空(未登録)→Success / 失敗 never-throw）。**※実機 SMB round-trip（再取得で控えがほぼ増えない / サーバー容量計算で実サイズが出る / 長パス / 多ホスト並行 GC 安全性）は別途必要**。
