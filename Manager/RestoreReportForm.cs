...
namespace TonePrism.Manager
{
    ...
    public class RestoreReportForm : Form
    {
        ...
        private string BuildReportText()
        {
            ...
            {
                // (#317) アセットを一緒に復元しなかった復元直後。_assetResult == null はこの 2 ケースを両方含むため
                // 理由は断定しない: (a) 旧/unknown 形式の DBのみバックアップ（そもそもアセット控えが無い）、
                // (b) 新形式バックアップだが復元前のゲームファイル退避に失敗し DBのみ復元へ degrade
                // （BackupSectionPanel の assetRetreatFailed）。(b) の具体的理由は呼び出し側が別ダイアログ
                // （retreatFailedNote）で告知するので、ここで「古い形式だった」と断定すると矛盾する。事実だけ述べる。
                sb.AppendLine("【まず確認】今回の復元では、ゲームファイル本体や初回説明の画像は一緒には復元されていません。");
                sb.AppendLine("そのため、いま復元した DB と、ディスク上のゲームファイルの「時点」がズレていると、");
                sb.AppendLine("以下のような食い違いが起きることがあります。");
            }
            else
            {
                // (#317) standalone 整合性チェック。#250 以降、通常のバックアップ／復元は DB に加えて
                // ゲームファイル本体・初回説明の画像も対象（旧「DB だけ」表記は誤りなので訂正）。ただし手動での
                // ファイル変更や、旧形式バックアップの復元等では DB とディスクの「時点」がズレることはある。
                sb.AppendLine("【まず確認】通常のバックアップ／復元は、toneprism.db（データベース）に加えて、");
                sb.AppendLine("ゲームファイル本体や初回説明の画像も対象です。ただし、手動でファイルを変更した場合や、");
                sb.AppendLine("古い形式のバックアップを復元した場合などは、現在の DB と、ディスク上のゲームファイルの");
                sb.AppendLine("「時点」がズレて、以下のような食い違いが起きることがあります。");
            }
            sb.AppendLine();
