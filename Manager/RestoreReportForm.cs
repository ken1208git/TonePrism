...
namespace TonePrism.Manager
{
    ...
    public class RestoreReportForm : Form
    {
        ...
        private string BuildReportText()
        {
            ...
            {
                // (#317) DBのみ復元（旧/unknown 形式でアセット控えが無い世代）。この復元は確かに DB のみ。
                sb.AppendLine("【まず確認】今回の復元は古い形式のバックアップ（DB のみ）だったため、");
                sb.AppendLine("ゲームファイルや初回説明の画像は含まれていません。そのため、いま復元した DB と、");
