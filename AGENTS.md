...
- **Bundle entry 追加時に CHANGELOG 末尾の参照リンク定義も同時追加** (Markdown 形式: `[Bundle vX.Y.Z]: https://github.com/ken1208git/GCTonePrism/releases/tag/vX.Y.Z`)。これは `### [Bundle vX.Y.Z]` 見出しを GitHub Releases ページへの正しいリンクに resolve するため。Release.ps1 が release 実行時にこの定義の有無を verify して、無ければ Fail で停止する。手追加忘れは **Release.bat 実行直後の Preflight assert (Phase 0.5 `Assert-ChangelogLinkDefs`、Godot export / msbuild より前)** で物理的に防ぐ仕組み (= 数分の build を捨てる前に fail-fast、`fix/changelog-link-sync` PR で導入、 SPEC §3.7.7 「Bundle release が SoT」規約整合)。
