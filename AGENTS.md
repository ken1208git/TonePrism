...
## Commit 運用
- AI は作業単位ごとに feature/fix/chore ブランチへ自律的にコミットしてよい（事前確認不要）。ただし**同じ箇所を編集している途中ではコミットしない**。会話が複数ラリーに渡っても、その箇所の作業が一区切りつき動作が一貫した状態になるまで待つ（中途半端な WIP を刻まない）。
- コミット単位は**ビルドが通り 1 つの論理変更で説明できる最小単位**。大きいタスクは論理ステップ（例: service 抽出 → 配線 → テスト）が完了するたびに刻む。ラリー単位・微調整 (typo/整形) 単位では刻まない。merge commit 方式で個別コミットが main 履歴に残るため、各コミットは単体で意味が通る状態にする。
- ただし push / PR 作成 / main へのマージ / リリースは明示指示を待つ（不可逆・外部操作）。main へ直接コミットしない。
- 版数 bump の提案は 1 PR 1 回（自律コミットごとには行わない）。コミットメッセージは `type(scope): 要約 (#issue)` 形式。
