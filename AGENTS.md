...
- **Release.ps1 内の関数命名**: **主目的が検証 (verification)** で、検証失敗時に `Fail` (exit 1) に到達する関数は `Assert-*` prefix を使う (PR #140 round 6 で `Test-Preflight → Assert-Preflight`、round 9 で `Test-ExpectedFiles → Assert-ExpectedFiles`、PR #156 で `Read-LauncherVersion` / `Read-ManagerVersion` / `Read-ComponentVersions` / `Read-GodotMinorFromProject` → `Assert-*` で sweep 完結)。
