...
ゲームが作られた年。**分からない場合は「不明」にチェック**を入れると、年の入力欄が無効になり「不明（空欄）」で登録されます（制作年が分からない古いゲームでもそのまま登録できます）。
...
- **追加**: 「追加」を押すと入力画面（製作者情報追加）が開きます。次を入れて「OK」を押すと、一覧に加わります。
    - **姓・名**: どちらか一方は必須です（姓だけ・名だけ・両方、いずれでも可）。
    - **期生**: その人が何期生かを数字（1 以上）で入れます（例: `50` → 50期生）。**教員の場合は「教員」にチェック**、**分からない場合は「不明」にチェック**を入れます（チェックすると数字の入力欄は無効になります）。「教員」と「不明」は同時には選べません。
...
いらなくなった**特定のバージョンだけ**を消せます。編集画面の「ランチャーで表示するバージョン」の欄の右にある **「このバージョンを削除」** ボタンを使います。

1. 「ランチャーで表示するバージョン」の欄で、**消したい版を選びます**。
2. **「このバージョンを削除」** を押します。
3. 確認画面で内容（ゲーム名・バージョン・消えるフォルダ）をたしかめて **「はい」** を押すと削除されます。

!!! warning "この削除は取り消せません"
    確認画面で「はい」を押すと、**その版のデータベース情報と、その版のフォルダ（`games/<ゲームID>/v<バージョン>/`）が一緒に消えます**。取り消しはできません。

- **最後の 1 版は消せません。** ゲームには最低 1 つの版が必要です。全部消したいときは、下の「[ゲームを削除する](#ゲームを削除する)」でゲームごと消してください。
- **いま表示中の版を消すと、残りの最新の版が自動で「表示する版」になります**（表示する版が無くなることはありません）。
- ほかの版や、ほかのゲームには影響しません。

!!! note "Launcher が動いているとき"
    その版がちょうど Launcher で**プレイ中**だと、フォルダが使用中で削除できないことがあります。
    その場合は「削除できませんでした」と表示されるので、**Launcher を閉じてからもう一度**「このバージョンを削除」を押してください
    （このときデータベースは変更されていないので、安全にやり直せます）。
