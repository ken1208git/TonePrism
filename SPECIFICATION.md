  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    4. ゲーム動作確認 — 3 段階の確認方法を選んで実行する（旧「ゲーム一覧状態確認」と「ゲーム起動テスト」を統合）。詳細ペインはまず確認方法の選択画面を出し、選ぶと中身全体がそのモード画面に切り替わる。モード画面で ←/B/Esc を押すと確認方法の選択へ戻る（詳細ペインからは出ない＝サブ階層ナビ）:
        ...
        - **③ 試遊確認**（手動終了）— 起動テスト②と同様のチェックリスト（各ゲームにチェックボックス+「すべてチェック/外す」+「試遊開始」+「中止」）。チェックしたゲームを **1本ずつ自動で起動** して試遊し、遊び終わったら **ゲームを手動で終了** するか **HOMEボタン**（中断メニューと同じトリガ。押すと強制終了）で復帰。戻ると「『〇〇』は正しく遊べましたか？」と尋ね、スタッフが **〇 遊べた / × 問題あり** を記録すると **自動で次のチェック済みゲームへ進む**。結果は各行に色付き表示。exe不在/起動失敗は自動で×。「ファイルはあるが実際に遊べるか」を人が最終確認する用途
