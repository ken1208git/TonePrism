...
- **検出 trigger** (= dialog 表示の起点):
  ...
  - **check 位置の判断軸** (round 5 Medium-1 で明示化): 「ShowDialog 前」(= user 親切性、編集 modal を開く前に他 PC 警告を見せて編集作業を無駄にしない) と「最終 DB write 直前」(= race fence、modal 開いてる間に他 PC が起動する race を catch) の trade-off。**学校 LAN 運用での実発生確率と user 親切性を優先して原則 ShowDialog 前 check に統一** (= user が編集途中で警告に切られる UX 退行を回避)。例外として最 destructive な `btnResetDatabase` (= DB 全削除 + 再構築) のみ ConfirmForm OK 後にも **2 段階 check** で race fence を厳密化。modal 内の race window は受容仕様 (= 他 PC が user の編集 modal 中に新規起動した case は次の操作で検出されるまで warning しない、heartbeat UPSERT で永久不可視化はしない)。
