...
- **優先度**: 中
- **状態**: **実装済** (#30 / #214)。ゲーム実行中に HOME キー / コントローラ Guide で開く中断メニュー (続ける / ホームに戻る / 退出) を、Launcher 内の単一 CanvasLayer + メイン窓 per-pixel 透明化で**背後のライブゲームに重ねて**表示する。HOME/Guide のグローバル検知と前面化は常駐 `LauncherAgent` (§2.4) が担う。
- **詳細**:
...
└── Companions/                               # 補助 exe 集約
    ...
    └── LauncherAgent/                        # Launcher 補助の常駐エージェント (probe #101/#216 + sensor HOME/Guide #30 + focus/topmost)
...
Launcher 補助の Win32 機能を 1 プロセスに統合した常駐エージェント。当初は別々の Companion として計画していた WindowProbe（単発クエリ）と PauseOverlay（透過 WPF 別窓の中断メニュー）を統合・廃止し、本エージェント 1 本に集約した（理由は後述「分割/統合の判断軸」、中断メニュー本体は #214 で Launcher(Godot) 内の単一ウィンドウ実装へ移行）。担う機能は次の 3 つ:
...
- **focus / topmost**: 中断オーバーレイ表示時に Launcher メイン窓を前面化・最前面化する。`AttachThreadInput` + `SetForegroundWindow` で入力フォーカスを奪い、`SetWindowPos`(HWND_TOPMOST) で z-order を即座に最前面へ。Godot のフルスクリーン窓は `always_on_top` を無視するため、z-order 制御を本エージェントが代行する（SetForegroundWindow 単独は foreground-lock で遅延/失敗しやすい）。
...
- WindowProbe（単発 OS.execute）は常駐 `LauncherAgent` に統合・廃止（#101 / #216 実装時）。PauseOverlay（透過 WPF 別窓）は実装されず、中断メニューは Launcher(Godot) 内の単一 CanvasLayer + メイン窓透明化方式に変更（#30 / #214）。統合エージェントは当初 `LauncherCompanion` 名だったが、Companions 配下の命名一貫性（Updater と同じ機能/役割ベース、カテゴリ名 "Companion" のスタッター回避）のため `LauncherAgent` にリネーム。
...
- **表示方法**:
  - Launcher メイン窓を per-pixel 透明化し、背後の**ライブゲームを透過表示**（旧案の「背景暗転」ではなく、ライトテーマの白 veil でうっすら覆う＝ゲーム起動〜終了はライトの世界観に統一）
  - メイン窓を topmost 化してゲームの上に重ね、同 viewport 内の**単一 CanvasLayer** としてメニューを描画（別 OS 窓ではないため既存のフォーカス/入力がそのまま効く）
- **レイアウト**（ライトテーマ）:
