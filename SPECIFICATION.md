...
- [ ] **`Manager/Services/UpdateDownloader.cs` の `ValidateStaging`**: 上記 `Release.ps1 Assert-ExpectedFiles` と **必ず同期**。release 側で同梱対象に追加 / 削除があれば本クラスの `rootExpected` / `filesExpected` 配列も同タイミングで反映する。drift すると Manager UI の apply フローが「存在しないファイルを要求して永久 abort」または「存在するのに missing 認識せず通る」silent bug を生む (PR #161 round 1 C1 で実発生)。長期的には manifest 同梱 (`expected_files.json`) で SoT 統一する方針
