...
- **アセットスナップショット（games/ + guide/ のハードリンク世代バックアップ、#250 PR1、Manager v0.21.0）**:
  - **動機**: DB バックアップは `toneprism.db` 単体のみで、実行ファイル/サムネ/背景/各 `v{}/` を含む `games/`（~6GB）と `guide/`（intro 画像）はバックアップされない。ディスク障害/誤削除で復旧不能になるのを防ぐ。
  - **方式**: rsync `--link-dest` 相当の**ハードリンク世代スナップショット**。`AssetSnapshotService.CreateSnapshot` を DB バックアップ成功直後に**同一 timestamp/triggerType** で呼ぶ（`BackupService.RunBackupCore`、best-effort = 失敗/キャンセルしても DB バックアップの成否・`last_backup_at` を壊さない）。
  - **レイアウト**: `<backup_dest>/asset_snapshots/<auto|manual>/<yyyyMMdd_HHmmss>[_host]/{games,guide}/...`。世代名 timestamp は同時刻の `.db` バックアップと対応（PR3 の「DB 時点に最も近いスナップショット」探索の布石）。構築は `.tmp_*` → 完了後 `Directory.Move` で atomic 昇格（部分世代を base/履歴に上げない）。
  - **link-dest 判定**: 直近の完成世代を base に、各ファイルが base に「相対パス + サイズ + mtime（UTC, 2 秒許容）一致」で存在すればハードリンク、無ければ実コピー。内容ハッシュは取らない（games/ はほぼ追記専用＝実用十分、誤判定は理論上のみ）。symlink/junction は辿らない、長パスは `\\?\`、除外リストは適用しない（丸ごと）。
  - **可否プローブ**: backup_dest ボリュームでハードリンクが張れるか実テスト（NTFS で true、FAT/exFAT 等は全実コピー fallback + 警告）。重複排除は「新↔前スナップショット」間（両方 dest 上）なので判定軸は dest が NTFS かのみ（src の場所は無関係）。
  - **容量**: 不変ファイルはハードリンク（実体非複製）のため実ディスク消費 ≈ ベースライン 1 本 + 版追加差分で**世代数に比例しない**。各世代は独立・完全（ハードリンク = 同一実体への対等エントリ。retention で古い世代を消しても新世代は無傷、チェーン依存ゼロ）。削除/縮小した内容は参照する旧世代が retention 内にいる間は保持され、全世代がウィンドウから抜けて解放（point-in-time 保持）。
  - **retention**: **auto のみ** `asset_snapshot_retention_count`（既定 30）で名前降順 Skip 削除（ディレクトリエントリのみ＝実体は共有新世代が残れば生存）。**manual は温存**（#235 同様）。`backup_retention_count` とは独立。
  - **復元 / guide 整合性チェック拡張は後続 PR**（PR3 = 復元 2 モード「DB のみ」/「DB+assets 一式」、PR2 = `RestoreReconciliationService` を `intro_slides.image_path`/`guide/` にも拡張）。本 PR1 は取得 + retention + 設定 + UI 可視化のみ。
...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-02 | 1.10.55 | **(#250 PR1、Manager v0.21.0)** バックアップを `toneprism.db` 単体 → **DB + アセット (`games/` + `guide/`) のハードリンク世代スナップショット**に拡張する #250 の **PR1 (取得エンジン)**。§機能12 にアセットスナップショットのサブ項目を追記、設定値リストに `asset_snapshot_enabled`/`asset_snapshot_retention_count` の 2 キーを追加。**方式**: rsync `--link-dest` 相当。DB バックアップ成功直後に同一 timestamp/trigger で `AssetSnapshotService.CreateSnapshot` を best-effort 呼び出し (失敗/キャンセルしても DB バックアップ・`last_backup_at` を壊さない)。`<backup_dest>/asset_snapshots/<auto|manual>/<ts>[_host]/{games,guide}/`、`.tmp_*`→`Directory.Move` で atomic。不変ファイルは base へハードリンク (size+mtime UTC 2秒許容)、差分のみ実コピー → 容量は世代数に比例しない。可否プローブで非対応 FS は全実コピー fallback。retention は auto のみ (`asset_snapshot_retention_count` 既定30、manual 温存)。新規 `HardLinkSupport` (kernel32 CreateHardLink P/Invoke + プローブ)。schema 版据置 v22 (settings は K/V data)。**復元 2 モード=PR3 / `RestoreReconciliationService` の guide/ 拡張=PR2** (本 PR は取得+retention+設定+UI のみ)。検証: Manager build 緑 + テスト 101 件合格 (新規 HardLink 4 + Snapshot 10)。実機 NTFS round-trip は別途。 | Kenshiro Kuroga \& Claude |
