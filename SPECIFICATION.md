...
- **stale timeout**: 60 秒 (= heartbeat × 6、#269 で 30→60 に緩和)。**理由**: stale 判定は「読み手 PC の now − 検出 timestamp」で PC 間の clock drift を被るため、想定 skew を吸収するマージンを取る。広げる方向は安全側 (= 死んだ session が長めに active 表示 = 過剰警告) でデータ事故方向には倒れない。⚠️ `manager_sessions` (Manager-Manager 検出) は DB ベースで per-row mtime が無く同 logic を適用できないため 30 秒のまま (同じ clock drift を持つが mitigation は閾値のみ。閾値統一は follow-up 候補)
