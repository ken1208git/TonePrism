...
- **優先度**: 中
- **状態**: **実装済** (#30 / #214)。ゲーム実行中に HOME キー / コントローラ Guide で開く中断メニュー (続ける / 別のゲームをあそぶ / 退出する) を、**透明・最前面の別 OS Window**で走行中ゲームの上に重ねて表示する (背面に不透明・全画面の playing 背景を据えてウィンドウゲームの隙間を埋める 2 枚構成)。HOME/Guide のグローバル検知と前面化は常駐 `LauncherAgent` (§2.4) が担う。
- **詳細**:
...
- **説明**: Alt+F4 やウィンドウの閉じるボタンによる終了を封印し、サービスモード（機能23）からのみランチャーを終了可能にする機能
- **優先度**: 高（必須）
- **詳細**:
  - Alt+F4 キーフック（終了要求を無視）
  - ウィンドウの×ボタンによる終了を無効化
  - ランチャー終了はサービスモード内の「ランチャー終了」ボタンからのみ可能
  - **例外**: サービスモード表示中は Alt+F4 での終了を許可（Ctrl+Alt+F12 を知るスタッフしか開けない隠し画面のため誤終了の懸念がなく、項目14 まで辿らず素早く終了できる利便性を優先）。ただしゲーム実行中はゲームを孤立させないため終了しない（項目14 の終了ガードと同じ）
...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    3. 画面表示テスト — 単色全画面（黒・白・赤・緑・青・グレー50%）、グリッド（格子線+中央十字+四隅対角線にセーフエリア枠90%/80%を統合し解像度/スケーリング/端の見切れを一括確認）、カラーバー（放送規格 SMPTE RP 219 / ARIB の HD カラーバー: 上段75%カラーバー+両端40%グレー袖 / リバース段 / 0-100%ランプ段 / PLUGE段で色再現・色順・黒レベルを確認）、グラデーション（黒→白の階調・バンディング確認）。モニター相性・拡大率・解像度ズレの事前発見用。実装は全画面 `Control` の `_draw()` で描画（画像アセット不要・解像度追従）
    4. ゲーム動作確認 — 3 段階の確認方法を選んで実行する（旧「ゲーム一覧状態確認」と「ゲーム起動テスト」を統合）:
        - **① ファイル存在チェック**（起動しない・全件一括）— 各ゲームの exe 存在チェック、パス切れ/ファイル欠落の事前発見。LAN 共有未接続・フォルダ移動・パス誤りを即検出
        - **② 起動テスト**（自動）— ゲームを自動起動→ウィンドウ生成確認→自動終了し、起動可否のみ判定。成功条件はプロセス起動→ウィンドウ生成→規定の生存時間超え（詳細は実装時に決定）。結果一覧を表示（OK/NG）。DLL 不足・起動時クラッシュ等の検出用
        - **③ 試遊確認**（手動終了）— 選んだ 1 本を実際に起動して試遊。終了はユーザーが手動で行う。「ファイルはあるが実際に遊べるか」の最終確認用
    5. ネットワーク接続テスト — 段階的に接続状況を表示。IP取得→ゲートウェイ→DNSまでが共通の幹で、そこからインターネット接続（外部接続確認・応答時間表示）とMonitor接続（先生PC接続確認・応答時間表示）に枝分かれする構造。どの段階で失敗しているか一目でわかる
    6. データベース整合性チェック — DBファイルの存在確認、テーブル存在確認、レコード数表示、読み書きテスト。「データが反映されない」問題の切り分け

    **ログ・情報:**
    7. 簡易ログ確認 — 直近のエラーログ表示（メモリバッファから現セッションのログを取得）
    8. システム情報表示 — PC名、OS、解像度、Godotバージョン、Launcherバージョン等

    **設定・操作:**
    9. デバッグオーバーレイ切り替え — ON/OFFトグル。ONにするとサービスモードを閉じても画面隅にFPS・メモリ使用量・PC名・現在のシーン状態・DB/Monitor接続状態等をリアルタイム表示し続ける（マイクラのF3的な常時表示）。メモリのみ保持（再起動でOFF）
    10. フルスクリーン切り替え — フルスクリーン↔ウィンドウの切り替え。モニター違いでの表示崩れ対応用
    11. ランチャー表示モニタ選択 — 複数モニタ環境で、ランチャーを表示するモニタをその場で切り替え（`window_set_current_screen` で即移動）。**メモリのみ保持（再起動でプライマリに戻る）**。本番=単一モニタでは無関係。恒久的に別モニタで運用する場合は Windows のメインディスプレイ設定を使う。関連: 中断メニュー（機能4 / #30）はゲーム窓をランチャーと同じモニタへ寄せるため、ここで選んだモニタにゲーム窓も揃う
    12. ランチャーの再読み込み — DB再読み込み。再起動より軽い
    13. ランチャーの再起動 — OSプロセスとして再起動
    14. ランチャー終了 — 確認ダイアログ付きでランチャーを終了。Alt+F4/×ボタン封印のため唯一の終了手段

    > ※ 旧「エラー内容表示+マニュアル」項目は廃止。エラーコード別の対処法は **エラー発生時のダイアログに併記**する方式に変更した（ゲーセン筐体のエラー表示と同様。スタッフが別途マニュアルを引かなくてよい）。実装は `error_dialog.gd` の `_REMEDY`（コード→対処法）。
...
- **詳細**:
  ...
  - 「アップデート」タブの表示内容：
    - 現在のバージョン情報一覧（Launcher / Manager / 各 Companion [Updater / LauncherAgent 等、§2.4] / DB schema）
...
主要 3 アプリ（Launcher / Manager / Monitor）以外の **補助 exe** をまとめる集約カテゴリ。Updater のような独立稼働ツール、LauncherAgent のような Launcher 補助の常駐エージェント等、すべてここに含める。
...
- フォルダ名 = 短縮（`Companions/Updater/` / `Companions/LauncherAgent/` 等）
...
└── Companions/                               # 補助 exe 集約
    ...
    └── LauncherAgent/                        # Launcher 補助の常駐エージェント (probe #101/#216 + sensor HOME/Guide #30 + focus)
        └── TonePrism_LauncherAgent.csproj
```

将来、複数 Companion が Win32 ヘルパーを共有する必要が出たら `Companions/Common/`（csproj 名は disambiguation のため `TonePrism_CompanionsCommon.csproj`、AGENTS.md「Naming Conventions」参照）を追加検討する。
...
| LauncherAgent | Launcher | Launcher 起動時に 1 個だけ常駐起動 (ゲーム起動で watch 開始) | 下記 / #101 / #216 / #30 |

#### Launcher との通信規約

- **呼び出し**: Launcher 起動時に Godot 側から `OS.create_process()` で 1 個だけ常駐起動する。exe path は実行時に `Companions/<名前>/TonePrism_<名前>.exe`（プロジェクトルート相対）で解決する。
- **IPC**: 双方向 localhost UDP。Godot 4 は子プロセスの stdout を逐次読めないため stdout/JSON ではなく UDP を採用。起動時ハンドシェイクで cmd/event ポートを交換し、以後 1 メッセージ = 1 行 JSON で双方向通信する。
- **ログ転送**: Companion の WARN/ERROR/主要イベントは log イベントで Launcher に届き、Launcher の godot.log テール経由で `[LauncherAgent]` prefix 付きで launcher ログに記録される（Manager のログ閲覧「Launcher タブ」に出る）。
...
##### LauncherAgent（常駐エージェント、#101 / #216 / #30 で実装済）

Launcher 補助の Win32 機能を 1 プロセスに統合した常駐エージェント。当初は別々の Companion として計画していた WindowProbe（単発クエリ）と PauseOverlay（透過 WPF 別窓の中断メニュー）を統合・廃止し、本エージェント 1 本に集約した（理由は後述「分割/統合の判断軸」、中断メニュー本体は #214 で Launcher(Godot) 側の透明オーバーレイ窓 + 不透明 playing 背景の 2 枚構成へ移行）。担う機能は次の 3 つ:

- **window probe (#101 / #216)**: 指定ゲーム PID の**プロセスツリー (PID + 全子孫)** の可視/前面ウィンドウ状態を ~150ms 間隔で監視する。状態は `visible_foreground` / `visible_background` / `not_visible` / `not_found` の 4 値。
  - 「ゲーム起動中 → プレイ中」遷移の確定 (#101)、プレイ中のランチャー前面化異常検知（要スタッフ対応、#216）を駆動。
  - プロセスツリーを辿る理由: Launcher は `cmd.exe /C "cd /d <dir> && game.exe"` でゲームを起動するため、握る PID は cmd.exe で、ゲーム窓は子/孫の game.exe に属する。PID 単体でなく子孫まで辿って可視ウィンドウを探す。
  - 使用 API: `EnumWindows` / `GetWindowThreadProcessId` / `IsWindowVisible` / `GetForegroundWindow` / `CreateToolhelp32Snapshot`。
- **input sensor (HOME / Guide、#30)**: ゲームが前面でもグローバルに中断トリガを検知する。低レベルキーボードフック (`SetWindowsHookEx` WH_KEYBOARD_LL) で HOME キー、XInput (`xinput1_4.dll` ordinal #100 = `XInputGetStateEx`) で Guide ボタンを検知 → Launcher へ open/close トリガを通知。
- **focus**: 中断オーバーレイ窓 / ゲーム窓を強制前面化する。`AttachThreadInput` + `SetForegroundWindow` で入力フォーカスを奪う（foreground-lock 回避）。`focus <pid>` はゲーム窓ツリーを前面化（メニューからの再開時）、`focus_hwnd <hwnd>` は中断オーバーレイ窓だけを前面化（メイン窓を巻き込まない、別 OS 窓 2 枚構成 #214）。あわせて `watch <pid> <x> <y> <w> <h>` 指定時はゲーム窓をランチャーのモニタ中央へ寄せる（`PlaceWindowCentered`、マルチモニタで 2 枚構成を同一モニタに揃える）。

##### 分割/統合の判断軸

Companion を別 exe にするか 1 つに統合するかは「**責務の数**」ではなく「**ライフサイクルと結合度**」で決める:

- **別 exe にする** = 主要アプリと**異なるライフサイクル**を持つ、またはハード制約で in-process 化できないもの。例: Updater は「実行中の exe は自分自身を置換できない」「親 (Manager) が落ちている間に動く」という Windows 制約のため必然的に別プロセス。
- **1 つに統合する** = 主要アプリと**同じライフサイクルで動き続け、IPC やイベントループを共有**する複数機能。例: LauncherAgent の probe/sensor/focus は Launcher が動く間ずっと働き、単一 UDP チャネル + 単一メッセージポンプ（キーボードフック配送がポーリングも兼ねる）を共有し、相互に絡む（#216 異常検知 = probe 状態 + フォーカス情報）。3 プロセスに割ると handshake / 親監視 / ポンプが重複するだけで分離の利点がない。

この軸に基づき、旧計画（WindowProbe + PauseOverlay を別 exe）は LauncherAgent への統合に改められた。

#### 実装方針の検討事項

- 各 Companion を独立 `.exe` にするか単一 `.exe` + サブコマンドにまとめるかは、上記「分割/統合の判断軸」で決着済み（ライフサイクル/結合で判断。LauncherAgent は probe/sensor/focus を統合、Updater は制約上別 exe）。新規 Companion を足す際も同じ軸で判断する。
- Common ライブラリ（複数 Companion で Win32 ヘルパーを共有する場合）の参照を静的リンクにするか動的にするかは、Common 追加時に配布物数とのトレードオフで判断する。
...
- WindowProbe（単発 OS.execute）は常駐 `LauncherAgent` に統合・廃止（#101 / #216 実装時）。PauseOverlay（透過 WPF 別窓）は実装されず、中断メニューは Launcher(Godot) 側の透明オーバーレイ窓（別 OS Window）+ 不透明 playing 背景の 2 枚構成に変更（#30 / #214。単一 CanvasLayer + メイン窓 per-pixel 透明化案も試したが、ウィンドウ起動ゲームの隙間にデスクトップが漏れる退行のため 2 枚構成に戻した）。統合エージェントは当初 `LauncherCompanion` 名だったが、Companions 配下の命名一貫性（Updater と同じ機能/役割ベース、カテゴリ名 "Companion" のスタッター回避）のため `LauncherAgent` にリネーム。
...
- **スループット**:
  ...
  - ゲームの多重起動を防止する仕組みが必要
- **リソース使用量**:
  - 想定環境（実機確認済み）: 内蔵GPU・8GB RAM の学校PC（具体スペックは下記「展示環境ハードウェア仕様」を参照＝確定値の SoT）
  - GPU が統合グラフィックスのため、ランチャー UI の描画負荷を低く抑える設計とする（プレイ中の描画/FPS 抑制は #214 で対応）
...
| 項目 | 仕様 |
| --- | --- |
| CPU | Intel Core i3-10105 @ 3.70GHz（第10世代、4コア8スレッド） |
| GPU | Intel UHD Graphics 630（CPU内蔵） |
| RAM | 8.00GB DDR4-2666 |
| ROM | 256GB |
| OS | Windows 11 |
| 台数 | 要確認（パソコン室キャパ約40人分） |
| 制約 | シャットダウン時にローカルデータがリセットされる |

> **重要**: 展示PCはシャットダウンの度にローカルのデータがリセットされるため、ランチャーの実行ファイル・ゲームデータ・データベース等すべてのファイルを **LAN教室サーバーの共有フォルダ上に配置し、そこから直接実行する** 構成とする（共有 DB / unified logs root / responses ドロップフォルダ等が共有フォルダ前提なのはこのため）。

##### LAN教室サーバー

| 項目 | 仕様 |
| --- | --- |
| 役割 | ランチャーシステム全体の格納・実行元（実行ファイル、ゲームデータ、データベース） |
| アクセス方式 | SMB共有フォルダ（ポート445 疎通確認済み） |
| 詳細スペック | 未確認（管理権限なし） |

##### ネットワーク環境（2026-04 時点の確認状況）

| 項目 | 状態 |
| --- | --- |
| TCP通信 | 利用可能（確認済み） |
| SMBプロトコル（ポート445） | 疎通確認済み |
| 共有フォルダアクセス | 利用可能見込み（要最終確認） |
| サーバー側からの接続テスト | 不可（管理権限なし） |
...
- **ハードウェア**:
  - 学校PCの仕様は実機確認済み（詳細は §3.1 パフォーマンス要件「展示環境ハードウェア仕様」を参照）
...
- **親子関係 (Companion → parent)**:
  ...
  - `Companions/LauncherAgent/` (#101 / #216 / #30) → Launcher (= Launcher 並走、技術的には realtime stdout redirect も可能だが pattern 統一のため post-hoc absorb)
...
- **zip 内の正規構造**: zip ルートに `Install.bat` / `INSTALL_README.txt` / `show_folder_dialog.ps1` / `Launcher.bat` / `Manager.bat` を直置きし、`files/` wrapper の中に component フォルダ群を置く。`Install.bat` は `files/` を `<親>/TonePrism/` にコピーし、zip ルートの `Launcher.bat` / `Manager.bat` を `<親>/` 直下 (= TonePrism/ の 1 階層上) にコピーする規約。
  ...
  └── files/                                 # 配布物本体（中身が <親>/TonePrism/ にコピーされる）
      ...
      └── Companions/                        # 補助 exe 集約 (§2.4)
          ├── Updater/                       # Manager 置換 + 再起動 (§3.7.4)
          └── LauncherAgent/                 # Launcher 補助の常駐エージェント: probe/sensor/focus (#101 / #216 / #30)
  ```
...
Launcher / Manager / 各 Companion (Updater / LauncherAgent 等、§2.4) は常に 1 つの zip に同梱され、1 回のアップデート操作で一括更新する。個別更新は許容しない。
...
2. Manager / 各 Companion (Updater / LauncherAgent 等、§2.4) を `msbuild /p:Configuration=Release` で個別ビルド（各プロジェクトの `bin/Release/...`。`.NET Framework 4.8` 系プロジェクトは `dotnet publish` 不可、`msbuild` 直叩き）
...
- **実装済みのみ記載**: 未実装機能（例: Launcher 終了制御 #84 / スタッフ呼び出し #83 / 音量 #82 / フィルター #29 等）は「実装予定」と明示し、動作するものとして書かない。機能 landing 時に該当章を更新（AGENTS.md「## Documentation」のドリフト防止に従う）。
...
##### 画面5: オーバーレイ画面（中断メニュー、#30 / #214 実装済）

> 初期案は「背景を暗転 + 左半分メニュー + 右半分に操作説明図」「オプション / 音量調整 を含む」だったが、実装は MVP として下記の通り。未実装項目は「将来拡張」に整理。

- **画面名**: オーバーレイ画面（中断メニュー）
- **目的**: ゲーム実行中に HOME / コントローラ Guide で開く中断メニュー
- **表示方法**（2 枚構成 #214）:
  - 中断メニューは**透明・最前面・borderless の別 OS Window**（`overlay_menu`、`per_pixel_transparency/allowed=true` + `embed_subwindows=false`）として走行中ゲームの上に重ねる。透明部分は背後の**ライブゲームを透過表示**（旧案の「背景暗転」ではなく、ライトテーマの白 veil でうっすら覆う＝ゲーム起動〜終了はライトの世界観に統一）
  - 背面に**不透明・全画面の playing シーン（背景アート）**を据え置き、ウィンドウ起動ゲームの隙間を背景アートで埋めてデスクトップが透けないようにする（透明窓→ゲーム / 隙間→背景アート の 2 枚サンドイッチ）。フォーカス奪取は `LauncherAgent.focus_hwnd` で overlay 窓だけを前面化（メイン窓を巻き込まない）
  - ※ 単一 CanvasLayer + メイン窓 per-pixel 透明化 + companion topmost 案も試したが、ウィンドウ起動ゲームの隙間にデスクトップが漏れる退行のため本 2 枚構成に戻した
- **レイアウト**（ライトテーマ）:
  - **画面右**: 縦並びのメニューリスト（右レール、濃いめの白半透明パネル）。項目は MVP の 3 つ:
    - 「続ける」（ゲームに戻る）
    - 「別のゲームをあそぶ」（ゲームを終了して選択画面に戻る）
    - 「退出する」（プレイを終了して席を離れる＝スクリーンセーバーへ）
  - **画面左**: 起動中画面と同位置・同サイズのゲームアイコン（サムネ）+ 時計（カルーセル画面 top_bar と同位置）
  - フォーカス枠は呼吸グロー（既存ブラウズ画面と統一）
- **表示トリガー**（常駐 `LauncherAgent` (§2.4) がゲーム前面でもグローバル検知）:
  - **コントローラー**: Guide ボタン（XInput `xinput1_4.dll` ordinal #100）
  - **キーボード**: Home キー
  - もう一度 HOME / Guide でトグル閉じ（= 続ける）
- **操作**:
  - 上下でフォーカス移動、決定で実行
  - 「続ける」: 閉じてゲームを前面に戻す（`LauncherAgent.focus`）
  - 「ホームに戻る」: ゲームを終了して選択画面へ復帰（#84 連動）
  - 「退出」: ゲームを終了してスクリーンセーバーへ
  - ESC キー / B ボタン（`ui_cancel`）で閉じる（= 続ける）
- **将来拡張（未実装）**: オプション / 音量調整（#82）/ スタッフ呼び出し（#83）/ 操作説明図（`controls` フィールドとリンク、キーボード・コントローラー両表示）等のメニュー項目。
...
- **UIデザイン**:
  ...
  - 詳細なレイアウトは実装時に決定
- **主要要素**:
  - **メニュー一覧**: 機能23で定義された14項目をカテゴリ別に表示
    - 診断・テスト（7項目）
    - ログ・情報（3項目）
    - 設定・操作（6項目）
...
│   └── LauncherAgent/        # TonePrism_LauncherAgent.exe（#101 / #216 / #30、Launcher 起動時に常駐起動）
├── games/                    # ゲームデータ格納フォルダ
│   ├── {game_id}/
│   │   ├── v1.0.0/
│   │   │   ├── game.exe
...
- **主要機能**:
  ...
  - オプション画面（基本設定の変更）
  - コントローラー・キーボード両対応
  - サービスモード機能（診断・テスト、ログ確認、設定・操作、終了制御）
...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-05-25 | 1.10.42 | §機能23 サービスモードの **「4. ゲーム一覧状態確認」と「5. ゲーム起動テスト」を 1 項目「ゲーム動作確認」に統合** し、中で 3 段階の確認方法を選ぶ方式に変更 (#74 実装中)。①ファイル存在チェック (起動しない・全件一括) / ②起動テスト (自動起動→ウィンドウ生成確認→自動終了で起動可否のみ判定) / ③試遊確認 (選んだ 1 本を実際に起動・終了は手動)。3 段階で「ファイルがあるか」→「起動するか」→「実際に遊べるか」を粒度別に確認できるようにする狙い。項目統合により機能一覧は 16→15 項目に。実装状況: ① は実装済 (GameRepository + GamePathResolver で exe 存在確認)、②③ はスタブ (後続)。**あわせて「8. エラー内容表示+マニュアル」項目を廃止** — エラーコード別の対処法はエラー発生時のダイアログに併記する方式 (`error_dialog.gd` の `_REMEDY`、ゲーセン筐体のエラー表示と同様。スタッフが別途マニュアルを引かなくてよい) に変更したため。これにより機能一覧は **15→14 項目** に、以降を繰り上げ (§画面8 メニュー一覧の項目数記述も 14 に同期)。**あわせて UI 文言の「アプリ」を「ランチャー」に統一** — 同じ対象を指すのに「ランチャーの再読み込み」と「アプリの再起動/終了」が混在しており、かつ筐体ではゲームも"アプリ"のため曖昧だったため。項目13/14 を「ランチャーの再起動」「ランチャー終了」に改称し、終了制御 (Alt+F4 封印) の案内ダイアログ・ボタン文言も同調 (機能「終了制御」§詳細も同期)。**項目3「画面表示テスト」を拡張** — 従来の単色6色に加え、グリッド (格子線+中央十字+四隅対角線+セーフエリア枠90%/80%を1パターンに統合)・カラーバー (放送規格 SMPTE RP 219 / ARIB 準拠の HD カラーバー: 75%カラーバー+40%グレー袖 / リバース / 0-100%ランプ / PLUGE の 4 段)・グラデーション (黒→白階調) を追加。全画面 `Control` の `_draw()` で描画する方式 (画像アセット不要・解像度追従) に変更。**機能22 終了制御に例外を追加** — サービスモード表示中のみ Alt+F4 での終了を許可 (スタッフ専用の隠し画面のため誤終了の懸念なし、素早い終了手段。ただしゲーム実行中は除外)。 | Kenshiro Kuroga \& Claude |
| 2026-05-25 | 1.10.41 | §3.1 パフォーマンス要件に **実機確認済みの展示環境ハードウェア仕様** を反映 (展示PC: i3-10105 / UHD Graphics 630 / 8GB DDR4 / 256GB / Win11、LAN教室サーバー、ネットワーク環境)。旧記述の暫定値「Core i3 11世代、メモリ8GB程度」(§3.1 / §3.5 の 2 箇所) を確定値へ更新し、§3.5 ハードウェアは §3.1 参照に集約。あわせて「展示PC はシャットダウンでローカルがリセットされるため全ファイルを LAN 教室サーバー共有フォルダから直接実行する」制約を明記 (共有 DB / unified logs root / responses ドロップフォルダが共有フォルダ前提である根拠)。本内容は 2026-04 の未マージブランチ `docs/spec-hardware-network` (当時 v1.7.0) に滞留していたハード仕様を現行 SPEC へ救出したもの。Monitor 通信の疎通状況・全体構成図 (mermaid) の改変は暫定 + Monitor 未実装 + ブランド rename 前で現行と競合するため本反映から除外。コード変更なし・SPEC のみ。 | Kenshiro Kuroga \& Claude |
| 2026-05-25 | 1.10.40 | §機能23 サービスモードの機能一覧から **項目「メンテナンスモード切り替え」を廃止** (#220)、以降の項目を繰り上げ。**廃止理由** (設計議論で確定): アンケートを任意設計にするためスキップ機能不要 / テストプレイのデータ汚染防止は将来のプレイ・アンケート閲覧画面の**日付範囲フィルタ**で本番日抽出する方が forget-proof (ランタイムトグルは忘れる人は忘れる) / 操作説明 (#31) は初回起動時のみ表示ならテスト中に繰り返し邪魔されず不要 / 残るアイドル・スクリーンセーバー無効も効く場面の頻度・実害が低く運用が回るため不採用。**項目数**: 元 16 項目、1.10.39 で「ランチャー表示モニタ選択」を追加して 17、本廃止で **16 項目** に戻る (§画面8 メニュー一覧の「16項目」記述と整合)。サービスモード本体 (機能23) は未実装かつメンテナンスモードの関連コードも Launcher 側に存在しない (grep 済) ため、コード変更なし・SPEC のみ。 | Kenshiro Kuroga \& Claude |
| 2026-05-25 | 1.10.39 | §機能23 サービスモードの機能一覧「設定・操作」に **項目13「ランチャー表示モニタ選択」を新規追加** (#30 マルチモニタ対応の議論から)。複数モニタ環境 (部員の動作確認等) でランチャー表示モニタをサービスモードからその場で切替 (`window_set_current_screen` で即移動)、**メモリのみ保持で再起動はプライマリに戻る** (項目11/13 と同じサービスモードの揮発方針 = スタッフが変な状態で離れる事故防止)。永続設定 / 起動時適用は持たず、恒久運用は Windows のメインディスプレイ設定で代替。サービスモード本体 (機能23) は未実装のため本項目もコードは未実装、実装時に同梱する設計メモ。**背景**: 中断メニュー (#30) は透明 overlay を always_on_top で重ねる 2 枚構成のため、ゲームが排他的フルスクリーンだとそもそも非対応 (= overlay が描けない) で、メニューが動くゲームは必ず移動可能な (ボーダーレス/ウィンドウ) 窓。ゲーム窓の出現モニタはゲーム任せで制御不能なため、本 PR では (A) overlay をゲーム窓のいるモニタへ追従 + (B) watch 開始時にゲーム窓をランチャーのモニタへ寄せる (`Win32Windows.PlaceWindowCentered`) の 2 段で吸収済 (Launcher v0.8.0 / LauncherAgent v0.1.0)。本項目13 はその「ランチャー自体の表示モニタ」を将来スタッフが選べるようにする補完。本 bump は §2.4 中心の 1.10.38 とは別節 (§機能23) の新規スコープのため別行。§機能4 / 画面5 のオーバーレイ実装記述更新は 1.10.38 と同じく後続対応予定。 | Kenshiro Kuroga \& Claude |
| 2026-05-23 | 1.10.38 | §2.4 Companions を実装実態に合わせて全面改訂 (#30 / #101 / #216 / #214、Launcher v0.8.0 / LauncherAgent v0.1.0)。旧仕様は WindowProbe (単発 CLI) と PauseOverlay (透過 WPF 別窓) を**別 Companion として計画**していたが、実装では (a) WindowProbe を常駐エージェントに統合・廃止、(b) PauseOverlay は作らず中断メニューを Launcher(Godot) 内の単一 CanvasLayer + メイン窓 per-pixel 透明化方式に変更 (#214) したため、§2.4 から両者の別 exe 記述を除去し、**probe(#101/#216) + sensor(HOME/Guide #30) + focus/topmost を 1 プロセスに統合した常駐エージェント `LauncherAgent`** の記述に置換。構造図 / 呼び出し主体表 / 通信規約 (常駐 UDP に一本化、stdout/JSON 単発系の記述を削除) / 各 Companion 機能節を更新。**「分割/統合の判断軸」subsection を新規追加**: Companion を別 exe にするか統合するかは「責務の数」でなく「ライフサイクルと結合度」で決める (Updater = 実行中 exe は自己置換不可 + 親 dead 期間に動くハード制約で別 exe / LauncherAgent = Launcher と同ライフサイクル + 単一 UDP + 単一メッセージポンプ共有で統合) を明文化。あわせて §2.4 内に二重化していた「通信規約」「各 Companion の機能」「実装方針の検討事項」の重複ブロックを 1 つに集約。**命名**: 統合エージェントは当初 `LauncherCompanion` 名だったが、`Companions/` 配下の命名一貫性 (Updater と同じ機能/役割ベース、カテゴリ名 "Companion" のスタッター回避) のため `LauncherAgent` (exe `TonePrism_LauncherAgent.exe` / namespace `TonePrism.LauncherAgent` / Godot autoload `LauncherAgent`) にリネーム、これも §2.4「旧仕様との差分」に記録。SPEC §3.x 側に残る旧 PauseOverlay / WindowProbe 参照と §機能4 / UI「画面5 オーバーレイ画面」の実装記述更新、docs ドリフト注記 (#30 を実装済へ) は後続で対応予定。 | Kenshiro Kuroga & Claude |
