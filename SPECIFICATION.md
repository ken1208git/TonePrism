  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    4. ゲーム動作確認 — 3 段階の確認方法を選んで実行する（旧「ゲーム一覧状態確認」と「ゲーム起動テスト」を統合）。詳細ペインはまず確認方法の選択画面を出し、選ぶと中身全体がそのモード画面に切り替わる。モード画面で ←/B/Esc を押すと確認方法の選択へ戻る（詳細ペインからは出ない＝サブ階層ナビ）:
        ...
        - **② 起動テスト**（自動）— 各ゲームにチェックボックス（既定全選択）+「すべてチェック/すべて外す」+「テスト開始」。チェックしたゲームを1本ずつ cmd 経由で起動し、LauncherAgent(Companion)がウィンドウ生成を検出したら **OK**、規定時間(12秒)内に窓が出なければ **NG: ウィンドウが出ない**、起動直後にプロセスが消えたら **NG: すぐ終了(クラッシュ?)**、起動自体に失敗したら **NG: 起動失敗**、exe 不在は **NG: 実行ファイルなし** と各行に判定表示。判定後は taskkill /T /F で自動終了し次へ。Companion 不在時(エディタ等)は窓検出不可のため一定時間生存で「OK(起動のみ確認)」にフォールバック。画面を離れる/閉じると進行中テストは中断しゲームも終了させる（置き去り防止）。DLL 不足・起動時クラッシュ等の検出用
