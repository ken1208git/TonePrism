...
- **カラム**:
  ...
  - **(#211/#212) Manager 編集 UI の条件付き規則**: (a) **タイプ↔ソース**: スライドショー(1)/タイルグリッド(2) は `section_source='manual'` のみ (背景画像/表示テキスト前提の厳選枠のため自動ソース不可)。通常カテゴリ行(0) は手動＋全自動ソース可。(b) **max_display_count の有効性**: ランキング系ソース (popular=play_count DESC / recently_played=last_played DESC / random) でのみ上限が意味を持つ (TOP5 等の特集枠) ため編集可。手動 (= 割当を全件表示) と条件系 (genre/players_*/difficulty/play_time/online/controller、および **recent=新作** [`release_year=今年` を display_order 順 = 本物のランキングでないため condition 扱い]、= 条件一致を全件表示) では UI をグレーアウトし `max_display_count=0` で保存する。
...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-01 | 1.10.54 | **(#211/#212、Manager v0.21.0)** ストアセクション編集 UI に条件付き規則を追加し §7.3 store_sections の `section_type`/`max_display_count` 説明に明記。**#212**: スライドショー/タイルグリッドはソースを `manual` のみに制限 (背景画像/表示テキスト前提の厳選枠、自動ソースは破綻しやすい)。通常カテゴリ行は手動＋全自動可。legacy の自動×スライドショー/タイルグリッドは編集時に手動へ coerce + 案内。**#211**: `max_display_count` はランキング系 (popular/recently_played/random) のみ編集可 (TOP5 等の特集枠で上限が有効)、手動 (割当全件) と条件系 (genre/players_*/difficulty/play_time/online/controller + **recent=新作**[今年の release_year を display_order 順=本物のランキングでないため condition 扱い、#290 review]、条件一致全件) はグレーアウト + `0` 保存 (= 上限なし、Launcher の `max_display_count<=0` 解釈)。スキーマ変更なし。docs `usage/store.md` も更新。実機 UI 確認は別途必要。 | Kenshiro Kuroga \& Claude |
