...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **設定・操作:**
    ...
    13. ランチャー表示モニタ選択 — 複数モニタ環境で、ランチャーを表示するモニタをその場で切り替え（`window_set_current_screen` で即移動）。**メモリのみ保持（再起動でプライマリに戻る）**。本番=単一モニタでは無関係。恒久的に別モニタで運用する場合は Windows のメインディスプレイ設定を使う。関連: 中断メニュー（機能4 / #30）はゲーム窓をランチャーと同じモニタへ寄せるため、ここで選んだモニタにゲーム窓も揃う
...
- **スループット**:
  ...
  - ゲームの多重起動を防止する仕組みが必要
- **リソース使用量**:
  - 想定環境（実機確認済み）: Intel Core i3-10105（第10世代・4コア8スレッド）/ GPU Intel UHD Graphics 630（CPU内蔵）/ RAM 8GB DDR4-2666 / ROM 256GB（詳細は下記「展示環境ハードウェア仕様」）
