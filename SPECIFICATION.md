...
- **優先度**: 中
- **詳細**:
  - **ゲーム個別アンケート**: ゲーム終了時に表示（`trigger='game_end'`、`game_id` 付き）。面白さ(1-5)、コメント（自由記述、上限 200 字）を収集。
  - **全体アンケート**: ランチャー終了時（退出時）に表示（`trigger='launcher_end'`、`game_id=null`）。全体の満足度(1-5)、コメントを収集。
  - **スキップ可能**: 強制すると評価品質が下がるため、両アンケートとも明示的にスキップ可能とする。スキップ時はデータを保存しない
  - **保存方式（#297）**: Launcher は SQLite に直接書き込まず、1 件 1 ファイルの JSON として `responses/surveys/YYYY-MM-DD/` に atomic 出力する（§6.5 / §7.5.3）。Manager の SQLite 取り込みは行わない（集計が必要なら Launcher / 別ツールが JSON を直読みする）。
  - 実装は #297 PR3（#35）。
...
- **優先度**: 中
- **詳細**:
  - **集計方式（#297）**: Launcher が `responses/play_records/` の JSON をメモリ集計してランキングを算出（SQLite を経由しない）。ストアの `popular`（人気順）/ `recently_played`（最近プレイ）セクションのデータソースになる。
  - #297 PR2 は「会期全体の count 合計 / 最新 N」までの**基本実装**に留め、集計方式の精緻化（回数順/時間順・会期/当日・専用画面）は **#36** で詰める。
  - ランチャーUIにランキングを表示・更新。
...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    1. 入力チェック（タイトルは「入力チェック」、コントローラー接続状況も含む）— **接続中のコントローラー一覧**（パッド番号＋デバイス名、抜き差しで自動更新）を常時表示。一覧表示中に入力を観測するとメニュー操作と競合するため、「**入力確認を開始**」ボタンで **確認モード** に入り、その間だけ押したボタン/キー/スティックを**分かりやすい名前**（A/B/X/Y・LB/RB・十字キー・左右スティック方向・LT/RT・キー名等）でライブ表示する。確認モード中はメニューのフォーカスを無効化してナビと競合させず（入力自体は消費しないので ServiceMode のアイドル計測/Ctrl+Alt+F12 は有効）、**Esc キー3回** または **Guide ボタン3回** で戻る（テスト中に押して誤って抜けないよう複数回）。これで「パッドが効かない」を ①一覧に出ない=OS未認識（ケーブル/レシーバー/電池/ドライバ）/ ②一覧に出るが反応しない=マッピング等の問題 / ③出て反応もする=正常（ゲーム側の問題）の3択で切り分け可能
    2. 音声チェック — テスト音を再生。音が出ない問題の即診断
    3. 画面表示テスト — 開始すると複数のテストパターンを**順番に全画面表示**する（任意キー/クリック/パッドで次へ、←/Backspaceで前へ戻る、Esc/Bで中断、最後まで行くと自動でメニューへ戻る）。テスト中はパターンの邪魔をしないよう画面に文字は出さない（操作方法は開始前の説明に記載）。表示順: ジオメトリ/クロスハッチ（細密32px+粗128pxのクロスハッチ+四隅対角線+中央&四隅の真円〔画素アスペクト/歪み確認〕+中央十字+セーフエリア枠90%/80%で解像度/スケーリング/歪み/端の見切れを一括確認）→ カラーバー（放送規格 SMPTE RP 219 / ARIB の HD カラーバー: 上段75%カラーバー+両端40%グレー袖 / リバース段 / 0-100%ランプ段 / PLUGE段で色再現・色順・黒レベルを確認）→ 解像度+グレースケール（中央シーメンススター〔放射状の白黒くさび、シャープネス・スケーリングのボケ/モアレ確認〕+上部に1〜4pxピッチの周波数縞+下部に11段の離散グレースケール〔ガンマ・階調段差確認〕）→ 単色（白・黒・赤・緑・青・グレー50%）。階調・バンディング確認はカラーバーのランプ段でカバーされるため、黒→白グラデーションの単体表示は設けない。モニター相性・拡大率・解像度ズレの事前発見用。実装は全画面 `Control` の `_draw()` で描画（画像アセット不要・解像度追従）
    4. ゲーム動作テスト — 3 段階の確認方法を選んで実行する（旧「ゲーム一覧状態確認」と「ゲーム起動テスト」を統合）。詳細ペインはまず確認方法の選択画面を出し、選ぶと中身全体がそのモード画面に切り替わる。モード画面で ←/B/Esc を押すと確認方法の選択へ戻る（詳細ペインからは出ない＝サブ階層ナビ）:
        - **① ファイル存在チェック**（起動しない・全件一括）— 各ゲームの exe 存在チェック、パス切れ/ファイル欠落の事前発見。LAN 共有未接続・フォルダ移動・パス誤りを即検出
        - **② 起動テスト**（自動）— 各ゲームにチェックボックス（既定全選択）+「すべてチェック/すべて外す」+「テスト開始」+「テストを中止」（実行中のみ有効、押すと進行中を止め残りを「中止」表示）。チェックしたゲームを1本ずつ cmd 経由で起動し、LauncherAgent(Companion)がウィンドウ生成を検出したら **OK**、規定時間(30秒)内に窓が出なければ **NG: ウィンドウが出ない**、起動直後にプロセスが消えたら **NG: すぐ終了(クラッシュ?)**、起動自体に失敗したら **NG: 起動失敗**、exe 不在は **NG: 実行ファイルなし** と各行に判定表示。判定後は taskkill /T /F で自動終了し次へ。Companion 不在時(エディタ等)は窓検出不可のため一定時間生存で「OK(起動のみ確認)」にフォールバック。画面を離れる/閉じると進行中テストは中断しゲームも終了させる（置き去り防止）。DLL 不足・起動時クラッシュ等の検出用
        - **③ 試遊テスト**（手動終了）— 起動テスト②と同様のチェックリスト（各ゲームにチェックボックス+「すべてチェック/外す」+「試遊開始」+「中止」）。「試遊開始」を押すと、まず **戻り方（ゲーム終了 or HOMEキー/Guideボタン）を案内する確認モーダル** を出して開始の同意を取る。チェックしたゲームを **1本ずつ自動で起動** して試遊し、遊び終わったら **ゲームを手動で終了** するか **キーボードのHOMEキー / コントローラーのGuideボタン**（中断メニューと同じトリガ。押すと強制終了）で復帰。戻ると**中央モーダル**で「『〇〇』は正しく遊べましたか？」と尋ね（サービスモードはDialogManagerより前面のため専用の汎用モーダルを使用）、スタッフが **〇 遊べた / × 問題あり** を記録すると **自動で次のチェック済みゲームへ進む**。結果は各行に色付き表示。exe不在/起動失敗は自動で×。「ファイルはあるが実際に遊べるか」を人が最終確認する用途
    5. ネットワーク接続テスト — 「接続テストを実行」で、手前から順に **①ローカルIP取得 → ②ゲートウェイ疎通 → ③DNS解決 → ④インターネット接続 → ⑤インターネット速度 → ⑥共有サーバー接続 → ⑦共有サーバー読み込み速度 → ⑧Monitor接続** を確認し各段階を OK/NG＋応答時間・速度で表示（最初に×が出た所が原因）。疎通(①〜④⑥)実装: ②ゲートウェイは `route print` から既定ゲートウェイIPを取得し `ping`（locale非依存に "TTL=" で判定）。pingを返さないルーターは多いため、ping無応答でも `arp -a` でMACが取れれば（L2到達OK＝機能している）OK扱いにフォールバック、③DNSは `IP.resolve_hostname`、④インターネットは 1.1.1.1:443 へ TCP 接続（DNS非依存）、⑥共有サーバーは DBパス(UNC)から抽出したホストへ TCP:445。ping/TCP はブロッキングのため**別スレッド**で実行し `call_deferred` で反映（画面を固めない）。速度(⑤⑦)は **LauncherAgent(Companion) に委譲**して正確に測る（Godot単体では困難なため）: ⑤インターネット速度は Companion が**6本並列DL**（各接続が75MBを1回要求し締切まで流しっぱなしの継続ストリーム）を約5秒回して Mbps 算出（細切れ再リクエストのスロースタート過小評価を回避し測定サイト並みの値。Cloudflare `__down` は100MBで403を返すため75MBが上限）、⑦共有サーバー読み込み速度は Companion が `FILE_FLAG_NO_BUFFERING` で**OSキャッシュを回避**して実 MB/秒を算出（最大100MB）。**測定対象は Launcher から渡された `games` フォルダ配下の最大ファイル**（`ResolveReadTarget` が走査。exe は Godot ゲームだと数百KBと小さく量不足で速度がブレるため対象にせず、`.pck`/プレビュー動画`.mp4`/Unityの`.resS`等の巨大ファイルを選ぶ）。Launcher は `request_speedtest(path, run_id)` で依頼し `speedtest_result(kind, ok, text, run_id)` シグナルで結果受信（`run_id` で要求と結果を照合し、古いプローブの遅延結果を無視）。⑧Monitor は未実装のため「未実装」表示。⑥⑦共有サーバーは本番がそこからゲーム/DBを読むため最重要
    6. データベース整合性チェック — DBファイルの存在確認、テーブル存在確認、レコード数表示、読み書きテスト。「データが反映されない」問題の切り分け

    **ログ・情報:**
    7. 簡易ログ確認 — 直近のエラーログ表示（メモリバッファから現セッションのログを取得）
    8. システム情報表示 — スクロール可能なリストで4分類表示: **ハードウェア**(CPU名+コア数 / GPU名+ベンダ / 物理メモリ+空き / アプリ使用メモリ)、**ディスプレイ**(現在モニタ+解像度 / リフレッシュレート / ウィンドウサイズ / モニタ数 / DPI / V-Syncモード / 現在FPS / 描画API)、**システム**(PC名 / OS+ビルド / ロケール / 現在日時 / 稼働時間 / ローカルIPv4)、**バージョン/パス**(Launcher版 / Godot版 / PID / 実行ファイルパス / DBパス)

    **設定・操作:**
    9. デバッグオーバーレイ切り替え — ON/OFFトグル。ONにするとサービスモードを閉じても画面隅にFPS・メモリ使用量・PC名・現在のシーン状態・DB/Monitor接続状態等をリアルタイム表示し続ける（マイクラのF3的な常時表示HUD。ただし画面上に「F3」とは表示しない＝本アプリのトグルはF3キーではないため）。サービスモード・中断メニュー・ゲームなど何の上にも出すため、中断メニューと同じ「透明・borderless・最前面(always_on_top)の別OSウィンドウ」として実装し、さらにunfocusable+mouse passthroughで下の操作を妨げない。出すモニタはゲーム実行中ならゲーム窓のいるモニタ、それ以外はランチャーのモニタに追従。メモリのみ保持（再起動でOFF）
    ...
    14. ランチャー終了 — ランチャーを終了。Alt+F4/×ボタン封印のため唯一の終了手段。サービスモード自体が隠し画面(Ctrl+Alt+F12)でここまで辿る操作が十分意図的なため、二段階確認は設けず押下で即終了する(ゲーム実行中は除外)
...
- **詳細**:
  - 削除実行は **DB レコード + `games/{game_id}/` フォルダのセット削除**（Manager v0.8.3 で実装、`DeleteGameConfirmForm`）。「DB のみ削除」のオプション分岐は持たない
    - DB 側は `games` 行と CASCADE で削除される関連レコード（developers / game_versions / store_section_games）。※ 旧 `play_records` / `surveys` は DB v23 (#297) で撤去済み
...
- **詳細**:
  ...
  - ゲームファイルの差し替え
  - **画像の差し替え仕様 (Manager v0.17.0 以降、M10)**:
    - サムネイル / 背景の選択 dialog はゲームフォルダ外 (デスクトップ / Downloads 等) のファイルも選べる
    - OK 押下時、選択した画像が **編集中バージョン (= ドロップダウン選択中) の `games/{game_id}/v{version}/` 配下** に自動コピーされる
      - コピー先で同名衝突した場合は `ImageNameConflictDialog` を表示し、衝突相手 path / コピー元 path / リネーム提案 (`<base>_2.<ext>` 形式で衝突しないまで increment) を提示。user は TextBox で自由にリネーム編集可能 (拡張子は `.png` / `.jpg` / `.jpeg` / `.bmp` 制限、invalid char check、再衝突 check)
      - **古い画像ファイルは削除しない**: 古いバージョンの行が旧画像 path を参照している可能性があるため、上書きや既存削除は禁止 (= 古いバージョンの参照を壊さない方針)
      - コピー処理は `ProcessingDialog` 経由で worker thread 実行 (SMB 越しの数百 ms にも UI freeze なし)
      - コピー中の cancel / 失敗時は partial copy 済ファイルを rollback して入力画面に戻る
    - 既にゲームフォルダ内にある画像を選んだ場合は従来通り (= コピー処理なし、相対 path で保存)
    - **実行ファイル (`txtExecutablePath`) には引き続きゲームフォルダ内限定**: 外部 exe は VersionUp 経路でしか取り込ませない (設計意図維持、ゲームの起動方法を「fresh copy + 全 asset 同梱」に強制する規約)
...
- **詳細**:
  ...
  - **自動バックアップ（#295 で再設計）**: データ変更操作（ゲーム追加/編集/版up/削除・ストアセクション・イントロスライド等）の**成功直後**に取る「操作単位 / replace-in-session」方式。1 Manager 起動 = 1 自動世代（同セッション内は前世代を消して上書き、retention は直近 N **セッション**）。DB（~124KB）は毎操作、重い games/guide のアセット取得（~6GB）は games/guide を実際に変える操作のときだけ。**perf 特性**: アセット取得は manifest 生成のため games/+guide/ を**毎回ツリー全列挙**する（CAS で再コピーは増分でも列挙 + stat は毎回＝コストは操作回数に線形）。連続アセット操作（文化祭の連続ゲーム追加等）の体感対策として、**#299 で非ブロッキング化＋コアレスを実装済**: バックアップはバックグラウンド worker で走り（モーダル廃止＝操作継続可）、進捗は下部ステータスバーに統合表示（`[バックアップ中...][中止][バー][N%][ファイル名]`）。実行中に来た変更はコアレスされ最後に 1 回だけ走る（`SessionBackupCoordinator.TryStartRun`/`TryContinue`）。中止ボタン・閉じる確認・未バックアップ時の「今すぐバックアップ」1 クリック復旧つき。本番 SMB での実体感は pre-release 全体テストで確認。設定「変更があったら自動でバックアップする」(`backup_auto_enabled`) で ON/OFF。手動は別枠で温存。実装は `Services/SessionBackupCoordinator.cs`（各 Form の `AssetsChangedOnDisk` で「ディスクのゲーム本体を触ったか」を判定し不要な走査を回避）。
    - 旧方式（Manager 起動時に時間間隔チェック）は「変更→閉じる→次の起動までに DB 破損でその変更が一度も控えられない」穴があり #295 で廃止。間隔 key (`backup_auto_interval_hours` / `_unit`) と起動時 lease (`TryAcquireBackupLease`) も撤去（操作単位では interval/多ホスト直列化が不要、同時編集は `SessionConflictHelper` が警告、アセット GC は auto 限定+grace で多ホスト安全）。
  ...
  - **マルチPC重複防止**: 旧「`last_backup_at` を `BEGIN IMMEDIATE` で更新する lease 方式」は #295 で撤去（操作単位化で起動時の多ホスト直列化が不要に）。現在は同時編集を `SessionConflictHelper` が警告し、アセット GC は auto 限定 + grace(24h) で多ホスト並行を許容する（被害は backup 世代のみで live は自己回復、#250 C2 は PR2/3 へ deferred）
  - **バックアップ履歴一覧**（Manager v0.17.0 / DB v19 で file-scan 化）: `backups/` フォルダを `BackupCatalogService` で走査して表示（作成日時 / 実行PC / トリガ / サイズ / ファイルパス）。種類はフォルダ位置（`auto/`・`manual/`・`safety/`、保存先直下の旧フラット形式は種類復元不能のため「不明」）、日時・実行 PC はファイル名 `<種類>_<日時>_<host>.db`、サイズは `FileInfo` から導出する。日時はファイル由来で開始 / 完了の区別が無いため「作成日時」1 列に統合（旧「開始日時 / 完了日時」の 2 列は同値だったため）。**DB の `backup_log` テーブルは廃止**したため、reconcile / register / failed 自動掃除といった「DB ↔ ファイルのズレ補正」処理は不要になった（走査結果は常に実在ファイルのみ）。「状態」列は #200 で削除済。
    - 走査由来のため `File.Exists` 追加フィルタ不要。並び順はファイル名タイムスタンプ降順（固定幅ゼロ埋めなので文字列ソート = 時系列）。
    - 失敗したバックアップはファイルを残さない（= 履歴に出ない）。失敗は Logger（ファイルログ）にのみ記録する。
  - **個別削除 UI**: 履歴を選んで「選択した履歴を削除...」ボタン → 確認ダイアログ → バックアップファイルを物理削除（DB 行は無い）。（#295 以前は auto 削除時に最新 auto の時刻へ `last_backup_at` を rewind していたが、#295 で last_backup_at はトリガ gate でなくなった＝取り直し skip の概念が無いため rewind も撤去）。
  - **プロジェクト移動耐性**: 常に現在の保存先フォルダを走査するため、プロジェクトを別ドライブに移動しても履歴がそのまま追従する（旧 `relative_path` 追従機構は不要になり撤去）。
  - **保存先設定**: デフォルトは `<DBファイルのフォルダ>/backups/`、設定で任意のフォルダに変更可能
  - **ファイル名規則**: `<種類>_YYYYMMDD_HHmmss_<host>.db`（種類 = `auto` / `manual`、host = 実行 PC 名をファイル名向けに正規化した値）。同一 PC・同一秒の衝突時のみ `_2` / `_3` を付与。退避は `backups/safety/safety_YYYYMMDD_HHmmss_<host>.db`（safety も auto/manual と同様 host を埋め込み、複数 PC の同時復元での同名上書きを防ぐ）。移行前の旧フラット形式 `toneprism_YYYYMMDD_HHmmss.db` / `prism_YYYYMMDD_HHmmss.db`（v0.20.0 以前）も走査対象だが、auto/manual の区別がファイル名に無く復元不能なため**「不明」表示**とする（retention は `auto/` フォルダ限定なので不明扱いでも自動削除されない）
  - **世代管理**: 設定値 `backup_retention_count`（デフォルト 30）を超える古いバックアップは自動削除
  - **リストア機能** (Manager v0.17.0 で atomic 方式へ改修):
    1. 履歴一覧から復元したいバックアップを選択
    2. 確認ダイアログ表示（「全Launcher を停止してください」「現DBは退避されます」、フルパスはカタログ走査で得た絶対パス）
    3. 4桁の確認コード入力による誤操作防止（`ResetDatabaseConfirmForm` と同じパターン）
    4. `SessionConflictHelper.CheckBeforeWrite` で他 PC からの同時編集を検出
    5. 現在の `toneprism.db` を `backups/safety/safety_HHmmss.db` として Online Backup API で退避（ライブコピー）
    6. 選択されたバックアップを `toneprism.db.restore-tmp` に先にコピー（`toneprism.db` はまだ無傷）
       - ここまでで中断・失敗しても `toneprism.db` は破壊されない（Codex P1 "Disallow cancellation after deleting active DB files" 対応）
    7. SQLite 接続プールをクリア
    8. `toneprism.db-wal` / `toneprism.db-shm` を削除（journal_mode=DELETE のため通常は不在、防御的）
    9. `File.Replace` で `toneprism.db` を tmp で atomic 置換（NTFS の ReplaceFile Win32 API、`backupFileName=null` で旧 DB の自動 `.bak` は作らない）
    10. `backups/safety/` のリテンション適用（既定 10 件、`RestoreService.DefaultSafetyRetentionCount`）
    11. スキーマ migration を実行（古い schema のバックアップ復元時に後続の Reconciliation クエリが失敗しないよう、Reconciliation の前に必ず走らせる）
    12. `RestoreReconciliationService` で DB と `games/` フォルダの整合性をチェック、findings があれば `RestoreReportForm` を表示
    13. `DatabaseChanged` イベントで各パネルを再ロード
- **設定値**（`settings` テーブルに保存）:
  - `last_backup_at`（UNIX秒）: 最終バックアップ完了時刻（#295 でトリガ gate / lease は廃止。**現状この値を read する経路は無い**＝write-only の dead-but-cheap write、将来の履歴/外部参照用に残置。「最終バックアップ」表示は `BackupCatalogService.GetLastSuccess` のファイル走査由来）
  - `backup_destination_path`（TEXT）: 保存先フォルダ。空ならデフォルト
  - `backup_auto_enabled`（TEXT, デフォルト "true"）: 変更時の自動バックアップ ON/OFF（"false" 厳密一致のみ無効）。#295 で時間間隔 key (`backup_auto_interval_hours` / `backup_auto_interval_unit`) は撤去
  - `backup_retention_count`（INTEGER, デフォルト 30）: 保持する世代数
  - `asset_snapshot_enabled`（TEXT, デフォルト "true"）: ゲーム本体バックアップ取得の ON/OFF（"false" 厳密一致のみ無効）。**round9 で UI から撤去＝隠し既定 true**（DB と一括バックアップに統一。将来/開発用の escape hatch として key は残置）。保持世代数は `backup_retention_count` に統一（専用 key `asset_snapshot_retention_count` は廃止）。

- **アセット控え（games/ + guide/ の共有プール (CAS) バックアップ、#250 PR1、Manager v0.21.0）**:
  - **動機**: DB バックアップは `toneprism.db` 単体のみで、実行ファイル/サムネ/背景/各 `v{}/` を含む `games/`（~6GB）と `guide/`（intro 画像）はバックアップされない。ディスク障害/誤削除で復旧不能になるのを防ぐ。
  - **方式選定**: 当初ハードリンク世代スナップショット案だったが、**プロジェクト全体が SMB 上 + ファイルサイズ管理がシビア**という制約で不採用（ハードリンクは 1 実体に複数エントリのため、ファイルサイズを単純合計するエクスプローラー/サーバー容量計算が二重カウントし削減が見えない）。実測で games/ は中身の 63% が重複（共通ランタイム/版違い）→ **共有プール (CAS)** を採用（中身ごとに実体 1 個＝どんな容量集計にも正直、SMB 越しでも有効）。
  - **取得**: `AssetSnapshotService.CreateSnapshot` を DB バックアップ成功直後に**同一 timestamp/triggerType** で呼ぶ（`BackupService.RunBackupCore`、best-effort = 失敗/キャンセルしても DB バックアップの成否・`last_backup_at` を壊さない）。
  - **プール**: `<backup_dest>/asset_pool/<hash[0:2]>/<sha256hex>` に中身ごとに実体 1 個（先頭 2 桁で分散）。**SHA-256** のため「中身違いが同一 hash」は起こらず、名前が同じでも中身が違えば別保存、名前が違っても中身が同じなら集約。配置は temp→rename で atomic。
  - **目録 (manifest)**: `<backup_dest>/asset_snapshots/<auto|manual>/<yyyyMMdd_HHmmss>[_host].manifest`（`META\t<ts>\t<host>\t<trigger>\t<fileCount>\t<logicalBytes>` ヘッダ + `hash\tsize\tmtimeTicks\trelpath` 行、relpath は `games/…`/`guide/…`）。world は temp→rename。世代名 timestamp は同時刻 `.db` バックアップと対応（PR3 布石）。
  - **SMB ハッシュキャッシュ**: 直近 manifest を読み `relpath+size+mtime` 完全一致なら前回 hash を流用（再ハッシュ＝再読込しない）。2 回目以降は新規/変更分だけ読む。size+mtime 据え置きで中身だけ変える特殊ケースに弱い rsync 同等トレードオフ（games/ 追記専用で実害なし）。symlink/junction はスキップ、長パスは `\\?\`（**UNC は `\\?\UNC\server\share\…`**＝単純前置だと SMB 上で列挙が構文エラーになるため、#250 C1）、**除外リストは適用しない（丸ごと取る）**。理由: 控えはバックアップであり、Electron 系ゲームの `node_modules` 等「実行時に必須」なフォルダを `ExcludedFolders`（import 時に取り込まないビルドキャッシュ等）と同様に除外すると、復元したゲームが起動しなくなる。再生成可能キャッシュが混ざってもプールの CAS で版間 dedup が効く。
  - **容量**: 中身が同じファイルは（別ゲーム間・版違い間でも）プールに 1 個。実ディスク消費 = ユニークな中身の合計（実測 5.73→2.1GB）。世代を増やしても新規の固有ファイルしか増えない。**どんな単純合計でも実サイズが出る**（ハードリンクの二重カウント問題が無い）。
  - **retention / GC**: **auto の古い manifest のみ削除**（**round9: 保持世代数は DB バックアップと共通の `backup_retention_count` に統一**＝1 バックアップ = `.db` + アセット manifest をペア保持/削除し、復元時のペア整合を保つ。既定 30、manual 温存・#235 同様）。その後 **mark-sweep GC**＝残る全 manifest が参照する hash 集合を作り pool 内の未参照を削除（**直近 1 時間以内更新の pool ファイルは grace で残す**＝並行/直近書込のレース回避）。
  - **運用注意**: manual アセット控えは **retention も GC も対象外＝無期限・無制限蓄積**（DB の manual バックアップと同方針だが、アセットは GB 級で「games/ の中身が版改訂で変わるたびに新 blob が永久に積まれる」性質がサイズ的に桁違い）。本番でプールが想定外に膨らんだ場合は manual 控えの蓄積を疑う。将来 manual にも容量/世代上限の GC を検討する余地あり（PR2/PR3）。
  - **復元 / guide 整合性チェック拡張は後続 PR**（PR3 = 復元 2 モード「DB のみ」/「DB+assets 一式」（Manager が manifest→pool→games/ へ書き戻す）、PR2 = `RestoreReconciliationService` を `intro_slides.image_path`/`guide/` にも拡張）。本 PR1 は取得 + retention/GC + 設定 + UI 可視化のみ。
...
- **言語/環境**: C# / .NET。runtime は Companion ごとに個別判断（#258、Manager は net10 へ移行済）: Updater=net48（常在ランタイムで自己更新の最後の砦、net10 化しない）、LauncherAgent=net10（#258 PR4.x で SDK 化 + net10 + self-contained single-file 化、配布形態を Manager と揃える。依存ゼロ・純 Win32 P/Invoke のため `net10.0` 非 -windows）。Godot や別 runtime が必要な特殊ケースは個別判断。
- **ビルド管理**: 各 Companion は独立 csproj。`Release.ps1` が runtime に応じて個別にビルドする形（Updater=net48 は msbuild、LauncherAgent=net10 は dotnet publish。Companion ごとに個別判断）。リポジトリルートに `TonePrism.slnx`（#245 で VS 開発用に追加。新 XML 形式＝git 差分が綺麗・VS18/net10 SDK が対応。Manager / Manager.Tests / LauncherAgent を収録。net48 classic の Updater は未収録）。**`Release.ps1` は solution を使わず各 csproj を個別 build する**（solution は VS の WPF 開発専用＝NuGet 復元が「プロジェクトを読めない」エラーになる folder/単体 csproj 起動を避けるため）。
...
- **言語/環境**: C# / .NET Framework 4.8（#258: 自己更新の最後の砦＝Windows 同梱の常在ランタイムで「起動が絶対こけない」を優先。Manager の net10 化後もここは 4.8 維持）
...
2. **Manager** を `dotnet publish`（net10、self-contained single-file、#258 PR4）でビルド（exe 1 個に集約）、**各 Companion**（Updater / LauncherAgent 等、§2.4）は `msbuild /p:Configuration=Release` で個別ビルド（`bin/Release/...`。net48 系は `msbuild` 直叩き、移行期の mixed）
...
- [ ] **Launcher 版数の Manager 側読み取り (#281/#283)**: SoT は `Launcher/project.godot` の `[application] config/version="X.Y.Z"` 1 箇所 (version.gd は数字を持たない薄いアクセサ)。**Manager UI「アップデート」タブの版数読み取りは #283 で 2 経路**: **(prod)** `<install>/Launcher/TonePrism_Launcher.exe` の FileVersionInfo を読む (= export_presets.cfg `application/file_version` を `Set-ExportPresetVersions` が SoT から stamp → Godot/rcedit が exe リソースに焼いた派生値、Updater 読み取りと同方式)。**(dev = exe 不在)** `<repo>/Launcher/project.godot` の `config/version` を `Manager/Services/VersionInventory.cs` の `ConfigVersionRegex` で直 parse する fallback。**project.godot の release zip 同梱は #283 で廃止** (prod は exe から読むため不要、`Release.ps1 $filesTemplates` / `$script:BundleManifestFiles` から除去)。**`config/version` の行 format を変える場合は `ConfigVersionRegex` (dev fallback) + Release.ps1 `Assert-LauncherVersion` の同一 regex を同期更新する** (崩すと dev で「Launcher: 不明」)。なお prod の exe FileVersion は多段 stamp パイプライン（config/version → `Set-ExportPresetVersions` → export_presets.cfg → rcedit → exe）の派生値のため、stamp 不発で silent に古い/欠落版数を表示しうる。これを **`Release.ps1 Assert-ExportedLauncherVersion`（`Build-Launcher` 直後・upload 前）が exe FileVersion ≠ SoT を hard fail で公開前に止める**（#283）。リリース時の成果物 版数目視は backstop
- [ ] **コンポーネント版数の公開前ゲート（exe ↔ SoT、#258 PR4/PR4.x）**: コンポーネントの **版数が stamp パイプライン（Godot rcedit / net10 apphost）経由で exe に焼かれる** なら、`Assert-<Name>Version`（`Build-<Name>` 直後・upload 前に **exe FileVersion ≠ SoT を hard fail**）を **必須**とする。理由: stamp の一手抜けで silent に嘘版数を焼く事故（#283）を公開前に止めるため。**判定軸は「stamp パイプラインの有無」であり、Manager「アップデート」タブでの表示の有無ではない**（#258 PR4.x レビュー指摘で修正）。表示は誤版数が*ユーザーに見える*かを左右するだけで、「誤版数を焼いた exe を zip 同梱する」という artifact 整合性の害は表示と無関係に起きる＝stamp 経路を持つなら表示前でも defense-in-depth でゲートする。**除外**: net48 等で版数が**直接コンパイルで exe に入る**コンポーネント（Updater 等）は exe FileVersion が SoT と自明に一致するため不要（= net48 時代の Manager にゲートが要らなかったのと同根）。**現状（全て実装済）**: Launcher=`Assert-ExportedLauncherVersion`（rcedit 派生）/ Manager=`Assert-PublishedManagerVersion`（net10 apphost 派生、#258 PR4）/ LauncherAgent=`Assert-PublishedLauncherAgentVersion`（net10 apphost 派生、#258 PR4.x）。**将来**: Monitor が net10/Godot で stamp 経由なら同様に必須（net48 直接コンパイルなら除外）。**相補ゲート（CHANGELOG ↔ SoT、#359）**: 本ゲートは「exe が嘘版数を焼かないか」を見るが、「CHANGELOG の各コンポーネント版数見出しが SoT と一致するか（= bump し忘れ）」は別軸で **net48 含む全部品に効く**（exe ゲートが無力な Updater の bump 忘れもこちらで捕捉）。SoT の向きは Bundle と逆（Bundle は CHANGELOG が SoT、コンポーネントは AssemblyInfo / project.godot が SoT で CHANGELOG は派生記録）。「1 PR 1 bump」規約の機械化として #359 で追加予定
...
- **`manager_sessions` table** (DB v13、§7.3 参照): 各 PC で稼働中の Manager process が `pc_name = Environment.MachineName` で 1 row 占有する SoT。heartbeat 周期で `last_heartbeat_at_unix_ms` を update、起動時に abandoned row (1 日超) を自動 cleanup (#271、検出 stale 閾値は 60 秒)。
...
- **heartbeat UPSERT 戦略** (round 3 H-2 で UPDATE → UPSERT 変更): heartbeat は `UPDATE WHERE pc_name = ...` ではなく `INSERT OR REPLACE` (= 5 field 全部 set) で実行する。理由: 自 row が物理 DELETE された case で、その後の UPDATE は row 不在で silent no-op となり **自 PC が他 PC から永久不可視化** する silent failure があった。UPSERT で row 不在時も自動 reanimate。(#271) なお「別 PC 起動の `DeleteStaleSessions` が 30 秒遅延した自 row を消す」という旧トリガは、cleanup を 1 日 abandoned 閾値に変えたため通常は発生しない (UPSERT reanimate は手動削除等への safety net として維持)。
- **stale timeout (検出)**: **60 秒** (heartbeat 間隔 × 6、#271 で 30→60)。`DetectOtherActiveSessions` は `last_heartbeat_at_unix_ms >= (now - 60000)` の他 PC row のみ active として返す。**理由**: stale 判定は「読み手 now − 書き手自己申告 heartbeat」で PC 間 clock drift を被る (#269 Launcher と同根)。DB ベースで per-row mtime が無く `max(json,mtime)` は使えないため、閾値マージンで読み手側 skew を吸収するのが SMB 非依存でできる主防御。サーバ時刻基準化は SMB 構成確定後の別案 (#271)。
- **起動時 cleanup (abandoned)**: 検出 stale 閾値とは**別**に、起動時 cleanup は `DELETE ... < (now − 1 日)` の **abandoned 閾値**のみを使う (#271)。stale 閾値 (60秒) で削除すると clock skew で **生存中の遠隔 Manager row を消し → 検出から外れ → 両者 write → DB 破損** に倒れるため。自 crash 残骸は次回起動の UPSERT (pc_name PK 上書き) で回収、検出は query 時 60 秒閾値で stale を除外するため放置 row が残っても誤検出しない。table は pc_name PK で 1 PC 1 row のため緩い cleanup でも肥大しない。
...
- **dialog 文言**: 技術用語 (「データ破損」「競合」) を避け、部員が「何が起きるか」を想像できる具体表現 (「データが破損したり消えたりする恐れ」「別の Manager / Launcher を閉じてから」) で統一。**(#251) 「他 PC」前提の表現は撤回** — 同一 PC 上の Launcher も検出対象 (§3.8.7.6) で検出 list に自 PC が並ぶ case があるため、他 PC / 同一 PC どちらでも読める「別の Manager / Launcher」の汎用文にする。
...
- **abandoned row 自動 cleanup (#271)**: `Initialize` 起動時に `last_heartbeat_at_unix_ms < (now − 1 日)` の **明らかに放置された** row のみ DELETE で回収。**旧仕様 (30 秒 stale で DELETE) は clock skew で生存中の遠隔 Manager row を消す危険があったため #271 で abandoned 閾値に変更**。Manager crash の self row 残骸は次回起動の UPSERT で上書き回収され、他 PC からの誤検出は検出閾値 (60 秒) で止まる。
- **network 切断時の false positive + 自動復帰** (round 3 H-2 で挙動見直し): local PC で DB 到達不能 → heartbeat upsert 失敗 → 他 PC から見ると検出閾値 (60秒) 超で stale 扱い (= active list に出ない) になる。**UPSERT heartbeat 戦略 (= §3.8.2 参照) により、自 PC の DB 到達が回復した瞬間に次の heartbeat で row を自動 reanimate** するため永久不可視化はしない。(#271 前は「他 PC の新規起動が自 row を 30 秒 stale cleanup で物理 DELETE」する副作用もあったが、cleanup を 1 日 abandoned 化したため通常は起きない。) network blip 中の最大 60 秒間は他 PC から「自分が唯一」と誤判定される可能性が残るが、blip 回復で自動復帰する tolerance 設計。
...
- **ProcessingDialog Cancel / SQLite catch / general Exception catch** (= file copy 開始後の path): `CopyGameFolder` 内で `Directory.CreateDirectory(gameBaseFolder)` により親 gameFolder が新規作成された後の rollback。**(#234 追加精査で対称化)** 旧実装は version subfolder のみ削除 + parent gameFolder を retain していたため、空の `games/{id}/` が残り次回 OK 押下で同 gameId なら `existingFolder check` (= #120) が誤 trigger していた。本実装で `baseGameFolderCreated` flag (= 上記「将来 `parentCreatedThisCall` flag 等で対称化」の実体) を導入し、`CleanupCopiedFoldersOnRollback` が **「この追加操作で親を新規作成した」かつ「親が空」** の両条件を満たすときだけ親も削除する。他 version 共存ケース (= 親が非空) や、警告未提示で race により後から出現した親 (= `baseGameFolderCreated=false`) では親に一切触らない (二重防御で誤削除を排除)。

**#120「既存フォルダあり」は wipe & recreate 方針** (#234 追加精査で設計変更): 追加 OK 時に `games/{gameId}/` が既存だった場合の方針を、旧「retain したまま続行」から **「OK = フォルダを中身ごと削除して新規作成」** に一本化。旧方針には (a) 既存フォルダ内に追加バージョンと同名の版フォルダがあると `CopyGameFolder` の「既に存在します」throw → rollback が**既存版フォルダを巻き込み削除**する footgun、(b) 警告文「※ 古いフォルダの中身は自動削除されません」と実装の矛盾、の 2 点があった。新方針では `wipeExistingOnCopy` gate (= #120 警告を出して OK した時のみ true) を立て、`CopyGameFolder` が worker thread 内で親フォルダを丸ごと削除 → 再作成する (UI freeze 回避、削除失敗時は「Launcher 等が使用中の可能性」と明示して中断 = 半削除フォルダへの上書きコピーを防止)。これにより version 衝突 footgun を構造的に排除し、警告文言 (OK = 全削除して作り直す) と実装を一致させる。「大事なデータは警告時点で退避」前提はゲーム削除フローと同思想。なお ⑪ の空フォルダ残り解消により本警告の発火頻度自体が低い。

#### 3.8.6 Launcher session tracking (PR3b で実装、Launcher v0.5.18 / Manager v0.11.0 以降)

本機構の Manager 側 (= `manager_sessions` table) は **Manager 間** の同時起動検出専用。**Manager と Launcher の競合** (= Manager 編集中に Launcher が SQLite read で file lock 競合する path) は §3.8.7 の Launcher session tracking で別途検出する。**設計原則**: SPEC §6.5 と §3.x 序論で「Launcher は SQLite に直接書き込まず、プレイ記録・アンケートを JSON ファイルとして `responses/` 配下に出力する」と明示されているため (#297 で JSON 直読み化、Manager への SQLite 取り込みは廃止)、Launcher session tracking は `manager_sessions` table と非対称の path (= JSON drop folder + Manager polling) で実装する。詳細仕様は §3.8.7 参照。
...
- **共有 disk format**: `<base>/responses/launcher_sessions/<pc_name>.json` (heartbeat 専用 sub-folder、§6.5 の data drop subfolder (`play_records/` / `surveys/`) とは別カテゴリの「IPC 用 subfolder」として明文化。#297 で旧 3-state 取り込み pattern は廃止)
...
- `launcher_version`: string (= `Version.get_version_number()`、SemVer 数値部 `X.Y.Z` のみ、leading `v` なし。値の SoT は project.godot `config/version`、version.gd はそれを読む薄いアクセサ #281)

##### 3.8.7.3 動作仕様

- **heartbeat 周期**: 10 秒 (= `manager_sessions` と統一、SMB 負荷許容 = 50 PC × 6 query/min = 5 query/sec)
- **stale timeout**: 60 秒 (= heartbeat × 6、#269 で 30→60 に緩和)。**理由**: stale 判定は「読み手 PC の now − 検出 timestamp」で PC 間の clock drift を被るため、想定 skew を吸収するマージンを取る。広げる方向は安全側 (= 死んだ session が長めに active 表示 = 過剰警告) でデータ事故方向には倒れない。⚠️ `manager_sessions` (Manager-Manager 検出) は DB ベースで per-row mtime が無く `max(json,mtime)` は適用できないが、**#271 で検出閾値を 60 秒に統一 + 起動時 cleanup を 1 日 abandoned 化**して同型の drift と破壊的 DELETE を是正済 (§3.8.2 参照)。サーバ時刻基準化は SMB 構成確定後の残課題 (#271)
...
- **stale 判定 baseline** (#269 で clock drift 耐性化): primary path は `max(JSON last_heartbeat_at_unix_ms, file mtime)` の **新しい方** で判定する。json は書込側 Launcher の自己申告時計 (PC 間 clock drift を被る)、mtime は SMB サーバの単一時計 (directory cache ~10 秒, §6.5 を被る) で、**互いの弱点を補完** (どちらか fresh なら active)。これにより「書き手 PC の時計が遅れていて json が古く見える」case を mtime が救い、「mtime の cache 遅延」を json が救う。JSON parse 失敗 / field 欠落時は file mtime 単独の fallback。**防御の分担** (#270 review #3): `max` が無効化するのは**書き手側**の時計ズレで、**読み手自身**のズレ (読み手 now が真値より進む) は json/mtime 双方が古く見え `max` では救えず、60 秒閾値 + NTP が吸収する。**残留リスク**: ①読み手 PC 自身の時計が大幅に進む単機ケース (両者とも古く見え過小警告)、NTP 同期 + 60 秒閾値で許容。②`max` が mtime を採るため、**死んだ Launcher の残骸 file の mtime を外部プロセス (バックアップ / 同期 / 再 stamp) が touch すると json が古くても false-active が続きうる** (過剰警告=安全側、次回同 PC の Launcher 起動で同名 file 上書き時に解消、AV の read は mtime 不変のため通常無関係。#270 review #2)
- **stale 判定 baseline の前提と実機検証 (F-1、#269 review #1)**: 上記 mitigation は **file mtime が SMB サーバの時計で刻まれる構成**に依存する。Launcher 書込側は timestamp を明示設定しない (確認済: `session_heartbeat.gd` は `store_string` → `rename_absolute` のみ) ため default SMB では前提成立だが、サーバ構成によっては書き手クライアント時計を反映しうる。**本番投入前に実機 SMB で要確認**: PC-A の OS 時計を 2 分進めて Launcher 起動 → 別 PC から `(Get-Item <pc>.json).LastWriteTimeUtc` が「サーバ時計」か「進めた PC-A 時計」かを確認。後者なら drift 防御は無効化 (ただし `max` は fresh 方向のみ = 安全側で過小警告の新規回帰は無い)。前提が崩れる構成が判明したら代替 (サーバ時刻を別経路取得 / heartbeat 往復) を別 issue で検討。**F-4**: 運用環境のバックアップ / 同期ツールが `responses/launcher_sessions/` 配下の mtime を touch するか (= 上記残留リスク② を踏むか) も併せて実機確認する
- **Launcher 終了時 cleanup**: `NOTIFICATION_PREDELETE` で self JSON 削除 (clean shutdown 即時反映)。削除失敗 / crash 時は Manager 側 60 秒 stale fallback で fail-safe
...
- **dialog title (Startup context)**: `【危険】別の Manager / Launcher が稼働中です` (= Manager 主眼から汎用化、#251 で「他 PC」前提も撤回し同一 PC 検出に整合)
- **dialog title (EditOperation context)**: `【危険】別の Manager / Launcher が稼働中です` (#251 で Startup と統一、同一 PC Launcher 検出で「他 PC で誰かが作業中」が噛み合わない case を解消)
...
- **プレイ記録データ**: SQLite には**格納しない**。Launcher が 1 件 1 ファイルの JSON として `responses/play_records/` に出力し、Launcher 自身がメモリ集計して人気順・最近プレイ等に使う（#297、DB v23 で `play_records` テーブルは廃止）
- **アンケートデータ**: SQLite には**格納しない**。Launcher が 1 件 1 ファイルの JSON として `responses/surveys/` に出力する（#297、DB v23 で `surveys` / `launcher_surveys` テーブルは廃止）
...
- **読み取り**: ゲーム一覧、設定の読み込み（SQLite に直接アクセス）。加えてプレイ記録・アンケートは `responses/` フォルダの JSON を直読みして**メモリ上で集計**する（#297、SQLite を経由しない）
- **書き込み**: **SQLite に直接書き込まない**。プレイ記録・アンケート結果は JSON ファイルとして `responses/` フォルダに出力する（#297 / §6.5 参照）。SQLite へ取り込む経路は廃止

#### 管理ソフトのデータアクセス

- **読み取り**: 全てのデータの読み込み
- **書き込み**: ゲーム情報の追加・編集・削除、設定の変更。**プレイ記録・アンケートの取り込みは行わない**（#297 で取り込み経路を廃止。これらは Launcher が JSON を直読み集計するため Manager は関与しない）
...
- **DELETE モード + busy_timeout による同時アクセス対応** (#103):
  ...
  - 書き込み中は他プロセスからの読み取りもブロックされるが、Manager は時々の操作・Launcher は実質 Read-Only（アンケート/プレイ記録は SQLite を経由せず JSON 直読み / §6.5 後述）のため運用上問題ない
...
#### Launcher 書き込みの JSON 直読み方式（#297・SQLite 取り込み廃止）

ネットワーク共有上で複数 Launcher PC が同時に SQLite へ書き込むと、ロック競合・データ破損のリスクがある。これを根本的に避けるため、Launcher は SQLite に直接書き込まず、プレイ記録・アンケートを JSON ファイルとして `responses/` 配下に出力する。

**設計変遷（#297）**: 当初は「Launcher が JSON を drop → **Manager が SQLite へ取り込む**（drop-folder 3-state / 2-phase）」設計だったが、(1) 取り込んだ瞬間にデータが固定され**常に最新を表示できない**（取り込みタイミング依存の陳腐化）、(2) `play_records` / `surveys` のスキーマ drift を Manager が抱え続ける、(3) Launcher が読む popularity に Manager 起動が必要、という構造的弱点があった。`play_records` / `surveys` / `launcher_surveys` は **定義のみで実装ゼロ**（取り込み INSERT も Launcher 書込も未実装、本番データ未蓄積）だったため、**撤去が最安**の段階で **JSON 直読み + Launcher メモリ集計** へピボットした（DB v23 で 3 テーブルを DROP）。これにより常に新鮮・スキーマ drift 解消・retention をフォルダ単位に単純化できる。
...
- **読み取り・集計（Launcher 側、Manager 取り込みは廃止）**:
  - Launcher が `responses/<category>/` 配下の JSON を**直読みしてメモリ上で集計**する（人気順・最近プレイ・★平均等）。SQLite を経由しない。
  - 集計の冪等性は uuid 単位で担保（複数 PC が書いたファイルは物理的に別＝二重カウントなし）。
  - 起動時に会期フォルダを seed 集計し、稼働中は当日フォルダを一定周期（数十秒）で再スキャンして反映（SMB のディレクトリ一覧キャッシュ ~10 秒を考慮し、リアルタイム性は要求しない）。詳細な集計サービス（`play_stats_service` 等）は #297 PR2 で実装。
  - **Manager 側に取り込み経路は存在しない**（旧 drop-folder 3-state / 2-phase / `RunAfterOperation` 連携・`source_uuid UNIQUE` + `INSERT OR IGNORE` 等はすべて廃止）。Manager は `responses/` の data drop を読まない。

- **IPC 用 subfolder / 直下 file は存続（#297 の撤去対象外）**:

  `responses/` 直下 vs subfolder の責務分離は維持する。撤去したのは **data drop の SQLite 取り込み経路**だけで、Manager ↔ Launcher の IPC 系 file は引き続き使う:
  ...
  **`responses/` 直下 vs subfolder の責務分離**:
  - **直下**: Manager ↔ Launcher の **system / IPC 用 file** (= `launcher_logs_root.json` 等の single-file write、追跡される file 数は固定)。
  - **subfolder (category 別)**: **data drop 用** (= プレイ記録 / アンケート、Launcher が大量に書き出し Launcher 自身が直読み集計、追跡される file 数は使用量に比例)。内部は **日付フォルダ** (`YYYY-MM-DD/`)。
  - **IPC 用 subfolder (`responses/launcher_sessions/`)**: heartbeat 専用（1 PC 1 file の content update、file は増えない）。data drop subfolder（`play_records/` / `surveys/`）とは別カテゴリで、命名は `<pc_name>.json`。詳細は §3.8.7 参照。

- **想定規模（年間）**:
  - アンケート: ~100 件（任意回答）
  - プレイ記録: 数千件（毎プレイ自動記録）
  - 日付フォルダ単位で蓄積されるため、retention は古い日付フォルダの削除で単純に行える（Manager の取り込み運用は不要になった）。

- **任意フィールドの NULL 扱い**:
  - `survey.comment` 等の任意入力フィールドは JSON 上で省略 or `null`（書かなかった）と空文字 `""`（空欄送信）を区別できる形で出力する。
  - 集計時は用途に応じて「コメント有り」を `comment != null && comment != ""` などで判定する。
...
- **データベースファイル**: `toneprism.db`（アプリケーションと同じフォルダに配置）
  ...
  - （プレイ記録・アンケートは SQLite に持たず `responses/` の JSON 直読み集計に移行 — #297、DB v23 で `play_records` / `surveys` / `launcher_surveys` を廃止）
...
toneprism.db (SQLiteデータベースファイル)
  ...
  # play_records / surveys / launcher_surveys テーブルは DB v23 (#297) で廃止。
  #   プレイ記録・アンケートは responses/{play_records,surveys}/YYYY-MM-DD/ の JSON を
  #   Launcher が直読みしてメモリ集計する方式へ移行（SQLite には持たない、§6.5 / §7.5.3）。

  └─ settingsテーブル (システム設定・KVS形式)
       ├─ key (PRIMARY KEY)
       └─ value

  # backup_logテーブルは DB v19 (Manager v0.17.0) で廃止。バックアップ履歴は backups/ フォルダ走査
  #   (BackupCatalogService) 由来に移行し、DB テーブルとしては持たない。
```
...
- **カラム**:
  ...
  | play_time | INTEGER | CHECK(1-3) | プレイ時間の分類（1=～5分、2=5分～15分、3=15分以上）。**DB v20 (#247) で復元**：`games`テーブル定義時にdifficultyのCHECKは残ったがplay_timeのCHECKが欠落していた drift を、`MigrateV19ToV20`（`games`再構築）で修正しdifficultyと対称化した（NULL許容はdifficultyと同じ） |
...
#### テーブル3: play_records（廃止 — DB v23）

- **状態**: **廃止**。DB スキーマ v23（`MigrateV22ToV23`、Manager v0.27.0 / #297）で `DROP TABLE`。テーブル番号 3 は後続テーブル参照を壊さないよう欠番として残し、`ExpectedSchema`（`SchemaManager`）からも除去（`VerifySchema` 対象外）。
- **経緯**: プレイ記録を SQLite に取り込む設計（drop-folder + Manager INSERT）を廃止し、**Launcher が `responses/play_records/YYYY-MM-DD/` の JSON を直読みしてメモリ集計**する方式へ移行（§6.5 / §7.5.3）。本テーブルは定義のみで実装ゼロ（取り込み INSERT も Launcher 書込も未実装、本番データ未蓄積）だったため、撤去が最安の段階で削除した。`games` を参照する子テーブルだが CASCADE 波及はなく安全（前例 `MigrateV18ToV19` の `backup_log` DROP）。
- **沿革**: SPEC v1.3.1/v1.5.1 でイベントログ方式に定義 → `MigrateV10ToV11` で drift 修正 → **v23 で DROP**。プレイ記録の JSON スキーマは §7.5.3 を参照。

#### テーブル4: surveys（廃止 — DB v23）

- **状態**: **廃止**。DB スキーマ v23（`MigrateV22ToV23`、Manager v0.27.0 / #297）で `DROP TABLE`。テーブル番号 4 は欠番として残し、`ExpectedSchema` からも除去。
- **経緯**: アンケートを SQLite に取り込む設計を廃止し、**Launcher が `responses/surveys/YYYY-MM-DD/` の JSON を直読み**する方式へ移行（§6.5 / §7.5.3）。本テーブル（および `launcher_surveys`）は定義のみで実装ゼロだったため撤去。`game_id` で `games` を参照する子テーブルだが CASCADE 波及なし。
- **沿革**: SPEC v1.5.1 で★評価+コメント形式に定義 → `MigrateV10ToV11` で drift 修正 → **v23 で DROP**。アンケートの JSON スキーマ（`type` / `game_id`（全体は null）/ `rating` / `comment` / `trigger`（`game_end` / `launcher_end`）/ `source_pc` / `created_at`）は §7.5.3 を参照。
...
- **インデックス**:
  - `idx_game_versions_game_id_version`: `UNIQUE (game_id, version)`（DB v15、#234 ②）。同一ゲームに同一バージョン番号の行が 2 行入る silent corruption を防ぐ最後の砦。アプリ層 dup-check（VersionUpForm / EditGameForm / GameSectionPanel）で塞ぎきれない複数 PC 同時操作の「check → write」間 race を DB 制約で確実に弾く。version は raw 文字列比較（意味的正規化 `v1.0.0`/`1.0.0` の同一視はアプリ層の責務）。既存 DB への追加 migration（MigrateV14ToV15）は重複残存時は throw せず skip + 警告し、user_version 据え置きで次回起動時に再試行する（V10→V11 と同じ "data residual → retry" パターン）。

#### テーブル7: game_genres（DB v18 で廃止）

- **状態**: **廃止** — DB v18（`MigrateV17ToV18`）で DROP 済み。ジャンルは正規化テーブルではなく `games.genre` / `game_versions.genre` にカンマ区切り文字列で保持する方式に一本化されており、本テーブルは実装上ずっと未使用だったため削除した。テーブル番号は後続テーブル（8 以降）の参照を壊さないよう欠番として残す。`ExpectedSchema`（`SchemaManager`）からも除去済み。
...
- **カラム**:
  ...
  | section_type | INTEGER | DEFAULT 0 | セクション表示タイプ (0=通常カテゴリ行 / 1=スライドショー / 2=タイルグリッド) |
  | section_source | TEXT | DEFAULT 'manual' | セクションのデータソース |
  | display_order | INTEGER | DEFAULT 0 | 表示順序 |
  | max_display_count | INTEGER | DEFAULT 5 | 最大表示件数 (`<=0` で上限なし=該当ゲームを全件表示、Launcher 共通解釈) |
  | is_visible | INTEGER | DEFAULT 1 | 表示/非表示 |

  - **(#211/#212/#291) Manager 編集 UI の条件付き規則**: (a) **タイプ↔ソース**: スライドショー(1)/タイルグリッド(2) は `section_source` が **手動 ＋ ランキング系 (= `IsRankingSource`: popular/recently_played/random)** のみ。厳選枠は「少数を大きく見せる」枠なので**表示数を制御できるソース**だけが合う (手動=割当で枚数決定 / ランキング系=`max_display_count` で TOP N 指定。背景画像/表示テキストはゲーム自身にフォールバックするので自動でも崩れない)。フィルター系 (genre/players_*/difficulty/play_time/online/controller/recent/release_year) は条件一致を「全件」表示で枚数を絞れず厳選にならないため不可。**= 「厳選枠で許可するソース集合」が「`max_display_count` が有効なソース集合 ∪ 手動」と一致する** (1 つの原則で説明可)。**UI**: ソースのドロップダウンは**タイプに応じて選択肢を構築**する (厳選枠では `{手動, 人気ランキング, 最近プレイ, ランダム}` の 4 つだけを出す。許可外はそもそも一覧に出さない＝選べない)。combo の表示 index と canonical ソース ID(0-12) がズレるため `_sourceMap`(表示index→canonical) + `SelectedSource` で解決。タイプを厳選枠に変えた/legacy データを開いた時点で現在ソースが許可外なら `manual` へ落として案内。通常カテゴリ行(0) は手動＋全自動ソース可。(b) **max_display_count の有効性**: ランキング系ソース (popular=play_count DESC / recently_played=last_played DESC / random) でのみ上限が意味を持つ (TOP5 等の特集枠) ため編集可。手動 (= 割当を全件表示) と条件系 (genre/players_*/difficulty/play_time/online/controller/**`release_year:YYYY`=制作年指定** [#291、指定年の `release_year` を display_order 順]、および **recent=新作** [`release_year=今年` を display_order 順 = 本物のランキングでないため condition 扱い]、= 条件一致を全件表示) では UI をグレーアウトし `max_display_count=0` で保存する。
    - **(#290 review) 値入力 UI**: 難易度 (`difficulty:N`) / プレイ時間 (`play_time:N`) はゲーム編集と同じラベル付きドロップダウン (例: `1 - 易しい`) で指定 (数字直打ちを廃止)。プレイ人数 (`players_min/max:N`) と制作年 (`release_year:YYYY`) は数値入力。
    - **(#211 fix) Launcher 側の `max_display_count<=0` 解釈**: 通常セクション/スライドショー/タイルグリッドのいずれも `<=0` を「上限なし」として扱う。**タイルグリッド** (`StoreBannerBuilder.build_tile_grid_section`) は従来 `mini(max_display_count, 3)` をガード無しで取っており、`0` 保存だと `tile_count=0` で空グリッドになる回帰があったため、`<=0 → 3` (物理上限) にフォールバックするよう修正。**スライドショー** は `max>0` のときスライド枚数を上限し (ランダムで「特集 N 枚」が成立)、生成枚数を container の `slide_count` meta に持たせて `store_browse._switch_slide` の wrap-around が存在しない `Banner_*` に飛ばないようにする。
    - **フィルター系ソースの並び順** (`store_section_repository.gd`): `genre:` / `players_min:` / `players_max:` / `difficulty:` / `play_time:` / `online` / `controller` は `ORDER BY release_year DESC, title ASC`（制作年の新しい順を頭に＋同年内は名前順）。`games` に作成日時列が無く `release_year` が唯一の新しさ指標のため (年単位)。`release_year=NULL` は末尾。単年フィルター (`recent`=今年 / `release_year:YYYY`=制作年指定) は「最新が頭」が無意味なので `display_order ASC, title ASC` を維持。`manual`/`popular`/`recently_played`/`random` は各々の固有順序 (割当順/ランキング/シャッフル)。
...
#### テーブル10: launcher_surveys（廃止 — DB v23）

- **状態**: **廃止**。DB スキーマ v23（`MigrateV22ToV23`、Manager v0.27.0 / #297）で `DROP TABLE`。テーブル番号 10 は欠番として残し、`ExpectedSchema` からも除去。
- **経緯**: 当初はランチャー全体アンケートを `surveys` テーブルへ統合（`trigger='launcher_end'`）して廃止する計画だったが、アンケート自体を SQLite に持たず Launcher が JSON 直読みする方式（#297）へ移行したため、`surveys` ともども撤去した。全体アンケートは `responses/surveys/YYYY-MM-DD/` の JSON（`game_id=null` + `trigger='launcher_end'`）として出力される（§7.5.3）。本テーブルは定義のみで実装ゼロ・本番データ未蓄積だったため移行処理は不要。

#### テーブル11: backup_log（廃止 — DB v19）

- **状態**: **廃止**。Manager v0.17.0 / DB スキーマ v19 (`MigrateV18ToV19`) で `DROP TABLE`。テーブル番号 11 は欠番、`ExpectedSchema` からも除去（`VerifySchema` 対象外）。
- **経緯**: バックアップ履歴を「バックアップ対象である `toneprism.db` の中」に持つ設計が、復元で DB が丸ごと置き換わるたびに履歴とディスク実ファイルのズレを生み、それを埋める reconcile / register コードがバグの温床（失敗した復元が `success` 化する等）になっていた。履歴は `backups/` フォルダ走査（`BackupCatalogService` — 種類 = フォルダ位置 `auto/`・`manual/`・`safety/`、日時・実行 PC = ファイル名 `<種類>_<日時>_<host>.db`、サイズ = `FileInfo`）から導出する方式へ移行した。
- **沿革**: v9 で追加 → v10 で `trigger_type` CHECK に `'safety'` 追加 → v12 で `relative_path` 列追加 → v16 で `'restore'` 追加 → **v19 で DROP**。既存運用 DB は migration で行が破棄されるが、物理バックアップファイルは残り初回走査で履歴に復活する（失われるのは失敗履歴と復元監査行のみ）。

  **バックアップ関連 settings キー**（`settings` テーブルに保存。backup_log 廃止後も不変 — マルチ PC の lease / retention 等で引き続き使用）:

  | キー | デフォルト | 説明 |
  | --- | --- | --- |
  | `last_backup_at` | 0 | 最終バックアップ完了時刻（UNIX秒）。#295 でトリガ gate/lease は廃止。現状 read 経路なし（write-only、将来用に残置。表示は `GetLastSuccess` のファイル走査由来） |
  | `backup_destination_path` | （空文字列） | バックアップ保存先フォルダ。空ならデフォルト（`<DBフォルダ>/backups/`） |
  | `backup_auto_enabled` | true | 変更時の自動バックアップ ON/OFF（#295。"false" のみ無効。旧 `backup_auto_interval_hours` / `_unit` は撤去） |
  | `backup_retention_count` | 30 | 保持する世代数（これを超えた古いバックアップは自動削除） |

#### テーブル12: manager_sessions

- **テーブル名**: `manager_sessions`
- **説明**: Manager の LAN-wide 同時起動検出に使う session tracking テーブル (#179、Manager v0.10.0 / DB スキーマ v13 で追加)。各 PC で稼働中の Manager process が `pc_name = Environment.MachineName` で 1 row 占有、10 秒間隔の heartbeat で `last_heartbeat_at_unix_ms` を update。検出 stale 閾値は 60 秒、起動時 cleanup は 1 日超の abandoned row のみ削除 (#271、clock skew で生存 row を消さないため)。詳細は §3.8 参照。
- **カラム**:
  ...
  | last_heartbeat_at_unix_ms | INTEGER | NOT NULL | 最終 heartbeat 時刻 (UTC Unix epoch ms)。10 秒間隔で update、60 秒以上経過で stale 扱い (#271) |
...
- **テーブル名**: `intro_slides`
- **説明**: 「初回説明」のスライド (#253、DB スキーマ v21 で追加・**v22 で `duration_sec` 廃止**)。スクリーンセーバー → ブラウズ間に**毎回**表示する案内 (展示の説明 / 楽しみ方 / 注意事項 等。来場者は入れ替わるため各人にとって初回)。画像実体は `guide/` フォルダにファイル別管理し、本 table は相対パス (`image_path`) のみ持つ (games のサムネ/背景と同流儀)。他テーブルへの FK は無い独立テーブル。Manager の編集パネル (タブ「初回説明」) で追加/編集/並び替え/画像選択、Launcher は**手動ナビ (全スキップ + 次へ/戻る、自動送りなし)** で表示する。
- **カラム**:

  | カラム名 | データ型 | 制約 | 説明 |
  | --- | --- | --- | --- |
  | slide_id | INTEGER | PRIMARY KEY AUTOINCREMENT | スライド ID |
  | display_order | INTEGER | DEFAULT 0 | 表示順 (昇順) |
  | body_text | TEXT | DEFAULT '' | スライド本文。空可 (= image-only スライド) |
  | image_path | TEXT | (NULL 可) | 画像の相対パス (`guide/` 基準)。NULL 可 (= text-only スライド)。空/空白入力は null に正規化 |
  | is_visible | INTEGER | DEFAULT 1 | 表示 ON/OFF (削除せず一時非表示にできる、store_sections と同 pattern) |

  ※ v21 で追加した `duration_sec` (自動送り秒数) は **v22 (#253 part 2) で廃止** (自動送りをやめ手動ナビに design 変更。CHECK 付き列のため `MigrateV21ToV22` は table recreate で削除)。
...
（旧 `play_records.game_id` / `surveys.game_id` → `games.game_id` のリレーションは DB v23 (#297) でテーブル撤去により消滅。プレイ記録・アンケートは `responses/` の JSON 内 `game_id` 文字列で論理的に games を指すが、DB 制約ではなく Launcher の集計ロジックが解決する。）
...
erDiagram
    ...
    %% play_records / surveys / launcher_surveys テーブルは DB v23 (#297) で廃止。
    %%   プレイ記録・アンケートは responses/{play_records,surveys}/YYYY-MM-DD/ の JSON を
    %%   Launcher が直読みしてメモリ集計する方式へ移行（§6.5 / §7.5.3、SQLite には持たない）。
    ...
    %% 10 秒間隔の heartbeat で last_heartbeat_at_unix_ms を update、起動時に abandoned row
    %% (last_heartbeat < now - 1 日) の abandoned row を自動 cleanup。詳細は §3.8 参照。
```
...
│   │   └── background.mp4
│   └── ...
├── guide/                    # (#253) イントロガイドのスライド画像（intro_slides.image_path が "guide/<file>" で参照、games/ の隣で Manager 管理）
...
`toneprism.db` / `games/` / `guide/` / `backups/` / `responses/` / `logs/` はアップデート時に保護される（§3.7.3）。`guide/` は #253 で追加された Manager 管理アセット（#250 のバックアップ拡張で `games/` と同様に保護対象に含める）。
...
#### 7.5.3 アンケート・プレイ記録 responses フォルダ（JSON 直読み・#297）

- **フォルダ名**: `responses/`
- **配置場所**: プロジェクトルート直下（`toneprism.db` と同じ階層）
- **目的**: Launcher が出力した プレイ記録・アンケートの JSON を**永続保管**する場所。Launcher 自身が直読みしてメモリ集計する（Manager の SQLite 取り込みは廃止 / §6.5 参照）。
- **フォルダ構成**（category 別 → 日付フォルダ）:
...
- **典型的な使い方**（プロジェクトルートから）:
  ...
  .\tools\sqlite3\sqlite3.exe toneprism.db "PRAGMA table_info(games);"
  ```
...
- **DB v23 (#297、Manager v0.27.0)**: `play_records` / `surveys` / `launcher_surveys` を
  `MigrateV22ToV23`（`DROP TABLE IF EXISTS`）で撤去。これらは `games` を参照する子テーブルだが
  CASCADE 波及はなく安全（前例 `MigrateV18ToV19` の `backup_log` DROP）。撤去に伴い `CreateTables`
  から CREATE 呼び出しを削除、`ExpectedSchema` から 3 エントリ削除、stale な
  `Models/Survey.cs` / `Models/PlayRecord.cs` も削除。`CreateTables()` 編集時は
  必ず `MigrateVxToVy` を書く（§7.6.1）規約どおり migration を同時実装し、撤去回帰テスト
  （v22 DB に 3 テーブルを空で作り `InitializeDatabase` 後に `sqlite_master` 0 件・
  `user_version=23`・`integrity_check ok` を assert）で固定した。プレイ記録・アンケートは
  以後 `responses/` の JSON 直読み集計へ移行（§6.5 / §7.5.3）。
...
- **技術的達成項目**:
  ...
  - データベーステーブルの作成（games, developers, settings ほか。※ プレイ記録・アンケートは DB v23/#297 で SQLite を廃し `responses/` の JSON 直読みへ移行）
...
- **管理場所**: `Launcher/project.godot` の `[application] config/version`（版数の SoT、#281）、Gitタグ（`launcher-vX.Y.Z`）。`Launcher/version.gd` はその値を読むだけの薄いアクセサ
...
詳細なバージョン管理（bump 判断・CHANGELOG/SPEC の更新規約）については、`AGENTS.md` の「Release and Versioning」「CHANGELOG Section Roles」を参照してください。
...
| 2026-06-11 | 1.10.62 | **(#258 PR4、Manager v0.27.9 / Release Tooling v0.1.27)** Manager の net48→net10 移行（#258 PR1-4）に伴う配布・ビルド記述の同期。**配布形態 = self-contained single-file**（net10 ランタイムを exe に同梱し「解凍→すぐ動く・オフライン・管理者不要」を net10 でも維持。Manager = `TonePrism_Manager.exe` 1 個に集約＝旧 `.exe.config`/`System.Data.SQLite.dll`/`x64,x86\SQLite.Interop.dll` は exe 内包、native は初回 self-extract、実機 smoke で DB round-trip 確認済）。**ビルド手順**（§3.7.x）を「Manager = `dotnet publish`（net10）／ Companion = `msbuild`（net48）」に更新（旧「.NET FW 4.8 系は publish 不可・msbuild 直叩き」は Manager net10 化で改定）。**§2.4 Companions・§3.7.4 Updater の「.NET Framework 4.8（Manager と同じ）」を訂正**（Manager は net10 へ移行済＝もう「同じ」でない。Updater=net48 を自己更新の常在性優先で維持、LauncherAgent は現状 net48・net10 化は PR4.x 予定＝Manager と揃える）。Release.ps1 は `Build-Manager` を dotnet publish 化 + expected-files（`$script:BundleManifestFiles` / `UpdateDownloader.ValidateStagingLegacy`）を exe 1 個に刷新 + dead な `Resolve-Nuget` 一式撤去。`Assembly.Location`→`Environment.ProcessPath`（single-file 対応）。検証: net10 ビルド 0 警告 0 エラー・xUnit 199 緑・single-file 実起動 smoke（DB/SQLite native 成立）。本番リリース不能窓を避けるため PR3+PR4 を束ねてマージ（main は net48 で release 可能を維持）。**（round3）§3.7.8 に「コンポーネント版数の公開前ゲート（exe↔SoT）」の条件ベースルールを明文化**（(a) Manager 版数タブに exe FileVersionInfo 表示 ∧ (b) stamp パイプライン経由＝Godot rcedit / net10 apphost → `Assert-<Name>Version` 必須、net48 直接コンパイルは除外。現状 Launcher/Manager 該当、LauncherAgent #310 / Monitor は net10 化＋表示で将来必須化）。相補の CHANGELOG↔SoT 版数ゲートは #359 で別管理（1 PR 1 bump 機械化）。 | Kenshiro Kuroga \& Claude |
| 2026-06-07 | 1.10.61 | **(#297 PR1、Manager v0.27.0 / Launcher v0.11.0 / DB v23)** プレイ記録・アンケートの **SQLite 取り込み廃止 → JSON 直読み化**へピボット（本番DB作成前の最後のスキーマ変更）。**スキーマ撤去**: `MigrateV22ToV23`（`DROP TABLE IF EXISTS play_records / surveys / launcher_surveys`、CASCADE 波及なし＝`games` 子テーブルだが安全、前例 `MigrateV18ToV19` の backup_log DROP）+ `CurrentDbVersion` 22→23。`CreateTables` の 3 テーブル CREATE 呼び出し削除、`ExpectedSchema` から 3 エントリ削除、stale な `Models/Survey.cs` / `Models/PlayRecord.cs` も削除。`MigrateV10ToV11`（旧 drift 修正）は chain 連続性のため no-op 化して呼び出しのみ残す。**設計判断**: 旧「Launcher が JSON drop → Manager が SQLite INSERT 取り込み（drop-folder 3-state / 2-phase）」は (1) 取り込み時点でデータ固定＝常に最新を出せない、(2) スキーマ drift を Manager が抱え続ける、(3) popularity 表示に Manager 起動が必要、という弱点。3 テーブルは定義のみ実装ゼロ・本番データ未蓄積だったため撤去が最安の段階で、**Launcher が `responses/{play_records,surveys}/YYYY-MM-DD/<unix_ts>-<uuid>.json` を直読みしてメモリ集計**する方式へ移行（常に新鮮・drift 解消・retention はフォルダ単位）。**SPEC 改定**: §6.2/§6.4/§6.5（drop-folder 3-state/2-phase → JSON 直読み + Launcher メモリ集計へ全面改定、IPC 用 subfolder `launcher_sessions` / 直下 `launcher_logs_root.json` は存続）、§7.1/§7.2/§7.3（テーブル 3/4/10 を「廃止 — DB v23」表記＝番号欠番、game_genres/backup_log 廃止記法を踏襲）、§7.4（ER 図から play_records/surveys エンティティ・FK 線削除）、§7.5.3（3-state → 日付フォルダ + JSON スキーマ確定）、§7.6.4（過去事例追記）、機能10-12（Manager 取り込み → Launcher 出力 + 自身で集計）。**Launcher 非参照化**（v23 で play_records 消滅対応）: `store_section_repository.gd` の popular を `is_visible=1` 安定順（display_order/title、順位は仮）、recently_played を空配列（`WHERE 1=0`、空セクションは `get_store_sections` が自動非表示＝UI 非破壊）、`database_manager.gd` `CURRENT_DB_VERSION` 22→23。実集計は PR2 で差し替え。**潜在ランタイムバグ修正**: `GameRepository.UpdateGameId` の子テーブル更新が `play_records`/`surveys`/`launcher_surveys` を UPDATE していた（コンパイル非検出の「no such table」で v23 後に game_id rename が失敗する穴）を除去。**レビュー対応**: v0 fast-path（versioning 導入前 DB の retrofit 経路）が `MigrateV22ToV23` を呼ばず 3 テーブルを残したまま v23 を刻む穴を是正＝no-op 化した `MigrateV10ToV11` 呼び出し＋stale コメント＋dead branch を撤去し、`MigrateV22ToV23`（`DROP TABLE IF EXISTS`＝冪等）を stamp 直前に明示適用（v0 path 回帰テスト `V0FastPath_DropsEventTables_AndReachesV23` 追加）。**検証**: build 緑 + テスト 189 件（撤去回帰 `V22ToV23_DropsEventTables`＝v22 DB に 3 テーブルを空で作り `InitializeDatabase` 後に `sqlite_master` 0 件・`user_version=23`・`integrity_check ok` を assert、`UpdateGameId` 回帰、v0-path 回帰、`SchemaMigrationTests` の play_records COUNT assert 撤去、`IntroSlideTests` の固定版数を `GetTargetDatabaseVersion()` 動的化、sqlite3.exe で v22→v23 実 DB round-trip）。中間 PR ゆえ `(#297)` 文末参照（closing keyword なし）、プレイ記録書込＋集計（PR2 / `Closes #34`）・アンケート UI（PR3 / `Closes #35` `Closes #297`）は後続。実機での popular/recently_played 表示・「DB が新しい」警告無しは pre-release で目視。 | Kenshiro Kuroga \& Claude |
| 2026-06-06 | 1.10.60 | **(#250 PR3b、Manager v0.26.0)** PR3a のアセット復元エンジン `AssetRestoreService` を**復元 UI へ配線**し、復元を **DB＋ゲームファイル(games/+guide/) の一貫時点復元**に拡張（控えのある世代は DB と games/guide を一緒に戻す／無い世代は DBのみ。#250 の核心「DB とアセットを一貫した時点に復元」を実機操作で完結）。§機能12 リストア手順に「アセット復元（reconcile-in-place）」を追記想定。**安全方式は reconcile-in-place（ユーザー判断、当初計画の rename-retreat は SMB+容量都合で不採用）**＝変わった分だけ pool から上書き・余剰だけ削除、取り消しは別世代の再復元（全状態は自動バックアップ済・DB は safety_*.db）。**`.db`↔`.manifest` 時刻ペアリング**（新 public `AssetSnapshotService.FindSnapshotForBackup`）＝T(= 復元する .db の作成時刻)**以下で最大時刻**の manifest を対にする（**replace-in-session** で .db>manifest になる事象を直前フル世代へ正しく解決）。host 一致優先（同秒 tie 分け）→無ければ全体最新、tie-break ファイル名降順、不正ヘッダ除外、SMB 不達等は null（DBのみ復元へフォールバック）、auto+manual 横断。**時刻ペアリング (FindSnapshotForBackup) の対象は auto/manual のみ**（DB と同一 timestamp の manifest を co-create するため）。unknown（旧フラット形式）は対の控えを持たず常に DBのみ復元。safety（復元 undo 用の退避 db）は round3 で専用 undo ペアを持つため別経路（後述 FindSafetyManifestForBackup の完全一致）で解決（review #1: safety を時刻 fallback で近接世代へ誤ペアし reconcile で誤削除しうる穴は、auto/manual の時刻探索から safety を除外＋safety は完全一致のみ、で構造的に回避）。**`RestoreService` は無改修**＝アセット復元は `btnRestore_Click` ワーカーで DB 置換成功**後**に実行、**アセット phase は非キャンセル**（`CancellationToken.None`＝DB 置換済で後戻り不可、途中キャンセルの「DB は変更されていません」誤報告を回避、reconcile は冪等）。`RestoreConfirmForm` は控えのある世代で「games/guide もこの時点に戻す＝以後に追加・変更したファイルは削除」を赤字明示＋控え日時/PC/件数表示（round2 でチェックボックス廃止＝一貫時点復元に一本化）、`RestoreReportForm` にアセット節（copied/skipped/deleted・MissingBlob・DeletionSuppressed、アセット問題で reconcile クリーンでも headline 警告色に格上げ）。配線: `DatabaseManager` に `AssetRestoreService` accessor 追加、`EnumerateManifests`/`ReadManifestHeader` を internal 化。schema 変更なし。**#250 はこの PR では閉じない**（PR3c [整合性レポートの半自動復元連携] の要否はマージ後判断）。（review round2 #1）host 優先は**最新秒グループ内の tie 解決に限定**（games/+guide/ は SMB 単一共有ツリーゆえ時点 T の最良推定は時間的最近の manifest、host 一致でも数世代前を選ばない＝degraded で T 以降の追加ファイル誤削除を防止）。**（review round2 ユーザー判断＋round3 で退避を全面再設計）**チェックボックス廃止で復元＝一貫時点復元に一本化。退避は当初 `RunManualBackup`（DB+assets を手動 BU）を DB 復元前に取っていたが、(i) DB 二重退避（safety_*.db と重複）、(ii) 手動 BU は retention 外で蓄積（既存 safety_*.db は `ApplySafetyRetention` で自動削除されるのに非対称）、(iii) 退避のアセット取得キャンセルが degrade に化けて DB 置換、が判明。本番本格運用前の今に正攻法へ＝退避を「**DB 復元後・reconcile 前に games/guide だけを safety_*.db と同 ts/host のアセット safety 控えとして取る**」に変更（`CreateSnapshot(ts,"safety",…)`、ts は safety_*.db 名から流用）。(i) 二重退避解消、(iii) 退避が DB 復元後で「キャンセル＝不変」の罠が構造的消滅（非キャンセル化、token 直読み撤去）、(ii) 新 `PruneSafetySnapshots`（safety_db と同件数で掃除＋GC）で自動削除。GC が退避控えを誤回収しないよう `EnumerateManifests(includeSafety)` を新設し **GC の参照集合だけ safety を含める**（ペアリング探索は auto/manual のまま）。undo は新 `FindSafetyManifestForBackup`（同 ts/host 完全一致）＝safety_*.db 復元で DB もゲームファイルも当時へ戻る。退避不完全なら DBのみ degrade。**（review round5）**退避＋reconcile は `RestoreService.Restore` の restore-lock 解放後（lock 外）で走るが、自己発火はモーダル操作不能で構造的に起きず（#295 時間トリガ撤去）、別 PC 同時復元は SessionConflict 警告＋DB phase restore-lock で抑止・被害は当該世代限定（multi-PC 厳密化は将来 heartbeat-lease、pre-release 確認）。`ParseSafetyTimestamp` は `RestoreService` の safety 命名と暗黙結合するため純ロジック化＋invariant テストで固定、空 games 退避の undo 過剰約束を `ManifestPath` 有無で gate。検証: build 緑 + テスト 186 件（PR3b 新規 30 ＝ペアリング 10/eligibility 7/safety undo 完全一致 3/GC が safety 控え blob 保護 1/退避控え retention 1/ParseSafetyTimestamp invariant 8、fail-first）。**実機 UI（警告ダイアログ・DB 復元→ゲームファイル退避→reconcile→レポート・退避失敗で DBのみ degrade・DB+assets round-trip・本番 SMB 体感・safety_*.db 復元で DB もゲームも当時へ戻る undo）は pre-release で目視（#250 の実機 UI 初確認）**。 | Kenshiro Kuroga \& Claude |
| 2026-06-04 | 1.10.59 | **(#250 PR3a、Manager v0.25.0)** アセット復元エンジン `AssetRestoreService` を追加（バックアップは PR1/PR2 で取れるが復元は DB のみ＝「取れるが戻せない」を埋める土台）。manifest(relpath→hash) を読み `asset_pool/<hash>` を live games/+guide/ へ書き戻す `CreateSnapshot` の逆操作。**reconcile-in-place**: 一致ファイル(size+mtime)は skip・違う/欠落だけ pool から copy・余剰 live は削除しツリーを manifest と完全一致、コピーを削除より先（pool 不変＝置換前に消さない）、コピー後 mtime を manifest 値に刻む（次回 cache hit/再復元 skip）。**安全側**: relpath を games//guide/ 配下に限定（パストラバーサル拒否）、空 manifest 暴発ガード（allowEmpty）、pool blob 不在は live 保持で per-file 失敗集計（IsPartial）、reparse/.pending-delete-* は削除除外、long-path/SMB 安全。**フォーマット SoT 一本化**: `AssetSnapshotService.TryParseManifestEntryLine`/`PoolPathFor` を internal 化し `LoadHashCache`/`GarbageCollectPool`/`AssetRestoreService` が共用。**UI 未配線**（2モードUI・ペアリング・rename-retreat safety は PR3b）。schema 変更なし。**#250 は閉じない**。**レビュー対応**: (#1) `EnumerateLiveFiles` の reparse 判定が dir のみで reparse ファイルを余剰削除しうる非対称を是正（ファイルにも判定＝snapshot と対称）。(#2) SoT リファクタで GC 参照抽出が strict 化し破損 manifest 行が blob を保護しなくなった退行を是正（GC は保護側に倒し、破損行でも先頭フィールドを hash 候補に。restore/cache は strict）。(#3) `AssetRestoreService` の未使用 SettingsRepository 注入を除去。**レビュー round2**: 「manifest が完全な desired tree でないのに restore が"余剰"として live を削除する」同根のデータ消失 2 点を是正。(#1 High) 部分取得(IsPartial)世代で snapshot 取りこぼし live が削除される穴を、META 行に skippedDir/skippedFile を追記して部分取得を永続化し（旧6フィールドは complete 扱い後方互換）、partial 世代では余剰削除を抑止。(#2 High) 破損 manifest 行の silent skip で対応 live が削除され IsPartial にも乗らない穴を、破損行を failed 計上＋削除フェーズ全体抑止（破損1行でも抑止。GC の安全側ポリシーを restore にも）。抑止は `AssetRestoreResult.DeletionSuppressed` で伝達。(#4) pool dir 名リテラル重複を `PoolDirName` internal 共用に。据え置き: #5(進捗は copy のみ→PR3b)/#6(`preferContentFromDir` は rename-retreat 用予約で保持)。**レビュー round3**: (#1 Low-Med) 旧 6 フィールド META 世代 (PR3a 以前) が無条件 complete に倒れ PR3b 復元で旧取りこぼし live を誤削除しうる穴を是正＝`TryReadMetaSkipped` で「8 フィールドで skipped==0 と明示確認できたときだけ complete」(旧形式/META欠落/判定不能は削除抑止＝不明なら消さない安全側) に統一。(#3) `TryParseManifestEntryLine` に hash 長検証 (SHA-256 64 桁) を追加し不正 hash 行を破損扱い (削除抑止+Substring crash 防止)。(#2) `SafeGetDirs` の例外握り潰しに Warn ログ追加 (SafeGetFiles と対称)。据え置き: #4 (pool root を現バックアップ先から導出＝任意パス manifest 復元の PR3b で co-location 検証)/#5 (進捗 UI)。検証: build 緑 + テスト 156 件（+round3 2、fail-first）。 | Kenshiro Kuroga \& Claude |
| 2026-06-04 | 1.10.58 | **(#250 PR2、Manager v0.24.0)** 復元後の整合性チェック（`RestoreReconciliationService`）を `games/` だけでなく **`guide/`（#253 初回説明スライド画像）** にも拡張。旧実装は games/ のズレ（起動不能ゲーム／無い版／孤児フォルダ）は検出するのに、`intro_slides.image_path`（`guide/<file>`）が指す画像の欠落が**無警告（silent）**という非対称があった（Codex PR #274 P1）。「DB だけ復元→初回説明スライドが存在しない画像を指す」silent breakage を塞ぐため intro_slides の画像参照を突き合わせ、欠落を新 finding `BrokenIntroSlides` として検出＋復元レポートに専用セクションを追加。画像欠落は起動を妨げないため warning（`HasCriticalFindings` 非算入、thumbnail/background と同格。text-only スライドは対象外）。**整合性チェックの手動再実行ボタンを追加**: `Analyze()` は復元直後（`btnRestore_Click`）にしか走らず通常起動では走らないのに、復元レポートが「修正後に Manager を再起動すると再チェックされる」と**不正確**な案内をしていた。バックアップタブに「整合性チェック」ボタンを追加し復元を伴わずオンデマンド再チェック可能に、手順③を「ボタンで再チェック（再起動では不可）」に修正。`RestoreReportForm` を復元後／手動の 2 モード対応（手動時は復元前提の文言を回避）。**復元レポートの safety 案内是正（ユーザー指摘）**: `safety_*.db`＝復元前の状態なので「今の games に合わせるなら safety 再復元」は問題ごと元に戻す誤誘導。「今回の復元を取り消す操作で元の問題も戻る」と明示し当時 games 補完を本筋に、手動モードでは safety 文言を非表示。**バックアップタブ 3→2 箱統合（ユーザー指摘）**: 「操作」箱を廃止し復元・削除・整合性チェックを「バックアップ履歴」の表下へ集約（選択行への操作なので同箱が自然）、保存先は最下部ラベルへ。テスト基盤: `PathManager` に test seam（`SetBaseDirectoryForTest`、internal、#239）を追加し PathManager 静的依存の reconciliation を一時 install dir で検証可能化、初テスト（欠落→検出/全存在→非検出、fail-first）。**レビュー対応（同 PR・version 据え置き）**: (#1) 非表示スライド（`is_visible=0`）を対象外に＝Launcher が表示しない＆「画像なしで表示されます」文言と整合（fail-first テスト追加）。(#2) 画像のみ finding 時に無関係な孤児削除案内しか出ない点を是正（孤児がある時だけ削除案内＋画像欠落の直し方を追記）。(#3) 手動チェックの `Analyze()`（SMB 上で全ゲーム×全版走査）に wait カーソル。(#4) `BrokenIntroSlides.ExpectedPath` の区切り混在を正規化（表示のみ）。**レビュー round2**: (#1) レポート文言「画像なしで表示」が image-only スライドでは虚偽（Launcher は本文なし＋画像なしのスライドを丸ごと除外＝スライドごと消える）なのを「本文ありは画像なし表示／本文なしはガイドから外れる」に一般化。(#2) 手動チェックモードの前置き「復元されません」→「含まれません」に一般化。(#3) 画像欠落のみの finding で手順1「Launcher を閉じる」が浮くのを、フォルダ差し替え/削除が要る finding 時だけに条件化。据え置き: #4（同一画像を複数スライドが参照→per-slide 列挙は「N スライドが影響」を正しく示すので妥当）。schema 変更なし。**#250 はまだ閉じない**（PR3＝DB+アセット一式の復元 2 モードが残る）。検証: build 緑 + テスト 139 件。実機 UI・SMB 体感は pre-release で目視。 | Kenshiro Kuroga \& Claude |
| 2026-06-04 | 1.10.57 | **(#299、Manager v0.23.0)** #295 の操作単位バックアップの進捗 UI を「モーダル」→「**非ブロッキング＋下部ステータスバー統合**」に改装。アセット操作直後のモーダル（操作不能）を撤去し、バックアップはバックグラウンド worker で実行・操作継続可に。進捗は statusStrip1 に統合表示（`[バックアップ中...][中止][バー][N%][ファイル名]`、1 段・左詰め）。**コアレス**: 実行中の変更は dirty＋includeAssets を OR 蓄積し終了後に 1 回だけ走る（`TryStartRun`/`TryContinue`、CAS＋replace-in-session で最終世代は正しい）。#295 レビューで保留した perf/debounce 論点を解消。中止/閉じる確認/未バックアップ時の「今すぐバックアップ」1 クリック復旧つき。§機能12 の perf 注記を「#299 実装済」に更新。あわせて共有 `ProcessingDialog` の全モーダル進捗に「フェーズ + N%」を併記、ユーザー表示の「控え/控える」→「バックアップ」に統一。実機フィードバック反映（見切れ→AutoSize / 2 段→1 段 / 左詰め / ファイル名末尾で中止位置固定 / % 表示 / 中止・今すぐバックアップを ToolStripButton + 枠描画レンダラーで「ボタンらしく」＝実 Button ホストはレイアウト破壊+残像のため不採用）。**レビュー対応（同 PR・非ブロッキング化で新たに開いた並行性の堅牢化、version 据え置き）**: (C-1) `AssetSnapshotService.WalkTree` の per-file 例外で世代まるごと Failed になる穴を修正＝走査中に並行操作（ゲーム削除/版up）でファイルが消えると `HashAndStore` が throw → `CreateSnapshot` catch で全体 Failed（~6GB 再走査+警告チラつき）だったのを、dir 列挙失敗と同じ best-effort skip（`SkippedFileCount`）にして世代は IsPartial Success に留める（消えたファイルは次コアレス再走査で整合、キャンセルは伝播）。(B-1) `CancelCurrentBackup` がコアレス済み pending を止めず再走査が走る点を、cancel 時に `_dirty`/`_pendingIncludeAssets` もクリアして解消。(D-1) `Task.Run` 起動失敗で `_workerRunning` が永久 true になり自動バックアップ無言停止+閉じる確認誤発火する穴を try/catch でフラグ巻き戻し。(A-1) DB フェーズ進捗 Detail のフルパスをファイル名に。据え置き: D-2（表示順レース＝自己回復）/ E-1（ProcessingDialog の % 全モーダル波及＝設計どおり、pre-release 実機確認）。**レビュー round2（同 PR・version 据え置き）**: (#1) round1 で skip 総数を `skippedDirCount` に合算したため「N 個のフォルダ」と誤報する回帰を、`SkippedFileCount` を別フィールド化し UI 文言を「フォルダ N / ファイル N」に是正。(#4) per-file の try スコープを実 I/O のみに絞り、`entries.Add`/`progress.Report` 例外の二重カウントを防止。(#2) 非ブロッキング化で worker 稼働中も復元を起動できるため、復元前に進行中の自動バックアップを協調キャンセル（live DB の File.Replace 置換と worker の DB コピーの衝突回避。手動バックアップは共有プール CAS + 24h grace で同時実行安全＝gate せず）。(#3) 復元は live を置換する前に復元元を `PRAGMA quick_check` で検証し、壊れ / 不完全 .db（worker の DB フェーズ中 abrupt exit 残骸を含む。`BackupCatalogService` は list 時 quick_check を省くため復元候補に出うる）を置換前に弾く（live 無傷で中止、SMB コピー破損一般への防御）。据え置き: #5（D-2 表示順レース＝自己回復）。**レビュー round3（同 PR・version 据え置き）**: (#1 High) `SessionAssetCaptureFailed` の getter だけが `_gate`（RunSessionBackup が ~6GB 走査中ずっと保持）をロックし、その UI 読み取り（BackupRunningChanged コールバック）が次 worker の `_gate` 取得で UI を走査完了までフリーズさせる「非ブロッキング化の自壊」を、フラグの `volatile` 化＋getter ロックフリーで解消。(#6) DB-only 実行中にアセット pending を抱えたまま中止すると pending アセットが無警告で未控えになる穴を、中止時に未バックアップ警告を立てて解消。(#3 ユーザー判断) 復元前 quick_check を段階化＝open 不能（切り詰め/非 DB）は中止、open 可だが非 ok な不健全バックアップは UI 確認（`CheckIntegrity`+`allowIntegrityWarnings`）で override 可（最後の手段の災害時。safety 退避済で可逆）。据え置き: #2（一時ロックで完全→部分 manifest 置換は SMB+常駐 AV では現挙動が堅牢＝Failed 案は毎回 ~6GB 再走査で完全世代が取れない悪化。live 無傷+前セッション完全世代 retention+IsPartial 警告が安全網）/ #4（協調キャンセルは多段の置換前処理で実害ほぼ無し）/ #5（限定ケース）。**レビュー round4（同 PR・version 据え置き）**: (H-1 High) 復元前に協調キャンセルした worker が live DB ハンドルを解放しきる前に `File.Replace` へ到達する狭い窓を、共有違反を最大 ~800ms リトライしてから既存 fallback に落とす `ReplaceWithSharingRetry` で緩和。(M-1) IsPartial を sticky 警告化＝部分取得世代が次の DB-only 緑✓で「完全に控えた」と誤認される穴を是正（回復は完全アセット成功時のみ。round3 #2 deferred の安全網を強化）。(L-1) 復元起点のキャンセルは「未バックアップ」警告を立てない（置換直前で spurious）。(L-3) ProcessingDialog の % 併記で空メッセージ時の先頭スペースを除去。据え置き: M-2（コアレス遷移境界の中止取りこぼし＝microsecond 窓・自己回復）/ L-2（% 固定幅 40px の高 DPI clip＝実機確認）。検証: build 緑 + テスト 136 件（+round4 fail-first 2）。本番 SMB 体感・並行編集 churn（C-1/M-1）・worker 稼働中の復元/手動同時実行（H-1）・連続アセット操作直後の UI 応答・整合性警告ダイアログ・% の高 DPI clip は pre-release 全体テストで。 | Kenshiro Kuroga \& Claude |
| 2026-06-03 | 1.10.56 | **(#295、Manager v0.22.0)** 自動バックアップを「Manager 起動時の時間間隔チェック」→「データ変更操作の成功直後・**操作単位 / replace-in-session**」に再設計。§機能12「自動バックアップ」記述と設定値リスト・§7.3 settings 表を更新（`backup_auto_interval_hours`/`_unit` 撤去、`backup_auto_enabled` を「変更時に自動」へ再定義、`last_backup_at` はトリガ gate でなく表示用に降格）。**狙い**: 旧方式は「変更→閉じる→次の起動までに DB 破損でその変更が一度も控えられない」穴があった。新方式は変更操作の直後に控えるので作業がその場で守られる。**1 起動 = 1 自動世代**（同セッション内は前世代を消して上書き、retention は直近 N セッション、手動は別枠で温存）。**非対称**: DB(~124KB)は毎操作（サブ秒・バックグラウンド）、重い games/guide アセット取得(~6GB)は games/guide を実際に変える操作のときだけ（各 Form の `AssetsChangedOnDisk` で判定、メタデータ編集は DB だけ）。新規 `Services/SessionBackupCoordinator.cs`（セッション状態 + enable gate + UI 段取り、best-effort=失敗しても操作を巻き戻さない）。`BackupService.RunSessionBackup(includeAssets,…)` 追加。**撤去**: 起動時トリガ (`StartAutoBackupIfDue`/`IsAutoBackupDue`/`RunAutoBackupIfDue`)、起動時 lease (`TryAcquireBackupLease`/`RollbackLeaseOnFailure`)、設定タブの時間間隔 UI。検証: build 緑 + テスト 127 件（新規 `SessionBackupCoordinatorTests` 15）。実機 SMB での操作単位動作確認は別途。**レビュー round2**: ゲーム本体の控え失敗/部分取得を「✓」に潰さず警告化 (旧 StartAutoBackupIfDue の可視化を DescribeResult に移植)、失敗ステータス sticky 化、操作後の backup タブ再描画、撤去済 last_backup_at rewind の削除。**レビュー round3**: 同一セッションのアセット操作失敗/キャンセルが直前の控えを消す High バグを修正 (削除述語を「新 manifest を書けたか」に統一)、並び替え (display_order) もトリガに配線、成功メッセージ→バックアップの順統一。**レビュー round4**: アセット走査を**キャンセル**すると緑✓が出る High バグを修正 (キャンセルは Skipped を返し DB 成功なので緑になっていた。`DescribeResult` に assetsRequested を渡し「控えを要求したのに非成功なら警告」へ)。`.db`/`.manifest` は DB-only 末尾セッションで timestamp が乖離する (replace-in-session) ため PR3 のペアリングは別ロジックが要る点を #250 に申し送り。**レビュー round5**: sticky 警告がセッション内の後続 DB-only 成功に緑✓で上書きされ消える Medium バグを修正 (coordinator が「ゲーム本体控え未完了」状態を保持、未控えの間は緑✓を出さず毎操作で警告再表示、回復はアセット成功時)。**レビュー round6**: retention が「直近 N セッション」を保てず履歴を過剰に削る High バグを修正 (replace-in-session の前世代削除が `ApplyRetention` の後に走るため、これから消す世代を母数に数え過去セッションを 1 操作ごと 1 件余計に削っていた=K 操作で過去 K-1 世代消失。`.db`/manifest/GC blob の三層に影響。前世代パスを retention の母数から除外して修正、再現テスト 2 本)。restore-lock 延期が完全 silent だった Medium を `BackupResult.Deferred` で警告化。**レビュー round7**: 撤去済 lease を多ホスト排他の根拠とする stale コメント＋「多ホスト安全」過大主張を訂正（lease 撤去で多ホスト並行 GC 窓が拡大、>grace 初回 ingest 中の他ホスト GC が取得中 blob を誤回収しうる #250 C2）。**対応（ユーザー判断）: `GcGracePeriod` を 1h→24h に延長して C2 を塞ぐ**＝現実的 ingest(数十分〜1h)は 24h 内で保護、24h 超のみ #250 heartbeat-lease 領域に残る（被害は backup 世代のみで live は自己回復。advisory lock は失効<ingest で穴が残るため不採用）。DB フェーズ中キャンセルの silent を警告化。**レビュー round8**: DB 側 retention 母数除外の full-path 完全一致依存を修正（非正規化 `backup_destination_path` で除外が外れ round6 バグ silent 再発しうる穴をファイル名比較=アセット側と対称に。fail-first で teeth 確認）。`IntroSlideEditForm.AssetsChangedOnDisk` に将来 orphan 掃除追加時の縛りコメント。連続アセット操作の coalesce(perf) は実機計測待ち。**レビュー round9**: `last_backup_at` の write-only 化に伴うコメント訂正、ゲーム削除の退避フォルダ `games/{id}.pending-delete-{guid}` を asset snapshot から除外（give-up 時に残った削除ゴミが manifest に蘇る混乱を防止）、操作単位トリガで debounce を採らなかった判断（reorder 含め commit 境界で都度発火＝「その場で守る」優先、reorder は DB-only）を明文化＝reorder 連打の churn は実機 perf 計測対象。**レビュー round10**: 用語統一「ゲーム本体→ゲームファイル」の取りこぼし 3 箇所（進捗ラベル・異常メッセージ）修正、`last_backup_at` の SPEC 記述を「write-only / read 経路なし」に是正＋#295 で撤去済の lease 方式・auto 削除時 rewind の旧記述を撤去、死にフォーム `StoreSectionListForm`（DB 変更するが backup trigger 未配線・参照ゼロ）を削除して coverage を完結。**レビュー round11**: §6.5 の取り込み/バックアップフェーズが旧モデル（起動時/定期バックアップ）のまま残存していたのを #295 操作単位モデルへ改定＝**取り込みフェーズ実装時に取り込み成功直後 `RunAfterOperation(assetsChanged:false)` を呼ぶ縛り**（さもなくば取り込みデータが次の編集まで未控え）と、**編集非依存の周期セーフティネット撤去というトレードオフ**を記録。§機能12 にアセット走査の perf 特性（毎操作でツリー全列挙＝コストは操作回数に線形、非ブロッキング化は #299）を追記。 | Kenshiro Kuroga \& Claude |
| 2026-06-02 | 1.10.55 | **(#250 PR1、Manager v0.21.0)** バックアップを `toneprism.db` 単体 → **DB + アセット (`games/` + `guide/`) の共有プール (CAS / SHA-256) 控え**に拡張する #250 の **PR1 (取得エンジン)**。§機能12 にアセット控えのサブ項目を追記、設定値リストに `asset_snapshot_enabled`/`asset_snapshot_retention_count` の 2 キーを追加。**方式選定**: 当初ハードリンク世代スナップショットで実装したが、プロジェクト全体が SMB 上＋ファイルサイズ管理がシビアで不採用（ハードリンクは 1 実体に複数エントリ＝単純合計が二重カウントし削減が見えない、実測 games/ の 63% が重複）。**共有プール**＝中身ごとに `asset_pool/<sha256>` に実体 1 個（名前同じ中身違いは別保存、中身同じは集約）、各世代は `asset_snapshots/<auto|manual>/<ts>[_host].manifest`（hash↔relpath の目録）。DB バックアップ成功直後に同一 timestamp/trigger で `AssetSnapshotService.CreateSnapshot` を best-effort（失敗/キャンセルしても DB バックアップ・`last_backup_at` を壊さない）。SMB 対策のハッシュキャッシュ（直近 manifest で relpath+size+mtime 一致なら再ハッシュしない）。retention は auto manifest のみ（既定30、manual 温存）＋ mark-sweep GC（残 manifest 参照 hash 以外の pool を削除、直近1h は grace）。SHA-256 でどんな容量集計でも実サイズ（実測 5.73→2.1GB）、SMB 越しでも有効。schema 版据置 v22（settings は K/V）。**復元 2 モード=PR3 / `RestoreReconciliationService` guide/ 拡張=PR2**。検証: Manager build 緑 + テスト 112 件合格（AssetSnapshot 16: 同一中身別パスで1個 / 名前同中身違いは別 / GC / retention / grace で直近未参照 blob は残る / blob mtime は配置時刻 / 履歴ありで sources 消失→Skipped / 非対称欠損→Skipped / DB games 登録済でフォルダ不在→Skipped / skip>0 は IsPartial 等 ＋ FileOperationServiceLongPath 9: UNC 長パス変換 + 260字超で throw しない回帰）。実機 SMB round-trip + 多ホスト並行 GC は別途。**レビュー round6**: 長パス `\\?\` の単純前置が UNC で構文不正パス (`\\?\\\server\…`) を生成し、SMB 直アクセスで列挙が全失敗→空の控えを Success 扱いする最重大欠陥を修正（UNC 分岐を `FileOperationService.ApplyLongPathPrefix`/`ForceLongPath` に集約、`localhost C$` 共有で end-to-end 実証、C1）。あわせて控えが除外リスト非適用＝丸ごと取る理由（Electron `node_modules` 等の実行時必須フォルダ保護）を本節に明記、進捗分母を走査と一致 (M1)。**レビュー round7**: silent failure 経路を堅牢化 — games/guide が「存在するが root 列挙失敗」を不在と同じ異常扱いに (M-1)、両 sub 不在時の新規 install/異常 判別に DB games 件数を権威軸として追加 (到達不能共有では manifest 履歴が空に見え判別不能なため、M-2)、pool blob の配置時刻 mtime stamp 失敗を Warn 化 (握り潰すと GC grace が静かに無効化、M-3)。**レビュー round8**: 深部フォルダの列挙失敗で部分的な控えになった世代を Success に潰さず `IsPartial` で UI/ログに「部分取得の可能性」を明示 (root のみ検出だった M-1 を深部 skip 集計に拡張、C1)、`.poolsize` 未計測時の実使用量 0 B 矛盾表示を「計測中」に是正 (A/L1)。多ホスト×初回フル取得が grace 超で取得中 blob を誤 GC (C2)・GC の毎回 pool 全列挙 perf (C3)・timestamp↔.db 名の衝突相関 (B-L2)・改行入り名での manifest 破損 (D-L3) は PR2/PR3 申し送り。**レビュー round9**: `ApplyLongPathPrefix` の `Path.GetFullPath` が legacy path handling で ≥260 字に PathTooLongException を投げる自己矛盾を fallback で解消 (深い backup_dest を Failed→Success、B-1)、設定 UI の TabIndex 補完 (A-1)、既存本番 DB が次回バックアップで ~6GB 初回 full ingest を始める点を docs に運用注意追記 (C-2)。D-L3 は NTFS が制御文字を許さず実質発生しないため降格。**実機検証フィードバック**: 進捗バーの重み付けを DB 0-10%/ゲーム本体 10-99% に是正 (旧 95-99% 圧縮の「固まって見える」を解消)、ProcessingDialog ラベル見切れ修正、UI 用語「アセット控え」→「ゲーム本体のバックアップ」統一、**設定のゲーム本体 enable/世代数 controls を撤去し DB と一括バックアップ化・保持世代数を `backup_retention_count` に統一**、最終バックアップ表示を 2 行 (日時 / ゲーム本体の中身) に整理。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.54 | **(#211/#212/#291、Manager v0.20.2 / Launcher v0.10.3)** ストアセクション編集 UI に条件付き規則を追加し §7.3 store_sections の `section_type`/`max_display_count` 説明に明記。**#212**: スライドショー/タイルグリッドはソースを **手動 ＋ ランキング系 (popular/recently_played/random)** に制限 (= `max_display_count` 有効ソース ∪ 手動。厳選枠は少数を大きく見せる枠なので表示数を制御できるソースのみ合う。背景画像/表示テキストはゲーム自身にフォールバックするので自動でも崩れない。genre/新作/制作年等のフィルター系は全件表示で枚数を絞れず不可)。通常カテゴリ行は手動＋全自動可。**UI 方式 (実機指摘で改訂)**: ソースのドロップダウン自体をタイプで構築し直し、厳選枠では 4 ソースだけ出す (許可外は一覧に出さない)。当初の「全ソースを出し選択時に手動へ戻す coerce」方式が「厳選枠でも全フィルターが並んで紛らわしい」ため変更 (`_sourceMap`/`SelectedSource` で表示index↔canonical を解決)。**(#211 fix)** `max_display_count=0` 保存セクションを開くと `nudMaxDisplayCount.Minimum=1` 違反で例外クラッシュする回帰を修正 (読込時に Min/Max クランプ)。**#212 関連修正**: Launcher の `build_tile_grid_section` が `max_display_count<=0` を「上限なし=最大3枚」と扱うよう修正 (従来 `mini(0,3)=0` で手動タイルグリッドが空表示になる #211 起因の回帰)。スライドショーは `max>0` でスライド枚数を上限し、生成枚数を container `slide_count` meta に持たせて `_switch_slide` の wrap を整合 (ランダム時の「特集 N 枚」)。**#211**: `max_display_count` はランキング系 (popular/recently_played/random) のみ編集可 (TOP5 等の特集枠で上限が有効)、手動 (割当全件) と条件系 (genre/players_*/difficulty/play_time/online/controller + **recent=新作**[今年の release_year を display_order 順=本物のランキングでないため condition 扱い、#290 review]、条件一致全件) はグレーアウト + `0` 保存 (= 上限なし、Launcher の `max_display_count<=0` 解釈)。**#291**: 新ソース `release_year:YYYY` (制作年指定) を追加 — 任意年の `release_year` を display_order 順で全件 (新作=今年固定 に対し任意年)、条件系として max グレーアウト。Launcher `store_section_repository.gd` に `release_year:` 分岐追加。**#290 review**: 難易度/プレイ時間の値指定を数字直打ちからゲーム編集同様のラベル付きドロップダウンに変更。**フィルター系並び順**: genre/players_*/difficulty/play_time/online/controller を `release_year DESC, title ASC`（なるべく最新の制作年を頭に＋名前順、ユーザー要望）に変更（単年フィルターと manual/ranking/random は据置）。スキーマ変更なし。docs `usage/store.md` も更新。実機 UI 確認は別途必要。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.53 | **(#209、Manager v0.20.0)** ゲーム編集で**個別バージョンを削除**できる機能を追加。`EditGameForm` の「ランチャーで表示するバージョン」ドロップダウン隣に「このバージョンを削除」ボタン。選択版を確認ダイアログのうえ即時削除し、版の DB 行 (version 別 developers は version_id FK cascade) + 版フォルダ `games/<gameId>/v<version>/` を消す。ガード: **最後の 1 版は削除不可** / 確認必須 / **アクティブ版 (games.version) を消したら残り最新版 (id DESC) に自動付け替え** (表示版 dangling 防止) / セッション競合は行 DML 扱い (Launcher 単独では警告せず、フォルダロックはリトライ案内)。整合は既存ゲーム削除と同じ 3-phase + rollback (フォルダを `.pending-delete` 退避 → DB 削除＋付け替えを 1 transaction → 物理削除)。新規 `Services/GameVersionDeletionService` (UI から分離) + `DatabaseManager.DeleteGameVersionAndReassignActive` + `VersionRepository` の in-transaction helper 群。**active 判定/フォルダ解決は DB・disk の確定値で行う** (フォーム上の pending リネームに依存せず dangling/orphan を防止)。スキーマ変更なし (version_id FK cascade は schema v18 既存)。**active 版削除時は `games` 行を残存版で full mirror する** (#209 Codex P1): `games` は active 版の `executable_path`/`thumbnail`/`title` 等もミラーし Launcher が `games.*` を直読みするため、version 文字列だけ付け替えると削除済みフォルダの exe を指して起動不能になる (`GameRepository.MirrorActiveVersionIntoGameInTransaction`)。「最後の1版は削除不可」は UI ガード + transaction 内 count 確認の二重 (Codex P2、`BEGIN IMMEDIATE` で並行削除を serialize)、`games.version` が NULL の異常 DB も付け替え対象 (Codex P3)。検証: 一時 DB で `VersionDeletionTests` (DB 層 + ミラー/P2/P3 + `GameVersionDeletionService` フォルダ 3-phase)、全 87 テスト合格。テスト基盤に `DatabaseManager(DatabaseConnection)` 内部 ctor 追加 (#239)、`GameVersionDeletionService` はフォルダパス注入式で PathManager 非依存。即時削除は OK を介さず DB 確定するため `EditGameForm.DataChangedOutsideOk` で親グリッドを Cancel 時も再読込。版フォルダ削除は file ロックで調停 (プレイ中は `FolderLocked` で安全に弾き、ボタン再押下で再試行)。docs `usage/games.md` 編集セクションに版削除手順追記。**実機 UI 確認は別途必要** (破壊的のため作業 DB で実行不可、DB コピーで目視)。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.52 | **(#283、Manager v0.19.5 / Release Tooling v0.1.24)** Manager の Launcher 版数読み取りを、同梱 project.godot の parse → **エクスポート済み `TonePrism_Launcher.exe` の FileVersionInfo 読み** (Updater と同方式) に変更。exe 不在の dev では `<repo>/Launcher/project.godot` の `config/version` 直 parse に fallback (#281 の `ConfigVersionRegex` / `ParseConfigVersion` を dev 用に残置)。これに伴い **`Launcher/project.godot` の release zip 同梱を廃止** (`Release.ps1 $filesTemplates` / `$script:BundleManifestFiles` から除去、prod は exe から読むため不要)。新規 `PathManager.LauncherExePath`。**トレードオフ + 安全網**: prod の版数は SoT (config/version) → `Set-ExportPresetVersions` → export_presets.cfg → Godot/rcedit → exe VERSIONINFO の多段 stamp パイプラインの派生値となり、一手抜けで silent に古い/欠落版数を表示しうる (実際 検証時に stamp 前の exe が stale 0.9.1.0 を焼いていた)。→ **`Release.ps1` に `Assert-ExportedLauncherVersion`（`Build-Launcher` 直後・upload 前）を追加し、exe FileVersion ≠ SoT を hard fail で公開前に止める**（Release Tooling v0.1.24）。リリース時の成果物 版数目視は backstop（§3.7.8）。**検証**: export_presets を SoT 値に stamp 後 `--export-release` で exe を生成し FileVersionInfo が `v0.10.2` 相当を返すことを実機確認、ゲートが stale `0.9.1.0` / 空を Fail・一致を PASS する判定を確認、dev fallback は exe 不在で project.godot を読むことを確認、Manager build 緑 + 全76テスト合格。#281 派生の単一 PR。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.51 | **(#281、Launcher v0.10.2 / Manager v0.19.4 / Release Tooling v0.1.23)** Launcher 版数の SoT を **`project.godot` の `[application] config/version`** に一本化（従来は `version.gd` の `const MAJOR/MINOR/PATCH` と project.godot config/version の二重持ち）。`version.gd` は `ProjectSettings.get_setting("application/config/version")` を読むだけの薄いアクセサに変更（`get_version_string`/`get_version_number` API 不変＝consumer 無改修）。**Manager**: `VersionInventory.ReadLauncherVersion` の読み取りを version.gd の 3 正規表現 → project.godot `config/version="X.Y.Z"` 1 正規表現（`ConfigVersionRegex`、`config_version` には誤マッチしない）に変更。**Release.ps1**: `Assert-LauncherVersion` を project.godot 読みに変更、Phase 2 の project.godot config/version 同期を撤去（SoT 自身）、install 先同梱ファイルを `Launcher/version.gd` → `Launcher/project.godot` に差し替え（`$filesTemplates` / `$script:BundleManifestFiles`）。エクスポート済み Launcher exe は project.godot を .pck 内に隠匿するため Manager UI parse 用にコピーを明示同梱（旧 version.gd 同梱と同理由、manifest 駆動 `ValidateStaging` は自動追従、`ValidateStagingLegacy` は v0.3.0 zip 構造凍結のため据置）。§3.7.8 チェックリスト・§3.7.6(heartbeat `launcher_version`)・§バージョン体系の「管理場所」を project.godot config/version 基準に更新。検証: 同梱 Godot 4.6.2 ヘッドレスで `Version.get_version_string()`=`v0.10.2`、Release.ps1 regex で 0.10.2 抽出、Manager build 緑 + 全76テスト合格（レビュー対応でパース部を純関数 `ParseConfigVersion` に切り出し + `VersionInventoryTests` 16ケース追加 + C#↔Release.ps1 regex に相互同期コメント）。あわせて §8 の dead link（実在しない `.cursorrules` 参照）を `AGENTS.md` の「Release and Versioning」参照に修正。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.50 | **(#278、Manager v0.19.3 / Launcher v0.10.1)** 文化祭当日に **Launcher を起動したまま Manager で編集**できるようセッション競合警告をスコープ分け（①）＋ブラウズの DB ハンドル開放（②）。検証事実: Launcher は DB 読み取り専用（SELECT のみ、heartbeat は file）で `journal_mode=DELETE`+`busy_timeout` 下では Manager の行 write と Launcher の read を SQLite が安全に調停（write-write 競合なし・持続ロックなし）。**①**: 警告要否を純ロジック `Services/SessionConflictPolicy.ShouldWarn` に抽出。**別 Manager 検出時は操作種別問わず常に警告**、**Launcher 単独稼働なら `toneprism.db` をファイルごと差し替える操作（"バックアップ復元"/"データベース初期化"）だけ警告し通常の行編集は警告しない**。`MainForm.CheckSessionConflictBeforeWrite` から委譲。**Manager 起動時ダイアログも同方針** (`ShouldWarnAtStartup`=別 Manager がいる時だけ警告、Launcher 単独では起動時も出さない＝立てっぱなしで Manager を開くたびの警告を解消、危険操作は操作時に警告)。`SessionConflictPolicyTests` 34 ケースで検証。文字列ラベル判定のため DB ファイル置換操作追加時は `IsWholeDbReplacingOperation` に足す規約をコメント明記。**②**: `store_browse` が DB 接続を表示中保持するのをやめ `_ready` のロード後に `close()`（build はメモリ上の `_sections`/`section.games` のみ使い DB 不使用）。「すべて見る」「すべてのゲーム」ナビは repo の `is_open()`→`open()` で一時再接続→取得後 close。これで store 表示中も DB ファイルハンドルを握らず、Restore/DB 初期化が表示中でも衝突しない（screensaver/カルーセル/intro_guide と同じ open→close に統一、①の Restore 例外の根拠を縮小する defense in depth）。検証: 同梱 Godot 4.6.2 ヘッドレスで store_browse `_ready` 後 `is_open()=false`・ナビ相当の自動再接続を確認。関連: 版削除のファイル整合=#209、遠隔再読込=#263（割り切り）、Monitor=#261。 | Kenshiro Kuroga \& Claude |
| 2026-06-01 | 1.10.49 | **(#253 part 3/3、Launcher v0.10.0 / Manager v0.19.2)** 「初回説明」の Launcher 表示を実装し #253 を完結。スクリーンセーバーでキー押下 → ストアブラウズに入る前に `intro_slides` (DB v22) の案内スライドを表示する。スライドのレイアウトは中身で分岐（画像＋本文=左:画像/右:本文の横並び、片方のみ=中央）。上部左に見出し「はじめに」(カルーセルのグループ名と同じ Bold 56・左揃え)、スライド位置は「1/3」表記でなく戻る/進む上のドット(白=現在/灰=他、切替はフェード)。先頭スライドの「戻る」は無効時 focus 不可(←で選べない)+自動退避。**毎回表示**（各来場者にとって毎回が初回）・**手動ナビ**（`→`/`Enter` 次へ・`←` 戻る・`Esc` スキップ、自動送りなし = part 2 の design 変更を踏襲）。送り/戻しは中断メニュー流儀の**横スライド + フェード**（`TRANS_QUINT`/`EASE_OUT`、cross-slide）、加えて**画面下部に操作ボタン**（配置: 戻る・進む=中央の2分割 segmented（左=戻る(左だけ角丸)・右=進む(右だけ角丸・青アクセント)を非対称角丸のまま少し隙間をあけて並べる、各ボタンに非対称角丸を当てる方式） / スキップ=右端の独立ピル、全ボタン枠なし。背景は `store_browse` と同じ単色 `Color(0.08,0.08,0.08)` に統一。カルーセル/ブラウズ共通の操作説明バー `BottomBar`（ヒントは Enter のみ）も追加。マウス操作中はグロー枠・操作説明バーを隠す）（**キーボード/コントローラーは `←`/`→` でフォーカス移動・`Enter`/決定で押下する形式** = 中断メニューと同じ操作系、Godot 標準フォーカス系に委ね `_input` で方向/決定を consume しない。マウス/タッチは直接クリック。初期フォーカス=進む、先頭で戻る無効=フォーカス自動退避、最終で進む=「ストアへ」表記、`Esc` は素早いスキップ shortcut、マウス移動でカーソル表示。**フォーカス表示はブラウズ/カルーセルと同じ追従グロー枠** = `store_browse` の `FocusBorder` StyleBox + `GlowAnimator` を流用し focus 中ボタンへ lerp 追従、初出現は他画面と同じ zoom-in+フェードイン pop（scale 1.15→1.0・α0→1、CUBIC/EASE_OUT 0.25s）、ボタン標準フォーカス装飾は空 StyleBox で抑止、**ダイアログ等にフォーカスが移ったら枠を隠し追従も奪い返しもしない**）。ボタンバーは下端中央にアンカー+offset で高さ72px帯として明示配置（`MarginContainer`+preset の min-size 依存で高さ0に潰れる問題の回避、実機で発覚）。新規 `scenes/intro_guide.gd`/`.tscn`（#244 方針: DB 由来動的コンテンツは builder = シーン script 内構築、`.tscn` はルート+script の最小構成）+ 読み取り `scripts/intro_slide_repository.gd`（`get_visible_slides` / `has_visible_slides`、`image_path` の DB NULL→空文字正規化）+ モデル `scripts/models/intro_slide_info.gd` + `path_manager.gd` に `get_guide_folder()`（画像格納先 `guide/`）。`screensaver.gd` は表示対象スライドが**有れば** intro を挟み**無ければ**直接ストアへ事前分岐（空のとき intro を挟むと `TransitionManager` の遷移中再入ガードで再遷移が無視され固まりうるため。まっさら新規は 0 件＝直行）。intro 側も全 store 遷移経路 (Esc/ボタン/最終スライド) を incoming 遷移完了まで gate し、遷移中操作で再入ガードに弾かれ固まる経路を排除（コードレビュー指摘）。`.gitignore` に runtime の `guide/` 追加。さらに Codex 指摘対応: 放置時 `IdleManager` でスクリーンセーバー復帰、OS キーリピート(echo)無視、本文なし＆画像が読めないスライドは除外(blank 防止)、Manager 画像ピッカーは Launcher 非対応の gif を除外。`database_manager.gd` の `CURRENT_DB_VERSION` を 14→22 に追従（Launcher が本リリースで `intro_slides` を読むため、本番 DB(v22) 起動時の「対応版より新しい」警告も解消。v15〜v20 は Launcher 非読取テーブル/制約のみで読取不変）。検証: リポジトリ同梱 Godot 4.6.2 で全新規 script の parse + intro_guide を main_scene 差し替えヘッドレス起動で build 経路を実行（テストデータ round-trip 後にクリーンアップ）。docs `usage/intro.md`（スタッフ向け「初回説明の設定」）追加。あわせて Manager v0.19.2: スライド画像選択で**選択元が既に `guide/` 直下なら複製せず再利用**（孤児画像の再利用 + 重複コピー防止、外部の同名別画像は従来どおり suffix 取り込み）、保存失敗時の orphan 掃除は新規コピー時のみに限定（再利用した既存ファイルを誤削除しない）。 | Kenshiro Kuroga \& Claude |
| 2026-05-31 | 1.10.48 | **(#253 part 2/3、Manager v0.19.0、DB v21→v22)** 「初回説明」(旧イントロガイド) の Manager 編集 UI 追加 + part 1 の自動送り `duration_sec` を廃止する design 変更 (1 PR につき 1 SPEC bump に集約、#274 review #5)。**UI**: タブ「初回説明」(`IntroGuidePanel` UserControl、`StoreSectionPanel` ミラー) + スライド編集 Form (`IntroSlideEditForm`、本文/画像/表示) + 画像取り込み helper (`IntroGuideAssetHelper`、`guide/` へコピー・衝突自動 suffix)。DB write 前 session conflict check 準拠。**design 変更** (ユーザー feedback): 呼称「イントロガイド」→「初回説明」、自動送り廃止 → 手動ナビ (全スキップ + 次へ/戻る) 方針へ。§7.3 テーブル13 から `duration_sec` 削除 (`MigrateV21ToV22` は CHECK 付き列のため table recreate、FK/子無しで安全)、説明文を「毎回表示=各来場者の初回 / Launcher は手動ナビ」に更新、`ExpectedSchema` 同期。**§7.2** 本番インストール構成に **`guide/` フォルダ**追記 (スライド画像置き場、`games/` の隣、`intro_slides.image_path` が `guide/<file>` 参照、アップデート保護 + #250 バックアップ拡張対象)。Launcher の手動ナビ表示は part 3。 | Kenshiro Kuroga \& Claude |
| 2026-05-31 | 1.10.47 | **(#253、Manager v0.18.0、DB v20→v21)** §7.3 に **テーブル13 `intro_slides`** を追加 (イントロガイドのスライド: スクリーンセーバー → ブラウズ間に表示する案内)。`slide_id` / `display_order` / `body_text` (空可=image-only) / `image_path` (NULL 可=text-only、相対パス、画像は `guide/` にファイル別管理) / `duration_sec` (CHECK 1-60) / `is_visible`。他テーブルへの FK 無しの独立テーブル。`SchemaManager`: `CurrentDbVersion` 20→21、`CreateIntroSlidesTable` helper + `MigrateV20ToV21` (CREATE TABLE IF NOT EXISTS で idempotent、manager_sessions v13 と同型) + dispatch + `ExpectedSchema` 追記 (AGENTS §7.6 ワークフロー準拠)。本変更は #253 part 1/3 (schema)。Manager 編集パネル (part 2) / Launcher スライドショー (part 3) は後続 PR。`Manager.Tests/IntroSlideTests` で fresh-DB 到達 v21・CRUD round-trip・空 image_path→null 正規化・duration CHECK・v20→v21 migration のデータ保持を検証済。 | Kenshiro Kuroga \& Claude |
| 2026-05-30 | 1.10.46 | **(#271、Manager v0.17.4)** §3.8.2 / §3.8.3 / §7.3 の `manager_sessions` (Manager-Manager 同時起動検出) を clock drift 耐性化。#269 (Launcher) と同根の「読み手 now − 書き手自己申告 heartbeat」で PC 間時計ズレを被る問題に対し、(1) **検出 stale 閾値 30→60 秒**に緩和 (DB ベースで per-row mtime が無く `max(json,mtime)` 不可のため閾値マージンが SMB 非依存の主防御)、(2) **起動時 cleanup を 1 日 abandoned 閾値に変更** (旧 30 秒 stale DELETE は clock skew で生存中の遠隔 Manager row を消し → 検出から外れ → 両者 write → DB 破損、の危険があった。自 crash 残骸は UPSERT 上書き、検出は query 時 60 秒で除外)。サーバ時刻基準化 (net time / マーカー mtime) は SMB 構成確定後の別案として #271 に残置。`Manager.Tests/ManagerSessionDriftTests` で 3 シナリオ検証。コード `ManagerSessionService.cs` / docstring `ManagerSessionRepository.cs`。 | Kenshiro Kuroga \& Claude |
| 2026-05-30 | 1.10.45 | **(#269、Manager v0.17.3)** §3.8.7.3 Launcher セッション stale 判定を clock drift 耐性化。stale = 「読み手 now − 検出 timestamp」で PC 間時計ズレを被るため、primary path を `max(JSON last_heartbeat_at_unix_ms, file mtime)` の新しい方に変更（json=書き手自己申告時計 と mtime=SMB サーバ単一時計 の弱点を相互補完、どちらか fresh なら active）+ stale 閾値を 30→60 秒に緩和（heartbeat ×6 マージン、広げる方向は安全側）。残留リスク（読み手自身の時計が進む単機ケース）は NTP 同期 + 閾値で許容と明記。⚠️ `manager_sessions`（Manager-Manager 検出）は DB ベースで per-row mtime が無く同 logic 不可のため 30 秒据え置き（同 drift・閾値統一は follow-up 候補）。外部 AI レビュー指摘をコードで cross-verify して確定、`Manager.Tests/LauncherStaleDetectionTests` で 4 シナリオ検証済。コード `LauncherSessionService.cs` のみ（Launcher 側 heartbeat 書込は変更なし）。 | Kenshiro Kuroga \& Claude |
| 2026-05-30 | 1.10.44 | **(#251、Manager v0.17.2)** §3.8.2 / §3.8.7.4 の `SessionConflictDialog` 文言から「他 PC」前提を撤回。同一 PC 上の Launcher も検出対象 (§3.8.7.6) で検出 list に自 PC が並ぶ case があり、「他 PC で…」「両方の PC で…」「他 PC の人に確認してから」が噛み合わなかったため、Startup / EditOperation 両 context の title・body を「別の Manager / Launcher」の汎用文に統一 (他 PC / 同一 PC どちらでも正しく読める)。title は両 context とも `【危険】別の Manager / Launcher が稼働中です` に統一。**文言のみ・挙動不変** (起動時 Cancel = Manager 終了 / 操作前 Cancel = 操作中止 は変更なし)。コード: `Manager/Services/SessionConflictDialog.cs`。 | Kenshiro Kuroga \& Claude |
| 2026-05-30 | 1.10.43 | **(統合: PR #236 / Manager v0.17.0、1 PR 1 SPEC bump)** 本 PR の SPEC 反映を1行に集約 (旧 1.10.43〜1.10.47 を圧縮)。詳細な技術履歴は CHANGELOG `## Manager v0.17.0` を参照。**主な SPEC 反映**: ①§3.8.5 ゲーム追加の parent gameFolder retention を `parentCreatedThisCall`/`baseGameFolderCreated` flag 方式で対称化 + #120「既存フォルダあり」を wipe&recreate 方針に変更。②§7.3 `game_versions` に UNIQUE INDEX `idx_game_versions_game_id_version` を追記 (DB v15、v17 で `COLLATE NOCASE` 化、同一ゲーム同一版番号の重複を DB レベルで弾く最後の砦。`ExpectedSchema` は列ベース検証のため index 追加では不変、§7.6.3)。③§機能12 リストア手順を現行 atomic 実装 (tmp コピー後 `File.Replace` / `backups/safety/safety_HHmmss.db` / `SessionConflictHelper` / `RestoreReconciliationService`+`RestoreReportForm` / Reconciliation 前に migration 先行) に同期。④§2.2 機能3 ゲーム情報編集に画像差し替え仕様 (フォルダ外画像の自動コピー + `ImageNameConflictDialog` + 古い画像非削除) を追記。⑤§7.3 テーブル7 `game_genres` を DB v18 で廃止 (§7.4 ER 図 / CASCADE 一覧からも除去、ジャンルは `games.genre`/`game_versions.genre` のカンマ区切りに一本化)。⑥§7.3 `backup_log` を DB v19 で廃止し履歴を `backups/` フォルダ走査 (`BackupCatalogService` — 種類=フォルダ位置 / 日時・実行PC=ファイル名 `<種類>_<日時>_<host>.db` / サイズ=`FileInfo`) 由来に再設計 (§7.2 階層図・§3.x 履歴/削除 UI/ファイル名規則を file-scan ベースに更新、`ExpectedSchema` からも除去、復元監査は `safety_*.db` で代替)。⑦§7.3 テーブル1 `games.play_time` に `CHECK(1-3)` を DB レベルで復元 (DB v20、`MigrateV19ToV20`、親テーブル recreate は migration 検出時のみ `foreign_keys=OFF`+commit 前 `foreign_key_check`、範囲外値残存時は skip+retry)。⑧`backup_log.trigger_type` CHECK に一時 `'restore'` を追加 (DB v16、後の v19 廃止で消滅) + `RestoreService` に LAN advisory lock (`settings.restore_lock_owner`、5 分 stale 失効) を導入。マルチ PC 協調 (`settings.last_backup_at` lease / `restore_lock_owner`) は `settings` テーブルにあり不変。移行 SQL は同梱 sqlite3 の実データ round-trip + 実機 skip/rebuild 両経路で検証済。 | Kenshiro Kuroga \& Claude |
| 2026-05-25 | 1.10.42 | §機能23 サービスモードの **「4. ゲーム一覧状態確認」と「5. ゲーム起動テスト」を 1 項目「ゲーム動作テスト」に統合** し、中で 3 段階の確認方法を選ぶ方式に変更 (#74)。①ファイル存在チェック (起動しない・全件一括) / ②起動テスト (自動起動→ウィンドウ生成確認→自動終了で起動可否のみ判定) / ③試遊テスト (選んだ 1 本を実際に起動・終了は手動)。3 段階で「ファイルがあるか」→「起動するか」→「実際に遊べるか」を粒度別に確認できるようにする狙い。項目統合により機能一覧は 16→15 項目に。①②③ すべて実装済。名称は当初「ゲーム動作確認」「③試遊確認」だったが、実際に起動する②③を「テスト」で揃える一貫性のため「ゲーム動作テスト」「③試遊テスト」に改称 (起動しない①は「ファイル存在チェック」のまま=実行しない事を明示)。**あわせて「8. エラー内容表示+マニュアル」項目を廃止** — エラーコード別の対処法はエラー発生時のダイアログに併記する方式 (`error_dialog.gd` の `_REMEDY`、ゲーセン筐体のエラー表示と同様。スタッフが別途マニュアルを引かなくてよい) に変更したため。これにより機能一覧は **15→14 項目** に、以降を繰り上げ (§画面8 メニュー一覧の項目数記述も 14 に同期)。**あわせて UI 文言の「アプリ」を「ランチャー」に統一** — 同じ対象を指すのに「ランチャーの再読み込み」と「アプリの再起動/終了」が混在しており、かつ筐体ではゲームも"アプリ"のため曖昧だったため。項目13/14 を「ランチャーの再起動」「ランチャー終了」に改称し、終了制御 (Alt+F4 封印) の案内ダイアログ・ボタン文言も同調 (機能「終了制御」§詳細も同期)。**項目3「画面表示テスト」を拡張** — 従来の単色6色に加え、グリッド (格子線+中央十字+四隅対角線+セーフエリア枠90%/80%を1パターンに統合)・カラーバー (放送規格 SMPTE RP 219 / ARIB の HD カラーバー: 75%カラーバー+40%グレー袖 / リバース / 0-100%ランプ / PLUGE の 4 段) を追加。グリッドは細密+粗のクロスハッチ+真円 (画素アスペクト/歪み確認) を加えたジオメトリ調整パターンに強化。黒→白グラデーションの単体表示は当初追加したが、カラーバーのランプ段と重複するため削除。テストカード(モノスコープ)を参考に、液晶でも役立つ要素として「解像度+グレースケール」スライド(中央シーメンススター+周波数縞+11段離散グレースケール)を追加(CRT時代のコンバージェンス円等は液晶で無意味なため不採用)。全画面 `Control` の `_draw()` で描画する方式 (画像アセット不要・解像度追従) に変更。さらに**画面表示テストを順次表示 (スライドショー) 形式に変更** — 個別選択をやめ、開始すると全パターンを順番に全画面表示し、任意キー/クリック/パッドで次へ・←/Backspaceで前へ・Esc/Bで中断・最後で自動的にメニューへ戻る。テスト中はパターンの邪魔をしないよう画面に文字は出さない (操作方法は開始前の説明に記載)。**機能22 終了制御に例外を追加** — サービスモード表示中のみ Alt+F4 での終了を許可 (スタッフ専用の隠し画面のため誤終了の懸念なし、素早い終了手段。ただしゲーム実行中は除外)。**項目9「デバッグオーバーレイ切替」を実装** — 新 autoload `debug_overlay.gd`(`DebugOverlay`)を追加し、ON にすると画面左上に FPS・フレーム時間・メモリ・オブジェクト数・PC名・Launcher版数・解像度/モニタ・シーン名・ゲーム実行状態・DB接続・稼働時間を常時表示。当初は主ウィンドウ内CanvasLayerだったが、中断メニュー(別OSウィンドウ)の上には原理的に出せないため、中断メニューと同じ「透明・borderless・最前面(always_on_top)の別OSウィンドウ」+unfocusable+mouse passthroughに変更。`PROCESS_MODE_ALWAYS`でサービスモード・中断メニュー・ゲーム・全ランチャー画面の上に表示継続、出すモニタはゲーム窓/ランチャーに追従、メモリのみ保持(再起動でOFF)。画面上に「F3」表示はしない(本アプリのトグルはF3キーではないため)。Monitor接続は未実装のため「未実装」表示。**項目5「ネットワーク接続テスト」を 8 段階(①IP→②ゲートウェイ→③DNS→④インターネット→⑤インターネット速度→⑥共有サーバー接続→⑦共有サーバー読込速度→⑧Monitor)に拡張・実装** — 疎通(①〜④⑥)はブロッキングのため別スレッドで `OS.execute`/TCP 実行し `call_deferred` で反映。②ゲートウェイは ping 無応答ルーター対策で `arp -a` の MAC 取得をフォールバックに加え + 3 回 ping (取りこぼし誤NG防止)。速度(⑤⑦)は Godot 単体で正確に測れないため LauncherAgent(Companion) に委譲: ⑤は 6 本並列の継続ストリーム DL(各75MBを締切まで流す、Cloudflare `__down` の403上限が75MB)で測定サイト並みの Mbps、⑦は `FILE_FLAG_NO_BUFFERING` で OS キャッシュを回避し `games` フォルダ配下の**最大ファイル**(`.pck`/プレビュー動画`.mp4`/Unityの`.resS`等、最大100MB)を読んで実 MB/秒を算出(exe は Godot ゲームだと数百KBと小さく量不足で速度がブレるため非対象)。**項目14「ランチャー終了」の二段階確認を廃止** — サービスモード自体が隠し画面(Ctrl+Alt+F12)でここまで辿る操作が十分意図的なため、確認画面を挟まず押下で即終了(ゲーム実行中は従来通り除外)。 | Kenshiro Kuroga \& Claude |
