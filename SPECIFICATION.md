...
- [ ] **Updater (`GCTonePrism_Updater`)**: 該当コンポーネントのプロセス終了待機ロジック追加、ファイル置換対象として登録（§3.7.4）。**Phase 3 Updater 着手時の追加チェック**: 緊急復旧経路として `Install.bat` を内部呼び出しする実装を入れる場合、必ず `Process.Start("cmd", "/c <Install.bat path> [args]")` 形式で呼ぶこと（`call <Install.bat>` 形式 / `Process.Start("<Install.bat>")` 直接起動は禁止）。理由詳細と trade-off は §3.7.4 「Install.bat の起動方式」参照
