...
└── Companions/                               # 補助 exe 集約
    ...
    └── (将来) CompanionsCommon/               # 共有 Win32 ヘルパー (DllImport 集約等)
...
[12] Updater (CLI): Manager プロセス完全終了を polling 待機
      ↓
[13] Updater (CLI): `Manager/` 配下を rename-rollback 方式で置換（Manager.exe + 依存 DLL）、ユーザーデータは旧 `.bak` から carry-over
      ↓
...
**保護されるユーザーデータ**: `prism.db`, `games/`, `backups/`, `responses/`, `logs/` は置換対象外。これらはユーザー固有の運用データであり、アップデートで失われてはならない。Manager 側 / Updater 側ともに rename-rollback の `.bak` から保護対象を新 dir に carry-over する形で保護を実現。
...
- **責務（最小スコープ）**:
  ...
  3. `Manager/` dir を rename-rollback 方式で置換（staging dir からコピー、ユーザーデータは `.bak` から carry-over）
  4. 新 Manager.exe を起動
  5. 自分自身を終了
...
- **通信**: Manager → Updater 起動時に CLI 引数で渡す（実装は `Companions/Updater/Program.cs` / `CliArgs.cs` 参照）
  ...
  - `--wait-timeout <seconds>` (任意): Manager プロセス終了待ち timeout（default 60s）
