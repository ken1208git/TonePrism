...
- **詳細**:
  - 削除実行は **DB レコード + `games/{game_id}/` フォルダのセット削除**（Manager v0.8.3 で実装、`DeleteGameConfirmForm`）。「DB のみ削除」のオプション分岐は持たない
    - DB 側は `games` 行と CASCADE で削除される関連レコード（developers / game_versions / play_records / surveys / store_section_games）
...
- **取り込み（Manager 側）**: 3-state folder + 2-phase 方式
  ...
  **`responses/` 直下 vs subfolder の責務分離**:
  ...
  - **⚠ (#295 改定) 起動時/定期の「バックアップフェーズ」は廃止 → 操作単位へ移行**: #295 で起動時・定期の独立したバックアップトリガは撤去し、バックアップは **データ変更操作の成功直後に `SessionBackupCoordinator.RunAfterOperation` が取る操作単位方式**へ移行した（§機能12）。下の旧 3 ステップとトリガ表の「バックアップ」「`imported/` 削除」列は **#295 以前のモデル**で、取り込みフェーズ実装時に新モデルへ合わせて改定する（※ **#297 で取り込みフェーズ自体の廃止＝JSON 直読み+メモリ集計へのピボットが提案中**。#297 が通れば取り込み INSERT 経路が消滅し下記の縛りも不要になる＝実装する場合の縛り）。改定時の縛り:
    - **取り込み実装時は `RunAfterOperation` を呼ぶ（必須）**: 取り込み（JSON → SQLite INSERT）は **Manager の編集操作を伴わない DB 変更**なので、操作単位モデルでは自動では控えられない。取り込み成功（1 件以上 INSERT）直後に `_dbManager.SessionBackupCoordinator.RunAfterOperation(owner, assetsChanged:false, "応答取り込み")` を呼ぶこと。**さもなくば取り込んだアンケート/プレイ記録は、次に誰かが Manager で games/store/slide を編集するまで一切控えられない**（`IntroSlideEditForm.AssetsChangedOnDisk` の round8 #3「将来縛りコメント」と同種の予防）。
    - **`imported/` 削除のタイミング**: 旧「バックアップフェーズ成功後にまとめて削除」を、取り込みトリガの操作単位 backup の成否へ紐づけ直す（実装時に確定）。
    - **トレードオフ（#295 が開いた対称な穴・記録）**: 旧モデルの「Manager を開けば（due なら）編集有無と独立に 1 世代控える」周期セーフティネットは廃止された。編集を伴わない DB 変化（上記取り込み、または長期間誰も編集しない運用）は控えられない。DB 破損が主リスクの文化祭運用でこの保険を残すかは別途判断（issue 化候補）。

  - **旧バックアップフェーズ（#295 以前の参考・実装は操作単位へ移行済）**:
  ...
  - **トリガ別動作（「取り込み」列は有効。「バックアップ」「`imported/` 削除」列は #295 で操作単位へ移行＝取り込み実装時に改定）**:

    | トリガ | 取り込み | （旧）バックアップ | （旧）`imported/` 削除 |
    | --- | :---: | :---: | :---: |
    | Manager 起動 | ✅ | ~~✅~~（#295 で廃止＝操作単位へ） | ~~✅~~ |
    | アンケート/プレイ記録タブ切り替え | ✅ | × | × |
    | 「更新」ボタン押下 | ✅ | × | × |
    | 「今すぐバックアップ」ボタン（手動・別枠で温存） | ✅ | ✅ | （実装時確定） |
    | ~~定期バックアップ（将来実装）~~ | — | ~~✅~~（#295 で廃止） | — |
