...
- **アセット控え（games/ + guide/ の共有プール (CAS) バックアップ、#250 PR1、Manager v0.21.0）**:
  ...
  - **SMB ハッシュキャッシュ**: 直近 manifest を読み `relpath+size+mtime` 完全一致なら前回 hash を流用（再ハッシュ＝再読込しない）。2 回目以降は新規/変更分だけ読む。size+mtime 据え置きで中身だけ変える特殊ケースに弱い rsync 同等トレードオフ（games/ 追記専用で実害なし）。symlink/junction はスキップ、長パスは `\\?\`、除外リストは適用しない（丸ごと）。
