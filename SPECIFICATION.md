  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    4. ゲーム動作確認 — 3 段階の確認方法を選んで実行する（旧「ゲーム一覧状態確認」と「ゲーム起動テスト」を統合）。詳細ペインはまず確認方法の選択画面を出し、選ぶと中身全体がそのモード画面に切り替わる。モード画面で ←/B/Esc を押すと確認方法の選択へ戻る（詳細ペインからは出ない＝サブ階層ナビ）:
        ...
        - **③ 試遊確認**（手動終了）— ゲーム一覧から行を選ぶとそのゲームを起動して実際に試遊。遊び終わったら **ゲームを手動で終了** するか **HOMEボタン**（中断メニューと同じトリガ。押すとゲームを強制終了）で復帰。戻ると「『〇〇』は正しく遊べましたか？」と尋ね、スタッフが **〇 遊べた / × 問題あり** を記録（一覧に表示、サービスモードを開いている間は保持）。「ファイルはあるが実際に遊べるか」を人が最終確認する用途
