...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **ログ・情報:**
    ...
    8. エラー内容表示+マニュアル — 直近のエラー詳細 + エラーコード別の対処法マニュアル（例: E-2001: ゲーム実行ファイルが見つかりません→Managerでパスを確認してください）
    9. システム情報表示 — PC名、OS、解像度、Godotバージョン、Launcherバージョン等

    **設定・操作:**
    10. デバッグオーバーレイ切り替え — ON/OFFトグル。ONにするとサービスモードを閉じても画面隅にFPS・メモリ使用量・PC名・現在のシーン状態・DB/Monitor接続状態等をリアルタイム表示し続ける（マイクラのF3的な常時表示）。メモリのみ保持（再起動でOFF）
    11. フルスクリーン切り替え — フルスクリーン↔ウィンドウの切り替え。モニター違いでの表示崩れ対応用
    12. ランチャー表示モニタ選択 — 複数モニタ環境で、ランチャーを表示するモニタをその場で切り替え（`window_set_current_screen` で即移動）。**メモリのみ保持（再起動でプライマリに戻る）**。本番=単一モニタでは無関係。恒久的に別モニタで運用する場合は Windows のメインディスプレイ設定を使う。関連: 中断メニュー（機能4 / #30）はゲーム窓をランチャーと同じモニタへ寄せるため、ここで選んだモニタにゲーム窓も揃う
    13. ランチャーの再読み込み — DB再読み込み。再起動より軽い
    14. アプリの再起動 — OSプロセスとして再起動
    15. アプリ終了 — 確認ダイアログ付きでランチャーを終了。Alt+F4/×ボタン封印のため唯一の終了手段
...
- **UIデザイン**:
  ...
  - 詳細なレイアウトは実装時に決定
- **主要要素**:
  - **メニュー一覧**: 機能23で定義された15項目をカテゴリ別に表示
    - 診断・テスト（7項目）
    - ログ・情報（3項目）
    - 設定・操作（6項目）
