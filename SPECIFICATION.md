...
- **インストール手順は `INSTALL_README.txt` が SoT**。`docs/install.md` は概要 + GitHub Releases / 同梱説明書への誘導に留め、手順本体を二重に書かない（版数固定の手順とオフライン参照性は zip 同梱物の責務）。
