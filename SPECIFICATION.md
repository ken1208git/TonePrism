...
- **詳細**:
  ...
  - **個別削除 UI**: 履歴を選んで「選択した履歴を削除...」ボタン → 確認ダイアログ → バックアップファイルを物理削除（DB 行は無い）。auto を削除した場合は残った最新 auto の時刻へ `last_backup_at` を rewind（取り直しが skip されないように）。
...
- **設定値**（`settings` テーブルに保存）:
  - `last_backup_at`（UNIX秒）: 最終バックアップ完了時刻（#295 でトリガ gate / lease は廃止、表示用にのみ更新）
...
- **沿革**: v9 で追加 → v10 で `trigger_type` CHECK に `'safety'` 追加 → v12 で `relative_path` 列追加 → v16 で `'restore'` 追加 → **v19 で DROP**。既存運用 DB は migration で行が破棄されるが、物理バックアップファイルは残り初回走査で履歴に復活する（失われるのは失敗履歴と復元監査行のみ）。
  ...
  **バックアップ関連 settings キー**（`settings` テーブルに保存。backup_log 廃止後も不変 — マルチ PC の lease / retention 等で引き続き使用）:
  ...
  | キー | デフォルト | 説明 |
  | --- | --- | --- |
  | `last_backup_at` | 0 | 最終バックアップ完了時刻（UNIX秒）。#295 でトリガ gate/lease は廃止、表示用に更新 |
