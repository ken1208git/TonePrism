...
- **検出 trigger** (= dialog 表示の起点):
  ...
  - **編集操作前** (DB write 直前): 各 SectionPanel (`GameSectionPanel` / `StoreSectionPanel` / `SettingsSectionPanel` / `BackupSectionPanel`) の save handler 冒頭で `MainForm.CheckSessionConflictBeforeWrite(operationDescription)` を呼出し → 検出時 `SessionConflictDialog` (EditOperation context)、user Cancel でその操作中止。対象 button は 13 箇所 (ゲーム追加/編集/削除/バージョンアップ、ストア追加/編集/削除/並び替え ×2、データベース初期化、バックアップ作成/復元/削除)。read-only 系 (Refresh / OpenLogFolder 等) は対象外。
...
本機構は **Manager 専用**。Launcher の LAN-wide session tracking は別 PR (PR3b) で扱う。**設計原則**: SPEC §3.X (このリポジトリの他箇所、`prism.db` SMB 共有運用の項) で「Launcher は SQLite に直接書き込まず、JSON drop folder 方式を採用」と明示されているため、Launcher session tracking は本 manager_sessions table とは異なる経路 (JSON drop folder / file system flag / TCP/UDP 通信) で実装する必要がある。candidates 検討は #179 issue 本文 update + PR3b 着手時に設計議論から。
...
erDiagram
    ...
    %% (last_heartbeat < now - 30000 ms) を自動 cleanup。詳細は §3.X 参照。
```
