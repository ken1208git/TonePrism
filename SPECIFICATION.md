...
- **設定値**（`settings` テーブルに保存）:
  ...
  - `asset_snapshot_enabled`（TEXT, デフォルト "true"）: アセットスナップショット取得の ON/OFF（"false" 厳密一致のみ無効）
  - `asset_snapshot_retention_count`（INTEGER, デフォルト 30）: auto アセットスナップショットの保持世代数
...
- **アセット控え（games/ + guide/ の共有プール (CAS) バックアップ、#250 PR1、Manager v0.21.0）**:
  ...
  - **retention / GC**: **auto の古い manifest のみ削除**（`asset_snapshot_retention_count` 既定 30、manual 温存・#235 同様）。その後 **mark-sweep GC**＝残る全 manifest が参照する hash 集合を作り pool 内の未参照を削除（**直近 1 時間以内更新の pool ファイルは grace で残す**＝並行/直近書込のレース回避）。`backup_retention_count` とは独立。
