...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    5. ネットワーク接続テスト — 「接続テストを実行」で、手前から順に **①ローカルIP取得 → ②ゲートウェイ疎通 → ③DNS解決 → ④インターネット接続 → ⑤共有サーバー接続 → ⑥Monitor接続** を確認し各段階を OK/NG＋応答時間で表示（最初に×が出た所が原因）。実装: ②ゲートウェイは `route print` から既定ゲートウェイIPを取得し `ping`（locale非依存に "TTL=" で判定）、③DNSは `IP.resolve_hostname`、④インターネットは 1.1.1.1:443 へ TCP 接続（DNS非依存）、⑤共有サーバーは DBパス(UNC)から抽出したホストへ TCP:445。ping/TCP はブロッキングのため**別スレッド**で実行し `call_deferred` で結果反映（画面を固めない）。⑥Monitor は未実装のため「未実装」表示。⑤共有サーバーは本番がそこからゲーム/DBを読むため最重要。なお速度(帯域)測定は、単一接続では実速より過小評価され、共有はOSキャッシュで実態と乖離するため**疎通確認のみ**とし速度測定は設けない
