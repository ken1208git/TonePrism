...
- **ビルド管理**: 各 Companion は独立 csproj。`Release.ps1` が runtime に応じて個別にビルドする形（Updater=net48 は msbuild、LauncherAgent=net10 は dotnet publish。Companion ごとに個別判断）。リポジトリルートに `TonePrism.sln`（#245 で VS 開発用に追加。Manager / Manager.Tests / LauncherAgent を収録。net48 classic の Updater は未収録）。**`Release.ps1` は sln を使わず各 csproj を個別 build する**（sln は VS の WPF 開発専用＝NuGet 復元が「プロジェクトを読めない」エラーになる folder/単体 csproj 起動を避けるため）。
