...
- **保存先**: default は project root (`toneprism.db` のあるディレクトリ) の `logs/<component>/` 配下、user 設定 `logs_root_path` (#201、Manager v0.15.0 以降) で **親 root を移設可能**
  ...
  - 検出方法 (default 時): exe 隣から最大 10 階層遡って `toneprism.db` を探す。見つからなければ exe 隣にフォールバック（PathManager と同じ思想だが Logger 専用に重複実装することで PathManager 起動ログのキャプチャ漏れを防ぐ）。**既知制約 (R3 review High-2)**: Logger の本 fallback 計算と `PathManager.FindBaseDirectory` (priority-1 `toneprism.db` → priority-2 `.git` → priority-3 `Manager+Launcher` sibling) は `toneprism.db` 不在の異常 path で divergence する。例えば dev 環境で `.git` 直下から exe 起動した case、Logger は `<exe 隣>/logs/` に書出、PathManager (= LogSectionPanel viewer の scan 元) は `<repo>/logs/` を見るため、viewer から log が見つからない path がある。健全 install (= `toneprism.db` 存在) では発火しないため本 PR では受容、根本解消は Logger 側の resolution を PathManager.BaseDirectory 経由に揃える別 PR を要する。
