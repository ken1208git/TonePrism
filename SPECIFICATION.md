...
- **カラム**:
  ...
  - **(#211/#212) Manager 編集 UI の条件付き規則**: (a) **タイプ↔ソース**: スライドショー(1)/タイルグリッド(2) は `section_source='manual'` のみ (背景画像/表示テキスト前提の厳選枠のため自動ソース不可)。通常カテゴリ行(0) は手動＋全自動ソース可。(b) **max_display_count の有効性**: ランキング系ソース (popular/recent/recently_played/random) でのみ上限が意味を持つ (TOP5 等の特集枠) ため編集可。手動 (= 割当を全件表示) とフィルター系 (genre/players_*/difficulty/play_time/online/controller、= 条件一致を全件表示) では UI をグレーアウトし `max_display_count=0` で保存する。
