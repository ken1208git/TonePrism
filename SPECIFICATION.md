...
- **言語/環境**: C# / .NET。runtime は Companion ごとに個別判断（#258、Manager は net10 へ移行済）: Updater=net48（常在ランタイムで自己更新の最後の砦、net10 化しない）、LauncherAgent=**現状 net48**（net10 化は PR4.x 予定＝net10 publish 形態を Manager と揃えるため SDK 化と一括実施。本稿時点では未移行）。Godot や別 runtime が必要な特殊ケースは個別判断。
