...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    5. ネットワーク接続テスト — 「接続テストを実行」で、手前から順に **①ローカルIP取得 → ②ゲートウェイ疎通 → ③DNS解決 → ④インターネット接続 → ⑤インターネット速度 → ⑥共有サーバー接続 → ⑦共有サーバー読み込み速度 → ⑧Monitor接続** を確認し各段階を OK/NG＋応答時間・速度で表示（最初に×が出た所が原因）。実装: ②ゲートウェイは `route print` から既定ゲートウェイIPを取得し `ping`（locale非依存に "TTL=" で判定）、③DNSは `IP.resolve_hostname`、④インターネットは 1.1.1.1:443 へ TCP 接続（DNS非依存）、⑥共有サーバーは DBパス(UNC)から抽出したホストへ TCP:445。②〜⑦の ping/TCP/ファイル読みはブロッキングのため**別スレッド**で実行し `call_deferred` で結果反映（画面を固めない）。⑤インターネット速度は疎通OKのとき `HTTPRequest` で約5MBをDLして Mbps 算出（非同期）。⑦共有サーバー読み込み速度は `FileAccess` で DBファイルを読み MB/秒 算出（外部不要・LAN内のみ）。⑧Monitor は未実装のため「未実装」表示。⑥⑦共有サーバーは本番がそこからゲーム/DBを読むため最重要
