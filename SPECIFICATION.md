...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    3. 画面表示テスト — 単色全画面（黒・白・赤・緑・青・グレー50%）、グリッド（格子線+中央十字+四隅対角線にセーフエリア枠90%/80%を統合し解像度/スケーリング/端の見切れを一括確認）、カラーバー（放送風の縦色帯で色再現・色順確認）、グラデーション（黒→白の階調・バンディング確認）。モニター相性・拡大率・解像度ズレの事前発見用。実装は全画面 `Control` の `_draw()` で描画（画像アセット不要・解像度追従）
