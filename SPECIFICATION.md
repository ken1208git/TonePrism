...
- **言語/環境**: C# / .NET。runtime は Companion ごとに個別判断（#258、Manager は net10 へ移行済）: Updater=net48（常在ランタイムで自己更新の最後の砦）、LauncherAgent=net10（Manager と揃える、移行は PR4.x）。Godot や別 runtime が必要な特殊ケースは個別判断。
