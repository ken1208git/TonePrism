...
- **詳細**:
  ...
  - **バックアップ履歴一覧**（Manager v0.21.0 / DB v19 で file-scan 化）: `backups/` フォルダを `BackupCatalogService` で走査して表示（開始日時 / 完了日時 / 実行PC / トリガ / サイズ / ファイルパス）。種類はフォルダ位置（`auto/`・`manual/`・`safety/`）、日時・実行 PC はファイル名 `<種類>_<日時>_<host>.db`、サイズは `FileInfo` から導出する。**DB の `backup_log` テーブルは廃止**したため、reconcile / register / failed 自動掃除といった「DB ↔ ファイルのズレ補正」処理は不要になった（走査結果は常に実在ファイルのみ）。「状態」列は #200 で削除済。
