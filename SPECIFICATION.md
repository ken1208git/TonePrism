...
- **テーブル名**: `intro_slides`
- **説明**: イントロガイドのスライド (#253、DB スキーマ v21 で追加)。スクリーンセーバー → ブラウズ間に表示する案内スライド (展示の説明 / 楽しみ方 / 注意事項 等)。画像実体は `guide/` フォルダにファイル別管理し、本 table は相対パス (`image_path`) のみ持つ (games のサムネ/背景と同流儀)。他テーブルへの FK は無い独立テーブル。Manager の編集パネルで追加/編集/並び替え/画像選択、Launcher がスライドショー表示する。
- **カラム**:
  ...
  | duration_sec | INTEGER | NOT NULL DEFAULT 5, CHECK(1-60) | 自動送り秒数 |
