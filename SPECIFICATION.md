...
└── Companions/                               # 補助 exe 集約
    ...
    ├── (将来) WindowProbe/                    # Launcher 補助: ウィンドウ可視検知
...
##### WindowProbe（単発クエリ、#101 で実装予定）

- **説明**: 指定 PID のプロセスが可視ウィンドウを持っているかを検知する
- **用途**: Launcher の「ゲーム起動中 → プレイ中」遷移タイミングを正確化
- **入力**: コマンドライン引数 `<pid>`
- **出力**:
...
- **保存先**: プロジェクトルート（`toneprism.db` のあるディレクトリ）の `logs/{component}/` 配下
  ...
  - 配置の理由: 共有上の toneprism.db と同じ場所に集約することで Manager の将来的なログビューア UI から複数 PC の Launcher / Manager ログを 1 箇所で閲覧可能になる
  - 検出方法: exe 隣から最大 10 階層遡って `toneprism.db` を探す。見つからなければ exe 隣にフォールバック（PathManager と同じ思想だが Logger 専用に重複実装することで PathManager 起動ログのキャプチャ漏れを防ぐ）
...
- **多行 entry 継続行サポート**: `Logger.Error(string, Exception)` 等が出力する stack trace 等の継続行 (= `[ts] [LEVEL]` ヘッダを持たない行) は直前 absorb 行に追従して 1 entry に merge、parent log への書出も改行込みの単一 entry として記録する (= ERROR 1 行目だけ拾って stack trace 本体が silent に欠落する path を構造的に閉鎖)。
...
- **部分 absorb 事故防止 + crash fallback**: Logger の Shutdown marker (例: 「Updater 終了」行) が含まれる file は通常 path で absorb、含まれていない file は **`LastWriteTime` から 10 分以上経過していれば process 終了確定** (= segfault / kill / OOM 等の abnormal terminate) として absorb 対象に含める + WARN level で `[CRASHED?]` marker 付き notice。これにより (a) Updater が Phase B/C 中の race 時に終端未達のまま mark 済になる事故を構造的に防止 + (b) crashed Updater が永久に skip され続ける silent failure path も閉じる。
...
- **zip 内の正規構造**: zip ルートに `Install.bat` / `INSTALL_README.txt` / `show_folder_dialog.ps1` / `Launcher.bat` / `Manager.bat` を直置きし、`files/` wrapper の中に component フォルダ群を置く。`Install.bat` は `files/` を `<親>/TonePrism/` にコピーし、zip ルートの `Launcher.bat` / `Manager.bat` を `<親>/` 直下 (= TonePrism/ の 1 階層上) にコピーする規約。
  ...
  └── files/                                 # 配布物本体（中身が <親>/TonePrism/ にコピーされる）
      ...
      └── Companions/                        # 補助 exe 集約 (§2.4)
          ├── Updater/                       # Manager 置換 + 再起動 (§3.7.4)
          ├── (将来) WindowProbe/             # Launcher 補助 (#101)
...
- **例外: `responses/launcher_sessions/<pc_name>.json` は heartbeat 用専用 sub-folder**:
  - PR3b (Launcher v0.5.18 / Manager v0.11.0) で導入された **Launcher session tracking** の disk format は本 §6.5 の 3-state folder pattern (`pending` / `imported/` / `failed/`) の **例外** として扱う
  - 1 PC 1 file の heartbeat 用途 (content だけ update、file は増えない)、SQLite への INSERT 経路を持たないため `imported/` / `failed/` への移動 path は存在しない
  - 命名規約も `<type>_<unix_ts>_<source_pc>_<random_uuid>.json` 形式とは別の **`<pc_name>.json`** (= 1 PC 1 file が本質)
  - 詳細は §3.8.7 参照
...
│   │   └── bin/              # ← .NET 標準のビルド成果物（gitignored）
│   ├── (将来) WindowProbe/    # Launcher 補助（#101 で実装予定）
...
├── Companions/               # 補助 exe 集約（§2.4。Phase 3 以降で Updater 配布開始、それまでは dir 自体が存在しない）
