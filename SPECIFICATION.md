...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-02 | 1.10.55 | **(#250 PR1、Manager v0.21.0)** バックアップを `toneprism.db` 単体 → **DB + アセット (`games/` + `guide/`) の共有プール (CAS / SHA-256) 控え**に拡張する #250 の **PR1 (取得エンジン)**。§機能12 にアセット控えのサブ項目を追記、設定値リストに `asset_snapshot_enabled`/`asset_snapshot_retention_count` の 2 キーを追加。**方式選定**: 当初ハードリンク世代スナップショットで実装したが、プロジェクト全体が SMB 上＋ファイルサイズ管理がシビアで不採用（ハードリンクは 1 実体に複数エントリ＝単純合計が二重カウントし削減が見えない、実測 games/ の 63% が重複）。**共有プール**＝中身ごとに `asset_pool/<sha256>` に実体 1 個（名前同じ中身違いは別保存、中身同じは集約）、各世代は `asset_snapshots/<auto|manual>/<ts>[_host].manifest`（hash↔relpath の目録）。DB バックアップ成功直後に同一 timestamp/trigger で `AssetSnapshotService.CreateSnapshot` を best-effort（失敗/キャンセルしても DB バックアップ・`last_backup_at` を壊さない）。SMB 対策のハッシュキャッシュ（直近 manifest で relpath+size+mtime 一致なら再ハッシュしない）。retention は auto manifest のみ（既定30、manual 温存）＋ mark-sweep GC（残 manifest 参照 hash 以外の pool を削除、直近1h は grace）。SHA-256 でどんな容量集計でも実サイズ（実測 5.73→2.1GB）、SMB 越しでも有効。schema 版据置 v22（settings は K/V）。**復元 2 モード=PR3 / `RestoreReconciliationService` guide/ 拡張=PR2**。検証: Manager build 緑 + テスト 110 件合格（AssetSnapshot 16: 同一中身別パスで1個 / 名前同中身違いは別 / GC / retention / grace で直近未参照 blob は残る / blob mtime は配置時刻 / 履歴ありで sources 消失→Skipped / 非対称欠損→Skipped / DB games 登録済でフォルダ不在→Skipped / skip>0 は IsPartial 等 ＋ FileOperationServiceLongPath 7: UNC 長パス変換の回帰）。実機 SMB round-trip + 多ホスト並行 GC は別途。**レビュー round6**: 長パス `\\?\` の単純前置が UNC で構文不正パス (`\\?\\\server\…`) を生成し、SMB 直アクセスで列挙が全失敗→空の控えを Success 扱いする最重大欠陥を修正（UNC 分岐を `FileOperationService.ApplyLongPathPrefix`/`ForceLongPath` に集約、`localhost C$` 共有で end-to-end 実証、C1）。あわせて控えが除外リスト非適用＝丸ごと取る理由（Electron `node_modules` 等の実行時必須フォルダ保護）を本節に明記、進捗分母を走査と一致 (M1)。**レビュー round7**: silent failure 経路を堅牢化 — games/guide が「存在するが root 列挙失敗」を不在と同じ異常扱いに (M-1)、両 sub 不在時の新規 install/異常 判別に DB games 件数を権威軸として追加 (到達不能共有では manifest 履歴が空に見え判別不能なため、M-2)、pool blob の配置時刻 mtime stamp 失敗を Warn 化 (握り潰すと GC grace が静かに無効化、M-3)。**レビュー round8**: 深部フォルダの列挙失敗で部分的な控えになった世代を Success に潰さず `IsPartial` で UI/ログに「部分取得の可能性」を明示 (root のみ検出だった M-1 を深部 skip 集計に拡張、C1)、`.poolsize` 未計測時の実使用量 0 B 矛盾表示を「計測中」に是正 (A/L1)。多ホスト×初回フル取得が grace 超で取得中 blob を誤 GC (C2)・GC の毎回 pool 全列挙 perf (C3)・timestamp↔.db 名の衝突相関 (B-L2)・改行入り名での manifest 破損 (D-L3) は PR2/PR3 申し送り。 | Kenshiro Kuroga \& Claude |
