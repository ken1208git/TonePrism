...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    1. 入力チェック（タイトルは「入力チェック」、コントローラー接続状況も含む）— **接続中のコントローラー一覧**（パッド番号＋デバイス名、抜き差しで自動更新）と、**最後に来た入力**（キー/パッドボタン/スティック軸/マウスボタンを操作するとライブ表示）を表示。これで「パッドが効かない」を ①一覧に出ない=OS未認識（ケーブル/レシーバー/電池/ドライバ）/ ②一覧に出るが反応欄が変わらない=マッピング等の問題 / ③出て反応もする=正常（ゲーム側の問題）の3択で切り分け可能。入力は消費せず観測のみ（ナビは通常どおり）
