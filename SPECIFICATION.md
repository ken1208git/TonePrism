...
- **検出 trigger** (= dialog 表示の起点):
  ...
  - **編集操作前** (user 操作開始時): 各 SectionPanel (`GameSectionPanel` / `StoreSectionPanel` / `SettingsSectionPanel` / `BackupSectionPanel`) の save handler 内で `MainForm.CheckSessionConflictBeforeWrite(operationDescription)` を呼出し → 検出時 `SessionConflictDialog` (EditOperation context)、user Cancel でその操作中止。対象 button は 13 箇所 (ゲーム追加/編集/削除/バージョンアップ、ストア追加/編集/削除/並び替え ×2、データベース初期化、バックアップ作成/復元/削除)。read-only 系 (Refresh / OpenLogFolder 等) は対象外。
