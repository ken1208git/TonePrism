...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-01 | 1.10.53 | **(#209、Manager v0.20.0)** ゲーム編集で**個別バージョンを削除**できる機能を追加。`EditGameForm` の「ランチャーで表示するバージョン」ドロップダウン隣に「このバージョンを削除」ボタン。選択版を確認ダイアログのうえ即時削除し、版の DB 行 (version 別 developers は version_id FK cascade) + 版フォルダ `games/<gameId>/v<version>/` を消す。ガード: **最後の 1 版は削除不可** / 確認必須 / **アクティブ版 (games.version) を消したら残り最新版 (id DESC) に自動付け替え** (表示版 dangling 防止) / セッション競合は行 DML 扱い (Launcher 単独では警告せず、フォルダロックはリトライ案内)。整合は既存ゲーム削除と同じ 3-phase + rollback (フォルダを `.pending-delete` 退避 → DB 削除＋付け替えを 1 transaction → 物理削除)。新規 `Services/GameVersionDeletionService` (UI から分離) + `DatabaseManager.DeleteGameVersionAndReassignActive` + `VersionRepository` の in-transaction helper 群。**active 判定/フォルダ解決は DB・disk の確定値で行う** (フォーム上の pending リネームに依存せず dangling/orphan を防止)。スキーマ変更なし (version_id FK cascade は schema v18 既存)。検証: 一時 DB で `VersionDeletionTests` 4 ケース (非アクティブ削除据え置き / アクティブ削除付け替え / id DESC 付け替え / developers cascade)、全 80 テスト合格。テスト基盤に `DatabaseManager(DatabaseConnection)` 内部 ctor 追加 (#239)。docs `usage/games.md` 編集セクションに版削除手順追記。**実機 UI 確認は別途必要** (破壊的のため作業 DB で実行不可、DB コピーで目視)。 | Kenshiro Kuroga \& Claude |
