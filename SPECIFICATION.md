...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-01 | 1.10.50 | **(#278、Manager v0.19.3 / Launcher v0.10.1)** 文化祭当日に **Launcher を起動したまま Manager で編集**できるようセッション競合警告をスコープ分け（①）＋ブラウズの DB ハンドル開放（②）。検証事実: Launcher は DB 読み取り専用（SELECT のみ、heartbeat は file）で `journal_mode=DELETE`+`busy_timeout` 下では Manager の行 write と Launcher の read を SQLite が安全に調停（write-write 競合なし・持続ロックなし）。**①**: 警告要否を純ロジック `Services/SessionConflictPolicy.ShouldWarn` に抽出。**別 Manager 検出時は操作種別問わず常に警告**、**Launcher 単独稼働なら `toneprism.db` をファイルごと差し替える操作（"バックアップ復元"/"データベース初期化"）だけ警告し通常の行編集は警告しない**。`MainForm.CheckSessionConflictBeforeWrite` から委譲、`SessionConflictPolicyTests` 29 ケースで検証。文字列ラベル判定のため DB ファイル置換操作追加時は `IsWholeDbReplacingOperation` に足す規約をコメント明記。**②**: `store_browse` が DB 接続を表示中保持するのをやめ `_ready` のロード後に `close()`（build はメモリ上の `_sections`/`section.games` のみ使い DB 不使用）。「すべて見る」「すべてのゲーム」ナビは repo の `is_open()`→`open()` で一時再接続→取得後 close。これで store 表示中も DB ファイルハンドルを握らず、Restore/DB 初期化が表示中でも衝突しない（screensaver/カルーセル/intro_guide と同じ open→close に統一、①の Restore 例外の根拠を縮小する defense in depth）。検証: 同梱 Godot 4.6.2 ヘッドレスで store_browse `_ready` 後 `is_open()=false`・ナビ相当の自動再接続を確認。関連: 版削除のファイル整合=#209、遠隔再読込=#263（割り切り）、Monitor=#261。 | Kenshiro Kuroga \& Claude |
