...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-05-31 | 1.10.49 | **(#253 part 2/3 design 変更、Manager v0.19.0、DB v21→v22)** イントロガイド → **「初回説明」** に呼称変更。ユーザー feedback で**自動送りを廃止**し手動ナビ（全スキップ + 次へ/戻る）方針に変更したため、§7.3 テーブル13 から `duration_sec` 列を削除（`MigrateV21ToV22` は CHECK 付き列のため table recreate で削除、FK/子テーブル無しで安全）。説明文も「毎回表示（各来場者にとって初回）/ Launcher は手動ナビ」に更新。Manager の Model/Repo/UI（タブ名・フォーム・秒入力欄）からも秒を全削除。`ExpectedSchema` 同期。Launcher の手動ナビ実装は part 3。 | Kenshiro Kuroga \& Claude |
| 2026-05-31 | 1.10.48 | **(#253 part 2/3、Manager v0.19.0)** イントロガイドの Manager 編集 UI を追加。§7.2 本番インストール構成に **`guide/` フォルダ**（イントロガイドのスライド画像置き場、`games/` の隣、`intro_slides.image_path` が `guide/<file>` で参照）を追記、アップデート保護対象 + #250 バックアップ拡張対象に含める旨を明記。Manager 側: 「イントロガイド」タブ（`IntroGuidePanel` UserControl、`StoreSectionPanel` ミラー）+ スライド編集 Form（`IntroSlideEditForm`、本文/画像/秒数/表示、画像は `guide/` へコピーし相対パス保存）+ 画像取り込み helper（`IntroGuideAssetHelper`、衝突自動 suffix）。DB write 前 session conflict check 準拠。part 3（Launcher のスライドショー表示）は後続。コードのみ・スキーマ不変（part 1 の v21 を利用）。 | Kenshiro Kuroga \& Claude |
