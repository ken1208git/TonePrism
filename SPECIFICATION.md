...
- [ ] **`Launcher/project.godot` の `[application] config/version` 行の format を変更する場合は `Manager/Services/VersionInventory.cs` の `ConfigVersionRegex` も同期更新する** (#281): Launcher 版数の SoT は project.godot の `config/version="X.Y.Z"` 1 箇所 (version.gd は数字を持たない薄いアクセサに変更済)。Manager UI Phase 4 の「アップデート」タブが Launcher 版数表示にこの行を parse し、`config/version="X.Y.Z"` 形式を literal match する。Manager は Godot を実行できないためファイルを直接 parse する設計。format 逸脱で silent null 返却 → UI が「Launcher: 不明」と表示する cross-component coupling。なお project.godot は Godot エディタが管理する自動生成ファイルのため通常は手で format を崩さないが、SoT として `<install>/Launcher/project.godot` が release zip に同梱される (`Release.ps1 $filesTemplates` / `$script:BundleManifestFiles`、旧 version.gd 同梱を置換)
