...
##### 機能22: 管理者用終了確認機能（Global Exit Confirmation）

- **説明**: Alt+F4 やウィンドウの閉じるボタンなどで終了要求があった際に、即座に終了せず、管理者向けの確認ダイアログを表示する機能
- **優先度**: 高（必須）
- **詳細**:
  - Alt+F4 キーフック
  - 管理者（スタッフ）向け警告メッセージの表示
  - キャンセル可能
  - どの画面にいても、最前面にオーバーレイする共通ダイアログシステムを使用
...
このプロジェクトでは、**4つのバージョンシステム**を使用します：
...
##### 4. Database Schema Version（データベース構造）
...
| 2025-12-27 | 1.2.5 | データベース技術仕様を追加：WALモードによる同時アクセス対応、リトライ機構、エラーハンドリングの詳細を6.5データの整合性セクションに追加。パス保存方式（相対パス）を7.3 SQLiteデータベース設計に明記。 | Kenshiro Kuroga |
| 2025-12-27 | 1.2.4 | データベーススキーマの仕様を実装に合わせて更新：developersテーブルのlast_nameカラムをNULL可に変更、settingsテーブルのidカラムの制約をCHECK制約に明確化 | Kenshiro Kuroga |
