...
- **ビルド管理**: 各 Companion は独立 csproj。`Release.ps1` が runtime に応じて個別にビルドする形（Updater=net48 は msbuild、LauncherAgent=net10 は dotnet publish。Companion ごとに個別判断。リポジトリルート `.sln` は現状なし、必要になった段階で追加検討）。
...
- [ ] **コンポーネント版数の公開前ゲート（exe ↔ SoT、#258 PR4 で一般化）**: コンポーネントが **(a) Manager「アップデート」タブに exe FileVersionInfo 経由で版数表示され、かつ (b) 版数が stamp パイプライン（Godot rcedit / net10 apphost）経由で exe に焼かれる** 場合、`Assert-<Name>Version`（`Build-<Name>` 直後・upload 前に **exe FileVersion ≠ SoT を hard fail**）を **必須**とする。理由: stamp の一手抜けで silent に嘘版数を焼く事故（#283）を公開前に止めるため。**除外**: net48 等で版数が**直接コンパイルで exe に入る**コンポーネント（Updater 等）は exe FileVersion が SoT と自明に一致するため不要（= net48 時代の Manager にゲートが要らなかったのと同根）。**現状**: Launcher=`Assert-ExportedLauncherVersion`（rcedit 派生）/ Manager=`Assert-PublishedManagerVersion`（net10 apphost 派生、#258 PR4）。**将来 (a)(b) 両成立で必須化**: LauncherAgent（net10 化 PR4.x ＋ 版数表示 #310 の両成立後）、Monitor（net10 で本格実装時）。条件ベースなので、コンポーネントが net10 化／版数表示化した時点で「除外→必須」に切り替わる（個別に覚えるのではなくこの条件で判定）。**相補ゲート（CHANGELOG ↔ SoT、#359）**: 本ゲートは「exe が嘘版数を焼かないか」を見るが、「CHANGELOG の各コンポーネント版数見出しが SoT と一致するか（= bump し忘れ）」は別軸で **net48 含む全部品に効く**（exe ゲートが無力な Updater の bump 忘れもこちらで捕捉）。SoT の向きは Bundle と逆（Bundle は CHANGELOG が SoT、コンポーネントは AssemblyInfo / project.godot が SoT で CHANGELOG は派生記録）。「1 PR 1 bump」規約の機械化として #359 で追加予定
