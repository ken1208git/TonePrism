...
- **詳細**:
  ...
  - **ファイル名規則**: `<種類>_YYYYMMDD_HHmmss_<host>.db`（種類 = `auto` / `manual`、host = 実行 PC 名をファイル名向けに正規化した値）。同一 PC・同一秒の衝突時のみ `_2` / `_3` を付与。退避は `backups/safety/safety_YYYYMMDD_HHmmss.db`。移行前の旧 `toneprism_YYYYMMDD_HHmmss.db` も走査対象（種類不明のため安全側で manual 扱い）
