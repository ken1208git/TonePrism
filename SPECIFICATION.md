...
- **取り込み（Manager 側）**: 3-state folder + 2-phase 方式
  ...
  **`responses/` 直下 vs subfolder の責務分離**:
  ...
  - **⚠ (#295 改定) 起動時/定期の「バックアップフェーズ」は廃止 → 操作単位へ移行**: #295 で起動時・定期の独立したバックアップトリガは撤去し、バックアップは **データ変更操作の成功直後に `SessionBackupCoordinator.RunAfterOperation` が取る操作単位方式**へ移行した（§機能12）。下の旧 3 ステップとトリガ表の「バックアップ」「`imported/` 削除」列は **#295 以前のモデル**で、取り込みフェーズ実装時に新モデルへ合わせて改定する。改定時の縛り:
