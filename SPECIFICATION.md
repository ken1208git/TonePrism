...
- **詳細**:
  ...
  - **機能一覧**:
    ...
    **診断・テスト:**
    ...
    3. 画面表示テスト — 開始すると複数のテストパターンを**順番に全画面表示**する（任意キー/クリック/パッドで次へ、Esc/Bで中断、最後まで行くと自動でメニューへ戻る）。左上に進捗とパターン名・操作ヒントを小さく表示。表示順: ジオメトリ/クロスハッチ（細密32px+粗128pxのクロスハッチ+四隅対角線+中央&四隅の真円〔画素アスペクト/歪み確認〕+中央十字+セーフエリア枠90%/80%で解像度/スケーリング/歪み/端の見切れを一括確認）→ カラーバー（放送規格 SMPTE RP 219 / ARIB の HD カラーバー: 上段75%カラーバー+両端40%グレー袖 / リバース段 / 0-100%ランプ段 / PLUGE段で色再現・色順・黒レベルを確認）→ 単色（白・黒・赤・緑・青・グレー50%）。階調・バンディング確認はカラーバーのランプ段でカバーされるため、黒→白グラデーションの単体表示は設けない。モニター相性・拡大率・解像度ズレの事前発見用。実装は全画面 `Control` の `_draw()` で描画（画像アセット不要・解像度追従）
