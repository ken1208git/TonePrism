...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-06-01 | 1.10.54 | **(#211/#212/#291、Manager v0.21.0 / Launcher v0.10.3)** ストアセクション編集 UI に条件付き規則を追加し §7.3 store_sections の `section_type`/`max_display_count` 説明に明記。**#212**: スライドショー/タイルグリッドはソースを `manual` または `random` に制限 (背景画像/表示テキストはゲーム自身にフォールバックするので random なら自動でも崩れず、ローテーションするヒーロー枠に合う。popular/genre 等の絞り込み系は厳選枠に不向きなため不可)。通常カテゴリ行は手動＋全自動可。許可外ソースは編集時に手動へ coerce + 案内 (combo は無効化せず 2 択選択可)。**#212 関連修正**: Launcher の `build_tile_grid_section` が `max_display_count<=0` を「上限なし=最大3枚」と扱うよう修正 (従来 `mini(0,3)=0` で手動タイルグリッドが空表示になる #211 起因の回帰)。スライドショーは `max>0` でスライド枚数を上限し、生成枚数を container `slide_count` meta に持たせて `_switch_slide` の wrap を整合 (ランダム時の「特集 N 枚」)。**#211**: `max_display_count` はランキング系 (popular/recently_played/random) のみ編集可 (TOP5 等の特集枠で上限が有効)、手動 (割当全件) と条件系 (genre/players_*/difficulty/play_time/online/controller + **recent=新作**[今年の release_year を display_order 順=本物のランキングでないため condition 扱い、#290 review]、条件一致全件) はグレーアウト + `0` 保存 (= 上限なし、Launcher の `max_display_count<=0` 解釈)。**#291**: 新ソース `release_year:YYYY` (制作年指定) を追加 — 任意年の `release_year` を display_order 順で全件 (新作=今年固定 に対し任意年)、条件系として max グレーアウト。Launcher `store_section_repository.gd` に `release_year:` 分岐追加。**#290 review**: 難易度/プレイ時間の値指定を数字直打ちからゲーム編集同様のラベル付きドロップダウンに変更。スキーマ変更なし。docs `usage/store.md` も更新。実機 UI 確認は別途必要。 | Kenshiro Kuroga \& Claude |
