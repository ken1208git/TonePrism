...
- **詳細**:
  ...
  - **リストア機能** (Manager v0.17.0 で atomic 方式へ改修):
    ...
    2. 確認ダイアログ表示（「全Launcher を停止してください」「現DBは退避されます」、フルパスは `BackupPathResolver` で解決した値）
...
toneprism.db (SQLiteデータベースファイル)
  ...
  └─ backup_logテーブル (データベースバックアップ履歴 - v0.8.0で追加)
       ...
       ├─ error_message
       └─ trigger_type ('manual' | 'auto' | 'safety' | 'restore')
```
...
- **テーブル名**: `backup_log`
- **説明**: データベースバックアップの実行履歴を格納するテーブル（Manager v0.8.0 / DB スキーマ v9 で追加。v10 で `trigger_type` の CHECK 制約に `'safety'` を追加。v12 で `relative_path` 列を追加。v16 で `trigger_type` CHECK に `'restore'` を追加 = リストア監査ログ）
- **カラム**:
  ...
  | trigger_type | TEXT | NOT NULL CHECK ('manual', 'auto', 'safety', 'restore') | 実行トリガ種別（v10 で 'safety' = リストア前の自動退避 を追加、v16 で 'restore' = リストアイベント自体の監査ログ を追加） |
