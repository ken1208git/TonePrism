...
- **詳細**:
  ...
  - ゲームファイルの差し替え
  - **画像の差し替え仕様 (Manager v0.18.0 以降、M10)**:
...
- **詳細**:
  ...
  - **ファイル名規則**: `<種類>_YYYYMMDD_HHmmss_<host>.db`（種類 = `auto` / `manual`、host = 実行 PC 名をファイル名向けに正規化した値）。同一 PC・同一秒の衝突時のみ `_2` / `_3` を付与。退避は `backups/safety/safety_YYYYMMDD_HHmmss.db`。移行前の旧フラット形式 `toneprism_YYYYMMDD_HHmmss.db` / `prism_YYYYMMDD_HHmmss.db`（v0.20.0 以前）も走査対象だが、auto/manual の区別がファイル名に無く復元不能なため**「不明」表示**とする（retention は `auto/` フォルダ限定なので不明扱いでも自動削除されない）
...
toneprism.db (SQLiteデータベースファイル)
  ...
  # backup_logテーブルは DB v19 (Manager v0.21.0) で廃止。バックアップ履歴は backups/ フォルダ走査
...
- **状態**: **廃止**。Manager v0.21.0 / DB スキーマ v19 (`MigrateV18ToV19`) で `DROP TABLE`。テーブル番号 11 は欠番、`ExpectedSchema` からも除去（`VerifySchema` 対象外）。
...
| 日付 | バージョン | 変更内容 | 変更者 |
| --- | --- | --- | --- |
| 2026-05-29 | 1.10.47 | §7.3 `backup_log` テーブルを **廃止** (DB v19、Manager v0.21.0、`MigrateV18ToV19` で `DROP TABLE`)。バックアップ履歴メタデータを「バックアップ対象である `toneprism.db` の中」に持つ設計が、復元で DB が置き換わるたびに履歴とディスク実ファイルのズレを生み、それを埋める reconcile / register コードがバグの温床 (失敗復元が `success` 化する Medium 欠陥等) になっていた根治。履歴は `backups/` フォルダ走査 (`BackupCatalogService` — 種類 = フォルダ位置、日時・実行 PC = ファイル名 `<種類>_<日時>_<host>.db`、サイズ = `FileInfo`) から導出する方式へ移行。あわせて §7.2 階層図から `backup_log` を除去、§3.x「バックアップ履歴一覧」「個別削除 UI」「ファイル名規則」を file-scan ベースに更新、実行 PC をファイル名 host に埋め込み (旧アドホックな同秒 LAN 衝突回避を一本化)、復元監査は `safety_*.db` で代替・失敗履歴は Logger のみに。マルチ PC 協調 (`settings.last_backup_at` lease / `settings.restore_lock_owner`) は `settings` テーブルにあり不変。`ExpectedSchema` からも `backup_log` を除去 (VerifySchema 対象外)。詳細は CHANGELOG Manager v0.21.0 セクション。 **追加 (#247、本 entry 内加筆)**: §7.3 テーブル1 `games.play_time` に `CHECK(1-3)` を DB レベルで復元 (DB v19 → v20、`MigrateV19ToV20`)。`difficulty` の CHECK は残るのに `play_time` だけ欠落していた drift (新規DB / 初代DB / SPEC の三者不一致、`VerifySchema` は列名のみで CHECK 差を検知しない) を解消。`games` は親テーブル (game_versions / developers / play_records / surveys / store_section_games が ON DELETE CASCADE、launcher_surveys が SET NULL で参照) で、`DROP TABLE games` の暗黙 DELETE が CASCADE 発火で子を全消去するため、`defer_foreign_keys` (検査遅延のみで CASCADE action は止まらない、sqlite3 実測確認) ではなく `InitializeDatabase` を「migration 検出時のみ transaction 開始前に `PRAGMA foreign_keys=OFF` → commit 前に `foreign_key_check` で整合検証」する形に変更 (通常起動は FK=ON のまま既存挙動維持、SQLite 公式のスキーマ変更手順)。範囲外 play_time 残存時は hard-fail せず skip + retry。あわせて `CheckAndMigrateDatabase` の v0 fast-path が `MigrateV10ToV11` (surveys/play_records の旧スキーマ drift 修正) を飛ばす非対称も修正 (冪等な call を stamp 前に追加、新規DBでは no-op)。移行 SQL は同梱 sqlite3 の実データ round-trip + 実機 Manager の skip/rebuild 両経路で検証済。 | Kenshiro Kuroga \& Claude |
| 2026-05-28 | 1.10.46 | §7.3 `backup_log.trigger_type` CHECK 制約に `'restore'` を追加 (DB v16、Manager v0.18.0、H4)。v0.17.0 が release notes で「監査ログ完備」を謳いつつ実装は file log のみで DB に痕跡を残さない drift があり、リストアイベント自体の audit trail を `backup_log` 行として永続化するため。`MigrateV15ToV16` で `backup_log` テーブルを recreate して CHECK 拡張 (V9→V10 と同 pattern、既存行は影響なし)。あわせて (a) `game_versions(game_id, version)` UNIQUE INDEX を `COLLATE NOCASE` に rebuild (DB v17、M3、`v1.0.0` と `V1.0.0` の case 違い重複を DB レベルで弾く最後の砦)、(b) `RestoreService` に LAN advisory lock (`settings.restore_lock_owner`) を導入し他 PC の write を `MainForm.CheckSessionConflictBeforeWrite` 経由で復元中 fence (5 分超過は stale 扱いで自動失効、H5)、(c) ゲーム追加・編集・バージョンアップで判明した High 5 / Medium 8 / Low 7 件の欠陥修正 (詳細は CHANGELOG Manager v0.18.0 セクション)。**round 3 追加 (本 entry 内加筆)**: §2.2 機能 3「ゲーム情報編集機能」に画像差し替え仕様を追記 (Manager v0.18.0 M10、ゲームフォルダ外画像の自動コピー + 同名衝突時 `ImageNameConflictDialog` 経由のリネーム編集 UX + 古い画像非削除方針)。あわせて (d) H6 `EnsureGameVersionsVersionUniqueIndex` の CREATE 文に `COLLATE NOCASE` を付与 (新規 DB の `currentVersion=0 → SetDbVersion(17)` 経路で `MigrateV16ToV17` の NOCASE rebuild が永久に skip される欠陥を構造閉鎖、§7.3 の最終形は不変)、(e) M9 `RestoreService` の catch 経路で `tempPath` を best-effort 削除 (復元失敗連続時の disk-full chain 緩和)、(f) M11 `ReleaseRestoreLock` の `LIKE @prefix` を parse + 完全一致に変更 (PC 名の `_` / `%` で他 PC lock を巻き込み削除する経路を構造閉鎖)、(g) M12 `TryAcquireRestoreLock` の SELECT を `@k` parameterized 化 (同 file 内 3 経路の非対称解消)、(h) M13 `VersionUpForm_Load` で baseVersion の相対 path を `ToAbsolutePath` で絶対化してから textbox に入れる (CWD 基準 `File.Exists` で永久 block する UX 退行を構造閉鎖)、(i) M14 `EditGameForm.ApplyRelativePaths` に `NormalizeRelative` helper 二段目 fence (`Path.IsPathRooted` 検出時に null 格下げ + Logger.Warn) を導入 (絶対 path の silent 流入を構造閉鎖、DB 保存値を「相対 path / null」のみに collapse する契約強制)、(j) M15 `BackupService.ApplyRetention` の DB delete 失敗時に `MarkFailed` で `status='failed'` に格下げ (`GetAutoSuccessRetentionTargets` の count から外して retention 件数 drift を構造閉鎖)。 **round 6 追加 (本 entry 内加筆)**: §7.3 テーブル7 `game_genres` を「DB v18 で廃止」に変更し、§7.4 リレーション・ER 図・機能「ゲーム削除」の CASCADE 一覧からも除去 (ジャンルは `games.genre` / `game_versions.genre` のカンマ区切り保持に一本化済で本テーブルは実装上ずっと未使用、`MigrateV17ToV18` で DROP・`ExpectedSchema` からも除去済の SPEC ドリフト解消、AGENTS.md「SPEC ↔ ExpectedSchema 同期」則)。テーブル番号は後続テーブルの参照を壊さないため欠番として残す。 | Kenshiro Kuroga \& Claude |
| 2026-05-28 | 1.10.45 | §機能12 リストア機能の手順記述を現行実装 (Manager v0.17.0 / atomic 方式) に同期。旧記述は (a) safety ファイル名 `safety_before_restore_HHmmss.db` (現行は `safety_HHmmss.db`)、(b) 「`toneprism.db` / WAL / SHM を**削除してから**バックアップをコピー」 (現行は tmp に先にコピー後 `File.Replace` で atomic 置換 = コピー失敗・中断で `toneprism.db` が一時欠落するパスを構造的に閉鎖)、(c) `SessionConflictHelper` チェック・`backups/safety/` 配下保存・`RestoreReconciliationService` での DB↔`games/` 突合と `RestoreReportForm` 表示が未記載、(d) Reconciliation 前に必ずスキーマ migration を走らせる順序保証 (= 古い schema バックアップ復元時の "no such column" 空振り対策、v0.17.1 で順序固定) が未反映、と複数点で実装からドリフトしていた。実コード変更は v0.17.1 で別途、本 bump は SPEC のみ。 | Kenshiro Kuroga \& Claude |
| 2026-05-28 | 1.10.44 | §7.3 `game_versions` に UNIQUE INDEX `idx_game_versions_game_id_version` (`UNIQUE(game_id, version)`) を追記 (#234 ② フォローアップ、DB v15、Manager v0.16.6)。v0.16.5 で「本番 live data に既存 dup があると migration が失敗しうる」ため app-level guard のみで見送っていた DB レベル制約を、本番本格運用前の安全な窓のうちに追加。同一ゲームに同一バージョン番号が 2 行入る silent corruption の最後の砦で、複数 PC 同時操作の「check → write」間 race（app-level dup-check ではすり抜けうる）を DB が確実に弾く。version は raw 文字列比較（意味的正規化はアプリ層責務）。既存 DB への追加 migration `MigrateV14ToV15` は重複残存時は throw せず skip + 警告し user_version 据え置きで次回再試行（`MigrateV10ToV11` の "data residual → retry" パターン踏襲、起動継続）。`ExpectedSchema` は列ベース検証のため index 追加では変更不要（§7.6.3）。あわせて `VersionUpForm.ValidateInput` の実行ファイル「コピー元フォルダ内」チェック欠落（サムネ/背景にはあり exe だけ非対称）と `EditGameForm` の `arguments` 防御経路 null 正規化漏れも 3 フォーム整合のため修正。 | Kenshiro Kuroga \& Claude |
| 2026-05-28 | 1.10.43 | §3.8.5 を #234 追加精査の 2 点で更新 (Manager v0.16.5、コード変更は Manager のみ)。**(1) parent gameFolder retention を対称化**: 旧記述が「別 issue 候補」としていた `parentCreatedThisCall` flag 方式を `AddGameForm` に実装。`baseGameFolderCreated` flag で「この追加操作が親 `games/{id}/` を新規作成したか」を記録し、rollback helper `CleanupCopiedFoldersOnRollback` が **「今回新規作成した」かつ「親が空」** の両条件成立時だけ親も削除する (他 version 共存ケース=親が非空 / race で後から出現した親=flag false には触らない二重防御)。新規追加失敗後に空 `games/{id}/` が残り次回追加で #120 警告が誤 trigger する受容仕様を解消。**(2) #120「既存フォルダあり」を wipe & recreate 方針に変更**: 追加 OK 時に既存フォルダがある場合を旧「retain して続行」から「OK=中身ごと削除して新規作成」に一本化。旧方針の (a) 既存に同名版フォルダがあると rollback が既存版を巻き込み削除する footgun、(b) 警告文「自動削除されません」と実装の矛盾、を解消。`wipeExistingOnCopy` gate を立て `CopyGameFolder` が worker thread で親を丸ごと削除→再作成 (削除失敗時は明示中断)、警告文言も「OK=全削除して作り直す」に書換え。 | Kenshiro Kuroga \& Claude |
