...
- **詳細**:
  ...
  - **自動バックアップ（#295 で再設計）**: データ変更操作（ゲーム追加/編集/版up/削除・ストアセクション・イントロスライド等）の**成功直後**に取る「操作単位 / replace-in-session」方式。1 Manager 起動 = 1 自動世代（同セッション内は前世代を消して上書き、retention は直近 N **セッション**）。DB（~124KB）は毎操作、重い games/guide のアセット取得（~6GB）は games/guide を実際に変える操作のときだけ。設定「変更があったら自動でバックアップする」(`backup_auto_enabled`) で ON/OFF。手動は別枠で温存。実装は `Services/SessionBackupCoordinator.cs`（各 Form の `AssetsChangedOnDisk` で「ディスクのゲーム本体を触ったか」を判定し不要な走査を回避）。
