...
- **Companions**: 主要アプリを補助する独立 exe 群です（SPEC §2.4）。Manager の自己置換に使う **Updater** と、Launcher がゲームの可視/前面ウィンドウ状態を検知するために使う **WindowProbe** を含みます。
...
- Companions（Updater）は Manager の自動アップデート適用フローで使用されます。WindowProbe は Launcher のゲーム起動→プレイ中遷移の正確化とランチャー前面化異常検知に使用されます。
