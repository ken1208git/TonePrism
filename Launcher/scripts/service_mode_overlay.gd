...
func _set_games_test_desc(mode: String) -> void:
	...
	match mode:
		"exists":
			_games_test_desc.text = "各ゲームの実行ファイル(exe)が実際にあるかを、起動せずに全件まとめて確認します。LAN 共有の未接続・フォルダ移動・パス設定ミスで実行ファイルが見つからないゲームを一覧で洗い出せます。ゲームを起動しないので、固まったり落ちたりする心配がなく、一番手軽に確認できます。"
		"auto":
