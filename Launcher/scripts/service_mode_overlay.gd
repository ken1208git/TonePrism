...
func _set_games_test_desc(mode: String) -> void:
	...
	match mode:
		...
		"play":
			_games_test_desc.text = "選んだゲームを実際に起動して遊んで確認します。終了は原則、遊んでいる最中に HOMEキー / Guideボタンで本番と同じ中断メニューを出して「ゲームを終了して記録へ」を選びます（試遊中は「続ける」との 2 択）。ゲームごとに『正しく遊べたか』『中断メニューが動いたか』の 2 つを 〇× で記録して次へ進みます。『実際にちゃんと遊べるか』＋『中断メニューが効くか』を見る最終確認向けです。"
...
func _build_games_playtest() -> void:
	...
	_add_text("チェックしたゲームを 1 本ずつ起動して試遊します。終了は原則、遊んでいる最中に HOMEキー / Guideボタンで本番と同じ中断メニューを出し、「ゲームを終了して記録へ」を選んでください（中断メニューの動作確認を兼ねるため）。", C_TEXT)
...
func _pt_start() -> void:
	...
	if not any:
		return
	_show_modal(
		"チェックしたゲームを順番に試遊します。\n\n【終了のしかた】原則、遊んでいる最中に HOMEキー（またはコントローラーの Guideボタン）で中断メニューを出し、「ゲームを終了して記録へ」を選んでください（中断メニューが正しく動くかの確認を兼ねます）。\n\n中断メニューが出ない・操作できないときは、ゲーム自体の終了操作で閉じてください。\n\nゲーム終了後に「遊べたか」「中断メニューが動いたか」の 2 つを 〇× で記録して次へ進みます。\n\n開始しますか？",
