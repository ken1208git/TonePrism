...
func _set_games_test_desc(mode: String) -> void:
	...
	match mode:
		"exists":
			_games_test_desc.text = "各ゲームの実行ファイル(exe)があるかを、起動せず全件まとめて確認します。ファイルが見つからないゲームを洗い出せます。"
		"auto":
			_games_test_desc.text = "（実装予定）各ゲームを自動で起動→終了し、起動できるか(OK/NG)だけを一覧で確認します。"
		"play":
			_games_test_desc.text = "（実装予定）選んだ 1 本を実際に起動して遊んで確認します。終了は手動です。"
